		...
		if($_val!="" and $_key!="btnsubmitform" and substr($_key,0,1)!="_" and substr($_key,-2)!="_2"){
			...
			switch ($_prompt):
				...
				case "json": //Multipla escolha
					...
					break;
				default:
					echo ("Informação de Operador default para clausula [WHERE][".$_arrModConf["tab"]."][".$_key."] n&atilde;o foi encontrado nas defini&ccedil;&otilde;es do Mà³dulo [". $_modulo."]");
...
//die("psqfull:".$_arrModConf["psqfull"]."iget:".$iget);
	...
	while ($_i < $_numcampos) {
	    ...
	    if($_arrpagpsq[$_arrModConf["tab"]][$_metacmp->name]["visres"] == 'Y' or $_mostrartodascolunas==true){
			...
			$ARRRESULTADOS["cols"][$_metacmp->name] = htmlentities($rottmp, ENT_QUOTES, 'ISO-8859-1');
			$icols++;
	    }
	    $_i++;
	}
            ...
            foreach ($_arrhlcond[$_modulo] as $_fldcond => $_cond) {
        		if ($_boocond==false){
        			...
        			switch($_cond["cond"]){
						...
						echo 5;
        					if ($_row[$_cond["col"]] != $_cond["valor1"]){
								echo $_cond["col"].' -> '.$_row[$_cond["col"]].' = '.$_cond["valor1"].'<br><br>';
        						echo $_tmphlcolor = $_cond["cor"];
        						$_boocond = true;
        					}
        					break;
        				case ">":
