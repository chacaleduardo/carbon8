<script>
    ...
    function getEtlRel(iddash){
        ...
        /*
        CB.modal({
            url:"./report/_8repprint_dash.php?_open_chart=Y&_modulo=etlconf&idobjeto="+iddash+"&_idrep=200&_fds="+oDates.fDate+"-"+oDates.lDate+idempresaUrl
        });
        */
    ...
    function recarregarIndicadoresCron(vthis, idPainel, idGrupo, idLp){debugger
    ...
    function recarregarIndicadores(vthis, idPainel, idGrupo, idLp){debugger
        if(!$(vthis).hasClass('updating')){
            $(vthis).addClass('updating');
            var listaCardRequest = [];
            $(`#dashpanel-conf-${idPainel}-${idGrupo}-${idLp} .card-space div[iddashcard]`).each(function(i, o){
