...
function alterarParaStatusPendente(idfluxo, idfluxostatushist, idfluxostatus, idstatusf, statustipo, idfluxostatuspessoa, ocultar, prioridade, tipobotao, idcarrimbo, log = 0) {
	const motivoPendencia = $('#motivo-pendencia').val();

	if(!motivoPendencia) return alertAtencao('Selecione um motivo.');

	ST.customFunction = false;
	ST.alterarstatus(idfluxo, idfluxostatushist, idfluxostatus, idstatusf, statustipo, idfluxostatuspessoa, ocultar, prioridade, tipobotao, idcarrimbo, log)	
...
$( document ).ready(function() {
	if(typeof ST != 'undefined') {
		ST.customFunction = (idfluxo, idfluxostatushist, idfluxostatus, idstatusf, statustipo, idfluxostatuspessoa, ocultar, prioridade, tipobotao, idcarrimbo, log = 0) => {
			if(statustipo != 'PENDENTE') {
				ST.customFunction = false; 
				return true;
			};

			const corpo = `<div class="w-100">
								<label for="">Motivo da pendência <strong class="text-danger">*</strong> </label>
								<select name="_1_u_amostra_motivopendencia" id="motivo-pendencia" class="form-control">
									<option value="SEMASSINATURA">Sem Assinatura</option>
									<option value="AUSENCIAINFORMACAO">Ausência de informação</option>
									<option value="TESTEINCOMPATIVEL">Teste Incompatível</option>
								</select>
							</div>`;
			const rodape = `<div class="w-100 d-flex flex-between">
								<button class="btn btn-primary" onclick="alterarParaStatusPendente(${idfluxo}, ${idfluxostatushist}, ${idfluxostatus}, ${idstatusf}, '${statustipo}', '${idfluxostatuspessoa}', '${ocultar}', '${prioridade}', '${tipobotao}', '${idcarrimbo}', ${log})">
									Confirmar
								</button>
								<button class="btn btn-link text-primary" onclick="CB.oModal.modal('hide')">Cancelar</button>
							</div>`;

			CB.modal({
				titulo: 'Status TEA Pendente',
				corpo,
				rodape
			});
	...
	VMasker(document.getElementById('numeroanimais')).maskMoney({
	...
	function buscaRegrasDoMapa(idespeciefinalidade){
		return $.ajax({
			type: "get",
			url : "ajax/buscaregrasmapa.php?tipo=regrasmapa&idespeciefinalidade="+idespeciefinalidade,
			success: function(data){
				try {
						dadosMapa = JSON.parse(data);
						if(dadosMapa){
							$("#regramortalidademapa").removeClass("hidden").text(`Regra de mortalidade MAPA: ${dadosMapa[idespeciefinalidade].mortalidademapa ?? "Não informado"}`);
							$("#regraidademapa").removeClass("hidden").text(`Regra de idade MAPA: ${dadosMapa[idespeciefinalidade].idademapa  ?? "Não informado"}`);
						}
					} catch (error) {
						console.error(error);
					}
...
function mostraInputHidden(target){
	$(target).removeClass("hidden");
}
function removeReadonly(target){
...
function confInputs(inv){ 
	...
	$(".rowDin [id*=lb_], .rowDin [id*=col_]").hide().each(async function(k,v){
		oTd = $(v);
		...
		if(sColuna!="" && sColuna!=undefined  && vIdSubtipoAmostra!=""){
			...
			if ($obrigatorio == 'Y' && sColuna != 'nroamostra') {
				$(oInput).attr("vnulo",'')
			}
			...
			if ($show) {
				...
				if(sColuna == "idespeciefinalidade"){
					idespeciefinalidade = $("[name=_1_"+CB.acao+"_amostra_idespeciefinalidade]").attr('cbvalue');
					buscaRegrasDoMapa(idespeciefinalidade);
				}
...
if($escondejquery == false) {
	...
	function duplicarAmostra(data,vmodulo){
		...
		if(CB.acao=="u" && CB.oModuloForm.is(":visible")){
			...
			$data.find(":input").each(function(k,obj){
				...
				}else if(($oNome) && $oNome.indexOf("_u_identificador_")>0){debugger
					//Quebra o nome capturando 2 grupos: numero da linha e nome do campo, excluindo-se o campo idresultado
	...
	$(":input[name=_1_"+CB.acao+"_amostra_idespeciefinalidade]").autocomplete({
		...
		,create: function(){
			...
			,select: function(event,ui){
				...
				buscaRegrasDoMapa(ui.item.value);
			}
		,delay: 0
	});
	...
	*/
	function listaInputsAmostra(){
		...
		$.each($(".rowDin"), function(i, row){
			...
			$.each(inputs, function(i, input){
				...
				if($campo == 'isolamento'){
					strclass = $('#isol').is(":visible") ? "btn btn-success btn-xs pointer" : "btn btn-default btn-xs pointer";
					stricon = $('#isol').is(":visible") ? "fa fa-eye" : "fa fa-eye-slash";
					strVisivel = $('#isol').is(":visible") ? "Y" : "N";
				} else {
		...
		if($inbt.attr("cbvisivel")=="Y"){
			...
			if ($coluna != 'idunucleo') {
				if($tabela=='dadosamostra'){
					...
					if(idamostra != ""){
						...
						if($coluna == 'idpessoaresponsavel' || $coluna == 'idpessoaresponsavelof' || $coluna == 'idpessoaresponsavelcrmv' || $coluna == 'idpessoaresponsavelcrmvof'){
							switch($coluna){
								case 'idpessoaresponsavel':
									apagacampo+='&_v_u_amostra_responsavel=';
									break;
								case 'idpessoaresponsavelof':
									apagacampo+='&_v_u_amostra_responsavelof=';
									break;
								case 'idpessoaresponsavelcrmv':
									apagacampo+='&_v_u_amostra_responsavelcolcrmv=';
									break;
								case 'idpessoaresponsavelcrmvof':
									apagacampo+='&_v_u_amostra_responsavelofcrmv=';
									break;
							}
			...
			CB.post({
				...
				,posPost: function(data, textStatus, jqXHR){
					...
					$(`[inputname=${nm}]`).each((i,e)=>{
						$(e).attr("idamostracampos",CB.lastInsertId)
						$(e).attr("idtelaamostraconf",CB.lastInsertId)
					})
					$collb.show();
	...
	function toggleInputsVisiveisTra(inBotao){
		$inbt = $(inBotao);
		$input = $inbt.attr("inputname")
		$coluna = $input.explodeInputNameCarbon()[4];
	...
	function Novaidentificacao(inidamostra, vthis){
		qtdsum = $(`#qtdsum`).val();
	
		identificador = $(`#descrconcat`).val();

		if((qtdsum != '' && qtdsum > 0) && (identificador != '')){
			inputqtd = $(`#nroamostra`);

			if(inputqtd.val() != ''){
				$(`#nroamostra`).val( parseInt(inputqtd.val()) + parseInt(qtdsum));
			}else{
				inputqtd.val(qtdsum);
			}

			inputdescr = $(`[name=_1_u_amostra_descricao]`).val();
			if(inputdescr != ''){
				$(`[name=_1_u_amostra_descricao]`).val(inputdescr + '; '+qtdsum+ ' ' + identificador.toUpperCase());
			}else{
				$(`[name=_1_u_amostra_descricao]`).val(qtdsum+ ' ' + identificador.toUpperCase());
			}

			separator = new Date().getTime();
			for (let index = 0; index < qtdsum; index++) {
				$(`#ident_add`).append(`<input ident="${separator}" name="${"_x"+separator+""+index+"_i_identificador_idobjeto"}" value="${inidamostra}">`);
				$(`#ident_add`).append(`<input ident="${separator}" name="${"_x"+separator+""+index+"_i_identificador_tipoobjeto"}" value="${"amostra"}">`);
				$(`#ident_add`).append(`<input ident="${separator}" name="${"_x"+separator+""+index+"_i_identificador_identificacao"}" value="${identificador.toUpperCase()}">`);
			}

			$(`#qtdsum`).val('');
			$(`#descrconcat`).val('');
			$(`#ident_temp`).append(`<div id="${separator}" class="input-group input-group-sm" style="padding: 5px 0px 5px 0px;"></div>`);

			$(`#${separator}`)
			.append(`<input class="size5 form-control" id="qtdsum${separator}" readonly value="${qtdsum}">`)
			.append(`<input class="size40 form-control" id="descr${separator}" readonly value="${identificador.toUpperCase()}">`)
			.append(`<span>
						<i class="fa fa-trash pointer" style="padding-left: 10px; padding-top: 6px;" onclick="removePorSeparador('${separator}');"></i>
					</span>`);
		}

		// CB.post({
		// 	objetos: cbpoststring
		// 	,refresh:"refresh"
		// });
	}

	function NovaidentificacaoSemQtd(inidamostra){
		CB.post({
	...
	function removePorSeparador(separador){

		orgao = ($(`[ident="${separador}"][name$="identificacao"]`).val() == null) ? $(`#descr${separador}`).val() : $(`[ident="${separador}"][name$="identificacao"]`).val();
		
		qtd = $(`#qtdsum${separador}`).val();

		valDescr = $(`[name=_1_u_amostra_descricao]`).val();

		if (valDescr.includes(orgao)) {
			// Dividir a string para encontrar a quantidade de 'valIdent'
			var partes = valDescr.split(';');
			var novaDescricao = '';
			identificacaoApagar = qtd + ' ' + orgao
			partes.forEach(function(part) {
				// Verificar se a parte contém 'valIdent'
				if (!part.includes(identificacaoApagar)) {
					novaDescricao += part.trim() + '; ';
				}
			});
		}

		// Remover a vírgula extra no final
		novaDescricao = novaDescricao.slice(0, -2);

		// Atualizar o valor do input
		$('#descricao').val(novaDescricao);

		$(`[ident="${separador}"]`).remove();
		$(`#${separador}`).remove();
		$(`#nroamostra`).val(parseInt($(`#nroamostra`).val()) - parseInt(qtd));

		if($(`[ident="${separador}"][name$="identificacao"]`).val() == null){
			$('#ident_temp')
				.append(`<input class="size5 form-control" type="hidden" name="removerIdentificacao${separador}" readonly value="${orgao}">`)
				.append(`<input class="size5 form-control" type="hidden" name="qtdRemoverIdentificaco${separador}" readonly value="${parseInt(qtd)}">`);
		}
	}

	function excluiridentificacao(vthis,ididentificador,ini){
	
		contador = $(vthis).attr('contador')
		valIdent = $(`.identificador_${contador}`).val();

		valDescr = $(`[name=_1_u_amostra_descricao]`).val();

		if (valDescr.includes(valIdent)) {
			// Dividir a string para encontrar a quantidade de 'valIdent'
			var partes = valDescr.split(';');
			var novaDescricao = '';
			var sair = false;
			partes.forEach(function(part) {
           		// Verificar se a parte contém 'valIdent'
				if (part.includes(valIdent)) {
					// Dividir a parte para obter a quantidade
					var detalhes = part.trim().split(' ');
					var quantidade = parseInt(detalhes[0]);

					// Diminuir a quantidade ou remover 'valIdent' se houver apenas um
					if (quantidade > 1 && sair == false) {
						quantidade -= 1;
						novaDescricao += `${quantidade} ${valIdent}; `;
						sair = true;
					}
				} else {
					novaDescricao += part.trim() + '; ';
	...
	function ExcluiPorPosicao(posicao,input){

		inputVal = $(`${input}`).val()

		if(inputVal != ''){
			inputVal2 = inputVal.split(';');
			inputVal2.splice(posicao,1);
			inputVal2 = inputVal2.join(';');
			$(input).val(inputVal2)
		}
		CB.post();
	...
	function InserirPosicao(inputValor,inputTarget){

		inputVal = $(`${inputValor}`).val();

		targetVal = $(`${inputTarget}`).val();

		if(targetVal == '' || targetVal == null){
			$(`${inputTarget}`).val(inputVal);
		}else if(inputVal != ''){
			$(`${inputTarget}`).val(targetVal + '; ' + inputVal);
		}

		if(inputVal != ''){
			count = $(`#divisolamento .panel-heading`).length;
			$(`#divisolamento`).append(`
				<div class="col-md-4 panel-heading d-flex space-between" >
					<span >${inputVal} <i class="fa fa-trash fa-1x cinzaclaro hoververmelho btn-lg pointer ui-droppable" onclick="ExcluiPorPosicao(${count},'#isolamento')" alt="Excluir identificação!"></i></span>
				</div>
			`)
		}
...
{
	document.getElementById('qtdsum').addEventListener('keydown', function(event)
	{
...
CB.on('posLoadUrl', function(){
	if($("[name='_1_"+CB.acao+"_amostra_status']").val() == "CONFERIDO"){
		$(`input:visible:not([id="input-search"])`).attr('readonly',true);
	}
});
<?
