<script>
	...
	var arrayModulo = '<?=json_encode(getModsUsr("MODULOS"))?>';
    ...
    var listarLote = '<?=(empty($lote2)) ? 0 : json_encode(NfEntradaController::buscarUnidadeObjetoLoteModuloPorIdnf('', $lote2))?>';
    ...
    var listarLoteNfItem = <?=json_encode(NfEntradaController::buscarLoteNfItem($_itens['idnfitem'])) ?> || 0;
    var loteconsarr = <?=json_encode(NfEntradaController::buscarConsumoLoteconsPorIdLoteEIdLoteFracao($_itens["idnfitem"], 'nfitem', $nfItem["idlotefracao"], $nfItem["idlote"])) ?> || 0;
    ...
    function inicializarAutoCompleteProdServ() {
        $(".insidprodserv").autocomplete({
            ...
            select: function(event, ui){
                /*if(CB.modulo == "comprasrh"){
                    CB.post(
                        {
                            objetos: $("#insidprodserv").attr("name")+"="+ui.item.value+"&"+$("#insprodservqtd").attr("name")+"="+$("#insprodservqtd").val()+"&_1_u_nf_idnf="+idnf
                            ,parcial: true
                            ,refresh: false
                        }
                    );		
    ...
    inicializarAutoCompleteProdServ();
    ...
    $(".datareceb").on('apply.daterangepicker', function(ev, picker) {
    ...
    function inserirImposto(idnfitem, indice){
        htmlTrModelo = $(`#imposto_${idnfitem}`).html();    
        htmlTrModelo = htmlTrModelo.replace("#imp_idnfitem", `_${indice}_u_nfitem_idnfitem`);
        htmlTrModelo = htmlTrModelo.replace("#imp_basecalc", `_${indice}_u_nfitem_basecalc`);
        htmlTrModelo = htmlTrModelo.replace("#imp_aliqicms", `_${indice}_u_nfitem_aliqicms`);
        htmlTrModelo = htmlTrModelo.replace("#imp_valicms", `_${indice}_u_nfitem_valicms`);
        htmlTrModelo = htmlTrModelo.replace("#imp_vst", `_${indice}_u_nfitem_vst`);
        htmlTrModelo = htmlTrModelo.replace("#imp_aliqipi", `_${indice}_u_nfitem_aliqipi`);
    ...
    function inserirImpostoImportacao(idnfitem, indice){
        ...
        htmlTrModelo = htmlTrModelo.replace("#imp_idnfitem", `_${indice}_u_nfitem_idnfitem`);
        htmlTrModelo = htmlTrModelo.replace("#imp_impostoimportacao", `_${indice}_u_nfitem_impostoimportacao`);
        htmlTrModelo = htmlTrModelo.replace("#imp_valipi", `_${indice}_u_nfitem_valipi`);
        htmlTrModelo = htmlTrModelo.replace("#imp_pis", `_${indice}_u_nfitem_pis`);
        htmlTrModelo = htmlTrModelo.replace("#imp_cofins", `_${indice}_u_nfitem_cofins`);
        htmlTrModelo = htmlTrModelo.replace("#imp_valicms", `_${indice}_u_nfitem_valicms`);
    ...
    {
        CB.post({
            objetos: "_x_u_nfconfpagar_idnfconfpagar=" + inI + "&_x_u_nfconfpagar_datareceb=" + indate,
            parcial: true,
            refresh: false,
            msgSalvo: false,
    ...
    {
        let pis = $("input[name='_1_u_nf_pis']").val().replace(",", ".")
    ...
    function atribuivalorselect(vthis, idnfitem, tipo) 
    {
        ...
        if (tipo == 'contaitem') {
            ...
            CB.post({
                objetos: "_x_u_nfitem_idnfitem=" + idnfitem + "&_x_u_nfitem_idcontaitem=" + $("#iidcontaitem" + idnfitem).val() + "&_x_u_nfitem_idtipoprodserv=" + $("#iidtipoprodserv" + idnfitem).val(),
                parcial: true,
                refresh: false
            });
        }
        ...
        if((qtdProdutosCadastrados > 0 && status == 'CONCLUIDO' && tipoLista == 'cadastrado') || (qtdProdutosSemCadastro > 0 && status == 'CONCLUIDO' && tipoLista == 'naocadastrado') || (status != 'CONCLUIDO')) {
            ...
            const columnDefs = [
                { 
                    ...
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            let checked = (params.data.cobrar == 'Y') ? 'checked' : 'checked';
                        ...
                        if (!params.data.qtd?.newrow) {
                            ...
                            if(tipoLista == 'cadastrado'){
                                if(params.data.tipoobjetoitem == 'nfitem' && params.data.idobjetoitem != null){
                                    if($nfOrigem != null){
                                        compraOrigem = `<a title="Compra Origem - ${$nfOrigem}" class="fa fa-bars fade pointer hoverazul" href="?_modulo=nfentrada&_acao=u&idnf=${$nfOrigem}" target="_blank"></a>;`;
                                    }                                    
                                }
                ...
                { 
                    ...
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            ...
                            if(params.data.qtd < params.data.qtdsol && params.data.qtdsol != null){
                                classColorirVermelho = 'colorirVermelho';
                                duplicarNf = `<div class="col-md-2 alinhamento-topo">`;
                                if(listarObjetoOrigem.qtdLinhas < 1){
                                    duplicarNf += `<a class="fa fa-plus-circle verde hoververde  pointer " title="Duplicar Compra" onclick="duplicarcompra(${params.data.idnfitem})"></a>
                                                </div>`;
                                }
                            ...
                            htlmQtd = `<div class="col-md-12 padding-zero h-100 d-flex justify-between align-items-center" style="gap: .5rem;">
                                            <div class="col-md-9 padding-zero h-100">
                                                <input ${readonly} name="_${params.data.idnfitem+10}_u_nfitem_idnfitem" size="8" type="hidden" class="idnfitem" value="${params.data.idnfitem}">
                                                <input name="_${params.data.idnfitem+10}_qtd_old" type="hidden" value="${params.data.qtd}">
                ...
                { 
                    ...
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            ...
                            if(params.data.idcontaitem == null || params.data.idtipoprodserv == 0) {
                ...
                { 
                    ...
                    hide: shouldHideTipoNfRtEdSoColumn(ListagemTodosItens),                    
                    width: 50,
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            if(internacional == true) {
                                return `<a idnfitem="${params.data.idnfitem}" indice="${params.data.idnfitem+10}" onclick="inserirImpostoImportacao(${params.data.idnfitem}, ${params.data.idnfitem+10})">                                                                    
                                    <img src="../inc/img/lionicon.png" style="width: 15px; height: 15px;">
                                </a>`;
                            } else {
                                return `<a idnfitem="${params.data.idnfitem}" indice="${params.data.idnfitem+10}" onclick="inserirImposto(${params.data.idnfitem}, ${params.data.idnfitem+10})">                                                                    
                                    <img src="../inc/img/lionicon.png" style="width: 15px; height: 15px;">
                                </a>`;
                            }
                    ...
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            ...
                            if(tiponf != 'R'){
                                ...
                                let loteNf = JSON.parse(listarUnidadeObjetoItem);
                                loteNfItem = loteNf[params.data.idnfitem];
                            ...
                            { 
                                if (params.data.idprodserv != null) 
                                {
                                    ...
                                    if (params.data.idlote2 != null) 
                                    {
                                        ...
                                        linkLote = `<a class="fa fa-bars cinza fa-x  btn-lg pointer" onclick="janelamodal('?_modulo=${$lote.idobjeto}&_acao=u&idlote=${params.data.idlote2}');" title="${params.data.partida2}/${params.data.exercicio2}"></a>`;
                                    } else if(params.data.qtd > 0) {
                                        unidade = (params.data.unidade == null) ? '' : params.data.unidade;
                                        ...
                                        var linkLote = `<a class="fa fa-x ${$figuranovo} btn-lg pointer" onclick="gerarlotem(${params.data.idnfitem}, '${$descricaolt}', '${params.data.qtd}', '${unidade}', '${$tqtdprod}', '${larguraTabela}')" title="Novo Lote"></a>`;
                                    } else {
                                        var linkLote = "-";
                                    }
                                } else {
                            ...
                            var divImposto = `<div id="imposto_${params.data.idnfitem}" style="display: none" class="panel-body">
                                            <table class="table table-hover planilha grade">                            
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <input name="#imp_idnfitem" value="${params.data.idnfitem}" type="hidden">
                                                    </td>                                                                
                                                </tr>                         
                                                <tr>
                                                    <td><b>NCM:</b></td>
                                                    <td><div class="div_ncm">${params.data.ncm}</div></td>
                                                </tr>     
                                                <tr>
                                                    <td colspan="2">
                                                        <table class="table planilha" style="border: 2px solid #ddd;">
                                                            ...
                                                            <tbody>
                                                                <tr>
                                                                    <td class="alinhar-centro">BC</td>
                                                                    <td class="alinhar-centro"><input name="#imp_basecalc" class="size10 alinhar-direita" value="${basecalc}" type="text"></td>
                                                                </tr>
                                                                <tr class="bc-cinza">
                                                                    <td class="alinhar-centro">ICMS %</td>
                                                                    <td class="alinhar-centro"><input name="#imp_aliqicms" class="size10 alinhar-direita imposto" value="${params.data.aliqicms}" type="text"></td>
                                                                </tr>
                                                                <tr>
                                                                    <td class="alinhar-centro">ICMS R$</td>
                                                                    <td class="alinhar-centro"><input name="#imp_valicms" class="size10 alinhar-direita imposto" value="${params.data.valicms}" type="text"></td>
                                                                </tr>
                                                                <tr class="bc-cinza">
                                                                    <td class="alinhar-centro">ICMS ST</td>
                                                                    <td class="alinhar-centro"><input name="#imp_vst" class="size10 alinhar-direita imposto" value="${vst}" type="text"></td>
                                                                </tr>
                                                                <tr class="bc-cinza">
                                                                    <td class="alinhar-centro">IPI %</td>
                                                                    <td class="alinhar-centro"><input name="#imp_aliqipi" class="size10 alinhar-direita imposto" value="${params.data.aliqipi}" type="text"></td>
                                                                </tr>
                                                            </tbody>                                             
                                                        </table>
                                                    </td>
                                        ...
                                        <div id="imposto_importacao_${params.data.idnfitem}" style="display: none" class="panel-body">
                                            <table class="table table-hover planilha grade">                            
                                                <tr>
                                                    <td><b>Imposto do Item:</b></td>
                                                    <td>
                                                        ...
                                                        <table class="table planilha" style="border: 2px solid #ddd;">
                                                            ...
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td class="alinhar-centro"><input name="#imp_impostoimportacao" class="size10 alinhar-direita" value="${impostoimportacao}" type="text"></td>
                                                                </tr>
                                                                <tr class="bc-cinza">
                                                                    <td class="alinhar-centro">IPI</td>
                                                                    <td class="alinhar-centro"><input name="#imp_valipi" class="size10 alinhar-direita" value="${valipi}" type="text"></td>
                                                                </tr>
                                                                <tr>
                                                                    <td class="alinhar-centro">PIS</td>
                                                                    <td class="alinhar-centro"><input name="#imp_pis" class="size10 alinhar-direita" value="${pis}" type="text"></td>
                                                                </tr>
                                                                <tr class="bc-cinza">
                                                                    <td class="alinhar-centro">COFINS</td>
                                                                    <td class="alinhar-centro"><input name="#imp_cofins" class="size10 alinhar-direita" value="${cofins}" type="text"></td>
                                                                </tr>
                                                            </tbody>                                        
                                                        </table>
                                                    </td>
                    ...
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            ...
                            {    
                                ...
                                if (listaTagEmpresa.qtdLinhas == 0 && tagNf.idtagclass != 3) 
                                {
                                                    ...
                                                    <div class="row m-0 d-flex align-items-right date-options">                            
                                                        <div class="w-100"> 
                                                            ...
                                                            <div class="col-xs-12 d-flex px-0">
                                                                <select name="#tag_idtagclass" onchange="mostrarTagsPorTagClass('${params.data.idnfitem}', this)">
                                                                    ${htmlTagClasse}
                                                                </select>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="col-xs-12">
                                                    <div class="row m-0 d-flex align-items-right date-options">                            
                                                        <div class="w-100"> 
                                                            ...
                                                            <div class="col-xs-12 d-flex px-0">
                                                                <input name="#tag_idnfitem" type="hidden" value="${params.data.idnfitem}">
                                                                <input id="idtag_${params.data.idnfitem}_old" type="hidden" value="${idobjeto}">
                                                                <input name="#tag_idnfitemacao" id="#tag_idnfitemacao_${params.data.idnfitem}" type="hidden" value="${idnfitemacao}">
                                                                <select name="#tag_idtag" id="#idtag_idtag${params.data.idnfitem}" vnulo ${disabledStatus}>
                                                                    <option value=""></option>
                                                                    ${htmlTagsDisponiveis}
                                                                </select>
                                                            </div>
                                                ...
                                                <div class="col-xs-12 #div_categoria">
                                                    <div class="row m-0 d-flex align-items-right date-options">                            
                                                        <div class="w-100"> 
                                                            ...
                                                            <div class="col-xs-12 d-flex px-0">
                                                                <select name="#tag_categoria" id="categoria${params.data.idnfitem}" style="width: 100%" vnulo ${disabledStatus}>
                                                                    <option value=""></option>
                                                                    ${htmlManutencao}
                                                                </select>
                                                            </div>
                                                ...
                                                <div class="col-xs-12 #div_km">
                                                    <div class="row m-0 d-flex align-items-right date-options" style="padding-bottom: 25px;">                            
                                                        <div class="w-100"> 
                                                            ...
                                                            <div class="col-xs-12 d-flex px-0">
                                                                <input name="#tag_km" value="${kmrodados}" type="text">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    cellRenderer: params => {
                        if (!params.data.qtd?.newrow) {
                            ...
                            if(params.data.tipoobjetoitem == 'nf' && params.data.idobjetoitem != null){
                                switch (params.data.tiponfobji) {
                                    case 'V':
                                        modulonf = "pedido";
                                    break;
                                    case jQuery.inArray(params.data.tiponfobjim, ['C', 'S', 'T', 'E', 'M', 'B', 'F']):
                                        modulonf = "nfentrada";
                                    break;
                                    case 'R':
                                        modulonf = "comprasrh";
            ...
            function addRowCadastrado() {
                if (gridApiCadastrado) {
                    ...
                    const newRow = {
                        qtd: {
                            input: "<input style=' border: 1px solid silver;' name='_"+contadorCadastrado+"#quantidade' campo='_"+contadorCadastrado+"-quantidade' id='insprodservqtd' title='Qtd' placeholder='Qtd' type='text' class='size5 guardar-"+contadorCadastrado+" alterarNovosCampos a_"+contadorCadastrado+"-quantidade' onblur='guardarDadosNovos("+contadorCadastrado+", this)'>",
                            newrow: true,
                            valorInput: contadorCadastrado
                        },
                        prodservdescr: {
                            input: "<input type='text' style='width: 35em;' id='insidprodserv' class='insidprodserv guardar-"+contadorCadastrado+" alterarNovosCampos a_"+contadorCadastrado+"-idprodserv' name='_"+contadorCadastrado+"#idprodserv' campo='_"+contadorCadastrado+"-idprodserv' cbvalue='' placeholder='Selecione o produto' onblur='guardarDadosNovos("+contadorCadastrado+", this)'>",
                            newrow: true,
                    ...
                    gridApiCadastrado.refreshCells({ force: true });
                } else {
            ...
            function calculateSum(tipoLista) {
                ...
                if(tipoLista == 'cadastrado') {
                    gridApiCadastrado.forEachNode(node => {
                        subtotalCadastrado += formatarStringDecimal(node.data.vlritem * node.data.qtd);
                        desCadastrado += formatarStringDecimal(node.data.des * node.data.qtd);
                        valipiTotalCadastrado += formatarStringDecimal(node.data.valipi);
                        cofinsTotalCadastrado += formatarStringDecimal(node.data.cofins);
                        valicmsTotalCadastrado += formatarStringDecimal(node.data.valicms);
                ...
                if(tipoLista == 'naocadastrado') {
                    gridApiNaoCadastrado.forEachNode(node => {
                        subtotalNaoCadastrado += formatarStringDecimal(node.data.vlritem * node.data.qtd);
                        desNaoCadastrado += formatarStringDecimal(node.data.des * node.data.qtd);
                        ...
                        valicmsTotalNaoCadastrado += formatarStringDecimal(node.data.valicms);
                ...
                setarValorImpostos(desCadastrado, desNaoCadastrado, 'descCadastrado', 'descNaoCadastrado');
                ...
                setarValorImpostos(valicmsTotalCadastrado, valicmsTotalNaoCadastrado, 'nf_valicmsTotalCadastrado', 'nf_valicmsTotalNaoCadastrado');
                ...
                $('#totalSubTotal').html(SubTotalTotalGeral.toFixed(2));
                $('#totalTotal').html(totalTotalGeral.toFixed(2));
                $('.valorTotalImpostoServico').html(totalTotalGeral.toFixed(2));   
                ...
                if((internacional != null && moedainternacional == 'Y') && (totalTotalGeral.toFixed(2) > totalcpFloat.toFixed(2) || totalTotalGeral.toFixed(2) < totalcpFloat.toFixed(2))){
                    alerta = `<i title="Valor ${totalcpFloat.toFixed(2)} diferente com o da nota ${totalTotalGeral.toFixed(2)}" class="fa fa-exclamation-triangle laranja btn-lg pointer"></i>`;
    ...
    function guardarDadosNovos(indice, event){
        ...
        $(`.guardar-${indice}`).each(function(index, element) {
            if($(`.${$(element).attr('campo')}`).length == 0){
                ...
                novosInputs += `<input id="${$(element).attr('id')}" class="${$(element).attr('campo')}" ${attrCbvalue} ${attrTitulo} type="hidden" value="${$(element).val()}">`;
            } else {
    ...
    $('#pis_iss').attr('onchange', "atualizaTotal('"+totalTotalGeral+"')");    
    $('#pis_issret').attr('onchange', "atualizaTotal('"+totalTotalGeral+"')");
    $('.nf_desconto').attr('onchange', "atualizaTotal('"+totalDes+"')");
            ...
            $o.on('click', function(e){
                ...
                $('.li_nfentrada').each(function(index, element) {
                    ...
                    if(valueListaCotacao == khref && CB.jsonModulo.jsonpreferencias[refNentrada] != null)
                    {
                        ...
                        CB.setPrefUsuario('m',`{"${CB.modulo}":{"nfentrada_${_idpk}":{"${navType}":{"${valueListaCotacao}":"Y"}}}}`,'');
                    }                    
                });
            });
    ...
    $(".mostrarAnexos").click(function(index, elemento)
    {   
        $(this).webuiPopover({
            trigger: "click",
