...
<script>
	...
	$clausulacq = ProdServController::BuscarSubcategoriaPorProdserv($_1_u_formalizacao_idformalizacao, $_1_u_formalizacao_idunidade, $_1_u_formalizacao_idempresa );
	$array = array_column($clausulacq, "idloteativ");
	...
	jAtividadeInsumos = <? if (empty($jAtividadeInsumos)) {
							echo "null";
						} else {
							echo $jAtividadeInsumos;
						} ?> || "";
	...
	const prodsersAtivos = <?= json_encode($prodsersAtivos) ?>;
	const prodsersInsumosAtivos = <?= json_encode($prodsersInsumosAtivos) ?>;
	const idFluxoStatusOP = '<?= $_1_u_formalizacao_idfluxostatus ?>';
	const idFluxoStatusNaoConforme = '<?= FluxoController::getIdFluxoStatus('prativ', 'INUTILIZADO') ?>';
	const idFluxoStatusCancelado = '<?= FluxoController::getIdFluxoStatus('prativ', 'CANCELADO') ?>';
	const idFluxoStatusFormalizacaoCancelado = '<?= FluxoController::getIdFluxoStatus($_GET['_modulo'], 'CANCELADO', $_1_u_formalizacao_idprproc) ?>';
	var idloteativcq = '<?=$jsonArray?>';
	//------- Injeção PHP no Jquery -------
	...
	if (idFormalizacao) {
		let delay = setInterval(() => {
			if (typeof QRCode != 'undefined') {
				let qrCodeElement = document.getElementById("qrcode-formalizacao");

				if (!qrCodeElement) clearInterval(delay);
				else {
					new QRCode(qrCodeElement, {
						text: `?_modulo=formalizacao&_acao=u&idformalizacao=${idFormalizacao}&_idempresa=${idempresa}`,
						width: 300,
						height: 300,
					});
	...
	CB.on('prePost', function() {
		$(`[hora-ativ][vnulo]`).each(function(i, o) {
			$o = $(o);

			if ($o.val() == "00:00" || $o.val() == "000:00" || $o.val() == "0:00" || $o.val() == "0" || $o.val() == "00") {
				$o.val("");
			}
		});
		...
		$.each($("#corpoFormalizacao").find(":input[nome*=_loteativ_status]"), function(i, o) {
			$o = $(o);
			//console.log($o.val())
			if ($o.val() == "CONCLUIDO") {
				StatusConcluido[i] = o;
	...
	CB.on('prePost', function() {
		debugger
		if($('.diluicaoerrada').length) {
			$('.diluicaoerrada').css('backgroundColor', "#efeb61");
			alertAtencao('Valor inválido. <br /> Diluição inválida');
			return false;
		}

		if($('.vlrincorreto').length) {
			$('.vlrincorreto').css('backgroundColor', "#efeb61");
			alertAtencao('Valor inválido.');
			return false;
		}
	});
	...
	function verificadiluicao(vthis, acao, iConsumo, vIdLoteCons) {
		...
		alterarInputInsertUpdate(vthis, acao, iConsumo, vIdLoteCons);
	}

	function alterarInputInsertUpdate(vthis, acao, iConsumo, vIdLoteCons) {
		if (vthis.value.length > 0) {
			$(`.alterarconsumo${iConsumo}`).each(function(i, o) {
				// A string original
				var StringOriginal = $(o).attr('nome');
				var idInput = $(o).attr('id');

				var count = (acao == 'i') ? (StringOriginal.match(/_i_/g) || []).length : (StringOriginal.match(/_u_/g) || []).length;

				if (idInput == undefined && count == 0) {
					// Transformar a string
					var novaString = StringOriginal.replace("_lotecons_", `_${acao}_lotecons_`);
					$(o).attr('name', novaString);
				}
			});
		} else if (vIdLoteCons != null && vIdLoteCons != '') {
			var StringOriginal = `_cons${iConsumo}_lotecons_idlotecons`;
			var novaString = StringOriginal.replace("_lotecons_", `_d_lotecons_`);
			$(`.${StringOriginal}`).attr('name', novaString);

			if($(`[name=_cons${iConsumo}_u_lotecons_idlote]`).length > 0){
				$(`.alterarconsumo${iConsumo}`).each(function(i, o) {
					// A string original
					var StringOriginal = $(o).attr('nome');
					var novaString = StringOriginal.replace("_lotecons_", `_${acao}_lotecons_`);
					if ($(`[name=${novaString}`).length > 0) {
						// Transformar a string
						var novaString = StringOriginal.replace("_lotecons_", `_${acao}_lotecons_`);
						$(o).removeAttr('name');
					}
	...
	function mostraConsumo(inOConsumo, acao, iConsumo, vIdLoteCons) {
		...
		//$sQtddisp= $trInsumo.find(".sQtddisp");
		//$sQtddisp_exp= $trInsumo.find(".sQtddisp_exp");
		$sParticipacao = $trInsumo.find(".sParticipacao");
		somaUtilizacao = 0;
		$oConsumos = $trInsumo.find("[nome*=_qtdd]");

		alterarInputInsertUpdate(inOConsumo, acao, iConsumo, vIdLoteCons);
		...
		$.each($oConsumos, function(isc, osc) {
			...
			if ($o.val()) {
				...
				if ($o.attr("cbqtddispexp") != "" && ($o.val().toLowerCase().indexOf("e") <= 0 && $o.val().toLowerCase().indexOf("d") <= 0)) {
					...
					inOConsumo.style.backgroundColor = "#efeb61";
					inOConsumo.classList.add('diluicaoerrada');
					//alertAtencao("Valor inválido. <br> Inserir e ou d.");
					return false;
				} else if ($o.attr("cbqtddispexp") != "" && ($o.val().toLowerCase().indexOf("e") >= 0 || $o.val().toLowerCase().indexOf("d") >= 0)) {
					...
					if (matches) {
						...
						if (valor1 != valor2) {
							inOConsumo.style.backgroundColor = "#e1b9a9";
							inOConsumo.classList.add('diluicaoerrada');
					...
					$o.addClass('vlrincorreto');
					return false;
				}

				$o.removeClass('vlrincorreto');
		...
		valorConsumo = $oc.val().replace(/,/g, '.');
		valorConsumo = normalizaQtd(valorConsumo);

		sQtddisp = normalizaQtd($oc.attr("sQtddisp"));
	...
	async function interromperOP(idLoteAtiv) {
		if(!$('#motivo-nao-conforme').val()) return alertAtencao('Selecione o motivo.');
		CB.oModal.find('button').attr('disabled', true);
		// Buscar todas as atividades anteriores e manter seus consumos
		try {
			// Buscar e cancelar atividades posteriores junto aos testes
			const etapaNaoConformeJQ = $(`div[idloteativ="${idLoteAtiv}"]`),
				idProxmasEtapas = etapaNaoConformeJQ
									.nextAll()
									.map((indice, element) => $(element).attr('idloteativ'))
									.filter(id => ![undefined, ''].includes(id))
									.toArray();

			alteraStatusECancelarAtiv(idLoteAtiv, 'Não Conforme', 'N', idProxmasEtapas);
		} catch(e) {
			CB.oModal.find('button').removeAttr('disabled');
			alertAtencao('Ocorreu um erro realizar a não conformidade da OP');
			console.log(e);
		}
	}

	function abrirModalNaoConformidade(idLoteAtiv) {
		// Buscar e cancelar atividades posteriores junto aos testes
		const etapaNaoConformeJQ = $(`div[idloteativ="${idLoteAtiv}"]`),
				etapas = etapaNaoConformeJQ
									.parent()
									.children()
									.map((indice, element) => {
										return {
											id: $(element).attr('idloteativ'),
											indice: indice + 1
										}
									})
									.filter(id => ![undefined, ''].includes(id))
									.toArray(),
				indexEtapaAtual = etapas.findIndex(item => item.id == idLoteAtiv),
				etapasAnteriores = etapas.slice(0, indexEtapaAtual).map((value) => value.indice).join(),
				proximasEtapas = etapas.slice(indexEtapaAtual + 1).map((value) => value.indice).join();

				let labelConsumidas = '',
					labelCanceladas = '';

				if(etapasAnteriores)
					labelConsumidas = `<h5 for="">As atividades <strong class="atividades-consumidas">${etapasAnteriores}</strong> terão consumo normalmente</h5>`;

				if(proximasEtapas)
					labelCanceladas = `<h5 for="">As atividades <strong class="atividades-canceladas">${proximasEtapas}</strong> serão canceladas</h5>`;
					

		const acoesRealizadasHTML = `<div class="col-xs-12">
										<div class="alert alert-warning">
											<h3>Ações que serão realizadas</h3>
											${labelConsumidas}
											${labelCanceladas}
										</div>
									</div>`;
		...
		const corpo = `<div class="row">
							<div class="col-xs-12 form-group">
								<label for="">Motivo da não conformidade</label>
								<select name="" id="motivo-nao-conforme" class="form-control" style="border: 1px solid #ccc !important;">
									<option value="">Selecionar motivo</option>
									<option value="Contaminação">Contaminação</option>
									<option value="Falha técnica (Equipamento)">Falha técnica (Equipamento)</option>
									<option value="Falha de procedimento (Acidentes)">Falha de procedimento (Acidentes)</option>
								</select>
							</div>
							${acoesRealizadasHTML}
						</div>`,
			titulo = `Motivo da não conformidade`,
			rodape = `<div class="w-100 d-flex">
							<button class="btn btn-success mr-2" onclick="interromperOP(${idLoteAtiv});">Salvar</button>
	...
	function abrirModalCancelamento(idLoteAtiv) {
		const acoesRealizadasHTML = `<div class="col-xs-12">
										<div class="alert alert-warning">
											<h3>Ações que serão realizadas</h3>
											<h5 for="">Os testes vínculados a atividades serão cancelados.</h5>
										</div>
									</div>`;
		...
		const corpo = `<div class="row">
							<div class="col-xs-12 form-group">
								...
								<select name="" id="motivo-cancelamento" class="form-control" style="border: 1px solid #ccc !important;">
									<option value="">Selecionar motivo</option>
									<option value="Aberta incorretamente">Aberta incorretamente</option>
									<option value="Planejamento cancelado">Planejamento cancelado</option>
								</select>
							</div>
							${acoesRealizadasHTML}
						</div>`,
			...
			rodape = `<div class="w-100 d-flex">
							<button class="btn btn-success mr-2" onclick="cancelarOP(${idLoteAtiv});">Salvar</button>
							<button class="btn btn-danger" data-dismiss="modal">Cancelar</button>
						</div>`
	...
	function cancelarOP (idLoteAtiv) {
		if(!$('#motivo-cancelamento').val()) return alertAtencao('Selecione o motivo.');
		CB.oModal.find('button').attr('disabled', true);
		...
		// Definir status da atividade atual como "Não conforme"
		objetos = {
			_statusla_u_loteativ_idloteativ: idLoteAtiv,
			_statusla_u_loteativ_status: `${status}`,
			_statusla_u_loteativ_idfluxostatus: `${idFluxoStatusCancelado}`,
			_statusla_u_loteativ_bloquearstatus: `N`
		}

		if(idProxmasEtapas) {
			idProxmasEtapas.forEach((id, indice) => {
				objetos[`_${indice + 1}_u_loteativ_idloteativ`] = id;
				objetos[`_${indice + 1}_u_loteativ_status`] = status;
				objetos[`_${indice + 1}_u_loteativ_bloquearstatus`] = 'N';
				objetos[`_${indice + 1}_u_loteativ_idfluxostatus`] = idFluxoStatusCancelado;
			});
		}
		
		// Cancelamento da OP
		var idfluxostatushist = idFluxoStatusOP;

		if(idfluxostatushist != ""){
			const motivoCancelamento = $('#motivo-cancelamento').val();
				...
				CB.post({
					...
					posPost: function() {
						$.ajax({
							...
							dataType: 'json',
							data: {
								idloteativ: idProxmasEtapas.join(),
								idfluxostatus: idFluxoStatusFormalizacaoCancelado,
								tipo: 'cancelarAtividades',
								idformalizacao: idFormalizacao,
								_modulo: CB.modulo,
								motivo: motivoCancelamento,
								idlote: $_2_u_lote_idlote,
								idunidade
							},
							success: res => {
								if(res.error) {
								...
								$.ajax({
									...
									,data: {
										"_modulo": CB.modulo,
										"_primary": 'idformalizacao',
										"_idobjeto": idFormalizacao,
										"idfluxostatushist": idfluxostatushist,
										"status": 'CANCELADO',
										"idfluxostatus": idFluxoStatusFormalizacaoCancelado,
										"motivo": motivoCancelamento,
										"motivoobs": motivoCancelamento,
										"acao": "restaurarfluxo"
									},
									success: res  => {
							...
							error: err => {
								console.log(err);
								alertAtencao('Ocorreu um erro ao cancelar os testes.');
							}
						});
					}
				});
	...
	var Formalizacao = function() {
		...
		selfer.getHtmlAtividade = function(inAtiv) {
			...
			if(inAtiv.status == 'PROCESSANDO') {
				if(inAtiv.naoconformidade == 'Y')
					btnNaoConforme = `<button onclick="abrirModalNaoConformidade(${inAtiv.idloteativ})" class="btn btn-danger btn-xs mr-2">Não conforme</button>`;

				if(inAtiv.cancelamento == 'Y')
					btnCancelar = `<button onclick="abrirModalCancelamento(${inAtiv.idloteativ})" class="btn btn-danger btn-xs mr-2">Cancelar</button>`;
			}
			...
			if ((inAtiv.status != "CONCLUIDO" && (inAtiv.statusFormalizacao == 'ATIVO' || inAtiv.statusFormalizacao == 'PENDENTE')) || ((inAtiv.status == 'PENDENTE' || inAtiv.status == 'PROCESSANDO') && inAtiv.idfluxostatus == null)) {
				...
				debugger
				if (idloteativcq.includes(inAtiv.idloteativ) && idempresa == 2) {
					desabilitado = 'disabled';
					informacao = ` onclick='alertAtencao("Para este produto essa atividade será concluída pelo controle de qualidade!")'`;
				}
			...
			if (inAtiv.duracao || inAtiv.tempoestimado) {
				if (inAtiv.tempogastoobrigatorio == 'Y' && escondediv == '' && inAtiv.status == 'CONCLUIDO') {
					inAtiv.tempogastoobrigatorio = 'vnulo';
				} else {
					inAtiv.tempogastoobrigatorio = '';
				}
				inicioFim = `
				<div class="d-flex justify-end align-items-center">
					<label class="screen">Tempo Estimado:</label>
					<input disabled placeholder="00:00" name="_${selfer.iAtividade}_u_loteativ_tempoestimado"  class="form-control" value="${inAtiv.tempoestimado||""}" type="text" title="Tempo estimado" style="width:45px !important;" autocomplete="off"     ${obnulo} >
					&nbsp;&nbsp;&nbsp;
					<label class="screen">Tempo Gasto:</label>
					<input placeholder="00:00" hora-ativ="${inAtiv.idloteativ}" ${inAtiv.tempogastoobrigatorio} oninput="setMaskPattern(this)" name="_${selfer.iAtividade}_u_loteativ_duracao"  class="form-control tempogasto" value="${inAtiv.duracao||""}" type="text" title="Tempo gasto" style="width:45px !important;" ${obnulo} >
				</div>`;
			}
				...
				<div class="${nodrop}">
					<div class='panel-heading ${statusloteativ(inAtiv.status)}' title="idloteativ: ${inAtiv.idloteativ}">
						...
						<table style="width: 100%;">
							<tr>
								<td class="screen col-md-4">[${ctativ}/${Object.keys(jLoteAtiv).length}] - ${inAtiv.nomecurtoativ}</td>
								<td class="form-group form-inline text-right col-md-6">
									<table>
										<tr>
											<td>
		...
		selfer.getSalas = function(inPrAtivObjetos) {
			...
			$.each(inPrAtivObjetos, function(i, o) {
				if (o.idtagclass && (o.idtagclass == "2" || o.idtagclass == "17")) {
		...
		selfer.getHtmlSalas = function(inPrAtiv) {
			...
			hSala = `
				...
				<select ${desabsala} ${statusATiv} idloteobj="${vidloteobj}" name="_${prAtiv.idloteativ}sala_${vacao}_loteobj_idobjeto" class="form-control selectsala ${$_1_u_formalizacao_status}" onchange="alteracaoSala(this);" style="width: 300px !important;">
					<option value=""></option>
					${sOpcoes}
				</select>
					${areserva}
				...
				**************************************************************************************************/
				...
				$.each(vEstoque, function(i, oe) {
					...
					if (vQtd > 0 || oe.status != 'ESGOTADO') {
						...
						Voeqtddisp = oe.qtddisp;
						Voeqtddisp_exp = oe.qtddisp_exp;
						...
						sInsumos += `
								<span class="label ${labelStatus} fonte10 itemestoque ${sHidden} ${cEsconderLoteDaUtilizacao} ${cEsconderimpressao} ${cEspecial} ${cEspecialVisivel}" qtddisp="${oe.qtddisp||""}" qtddispexp="${oe.qtddisp_exp||""}" idlote="${oe.idlote}" data-toggle="tooltip" title="${oe.partida} - ${rotuloStatus.toUpperCase()}" data-original-title="${oe.partida} - ${rotuloStatus.toUpperCase()}">
									<div style="text-align: left;">
										...
										<input type="text" ${statusATiv} nome="_cons${selfer.iConsumo}_lotecons_qtdd" value="${vQtdInput}" idlote="${oe.idlote}" class="reset screen ${classAtvi} alterarconsumo${selfer.iConsumo} _cons${selfer.iConsumo}_lotecons_qtdd" sQtddisp="${Voeqtddisp}" sQtddisp_exp="${Voeqtddisp_exp}" cbqtddispexp="${oe.qtddisp_exp||""}" style="width: 80px !important;" onkeyup="mostraConsumo(this, '` + acaoCons + `', '` + selfer.iConsumo + `', '` + vIdlotecons + `')" onchange="verificadiluicao(this, '` + acaoCons + `', '` + selfer.iConsumo + `', '` + vIdlotecons + `')">
										...
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" id="antigo" nome="_cons${selfer.iConsumo}_lotecons_valor_antigo" value="${vIdlotecons}">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo} _cons${selfer.iConsumo}_lotecons_idlotecons" nome="_cons${selfer.iConsumo}_lotecons_idlotecons" value="${vIdlotecons}">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" nome="_cons${selfer.iConsumo}_lotecons_tipoobjeto" value="lote">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" nome="_cons${selfer.iConsumo}_lotecons_idobjeto" value="${$_2_u_lote_idlote}">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" nome="_cons${selfer.iConsumo}_lotecons_tipoobjetoconsumoespec" value="${vLocalConsumo}">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" nome="_cons${selfer.iConsumo}_lotecons_idobjetoconsumoespec" value="${idLocalConsumo}">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" nome="_cons${selfer.iConsumo}_lotecons_idlote" value="${oe.idlote}">
										<input type="hidden" class="alterarconsumo${selfer.iConsumo}" nome="_cons${selfer.iConsumo}_lotecons_idlotefracao" value="${oe.idlotefracao}">
										${sprintse}
		...
		selfer.getHtmlResultadosBioensaio = function(inResultadosbioterio) {
			...
			if (tamanho(inResultadosbioterio.amostrasRelacionadasbioterio) == 0) {
				strAtalhoRegistrobioensaio = `<div class="checkbox checked" style="text-align-last: center;">
				<a class="fa fa-plus-circle pointer fade hoververde fa-2x " onclick="gerarbioensaio(this)"  title="Vincular Estudos."></a>
				</div>`;
			} else {
				$.each(inResultadosbioterio.amostrasRelacionadasbioterio, function(i, o) {
					...
					strAtalhoRegistrobioensaio += `
						<div class="checkbox checked">
							...
							<i title="Registro Operacional" class="fa fa-print pull-right fa-lg cinza pointer hoverazul" onclick="janelamodal('report/relbioensaio.php?idbioensaio=${o.idbioensaio}')">&nbsp;&nbsp;</i>
						</div>`;
				});
				strAtalhoRegistrobioensaio += `<div class="checkbox checked" style="text-align-last: center;">
				<a class="fa fa-plus-circle pointer fade hoververde fa-2x " onclick="gerarbioensaio(this)" title="Vincular Estudos."></a>
		...
		selfer.getHtmlContrProc = function(inPrAtiv) {
			...
			for (let o of $controlesProc) {
				...
				if (o.inputmanual == "text") {
					...
					if (jLoteObj.ctrlproc != undefined) {
	...
	function toggleCheckbox(inObj, statusAtividade) {
		...
		if (statusAtividade == 'PENDENTE' || statusAtividade == 'PROCESSANDO') {
			if (inidloteobj) {
				...
				var idobjeto = $objCheck.attr('valor');
	...
	function alteraStatusAtiv(idloteativ, status, bloquearstatus) {
		if (status == 'CONCLUIDO') {
			if ($(`[hora-ativ=${idloteativ}][vnulo]`).val() == '00:00' ||
				$(`[hora-ativ=${idloteativ}][vnulo]`).val() == '' ||
				$(`[hora-ativ=${idloteativ}][vnulo]`).val() == '0' ||
				$(`[hora-ativ=${idloteativ}][vnulo]`).val() == '00' ||
				$(`[hora-ativ=${idloteativ}][vnulo]`).val() == '00:0' ||
				$(`[hora-ativ=${idloteativ}][vnulo]`).val() == '000:00') {

				alertAtencao('Favor preencher a duração da atividade.');
				$(`[hora-ativ=${idloteativ}][vnulo]`).addClass("alertaCbvalidacao").focus();
				return false;
	...
	async function alteraStatusECancelarAtiv(idLoteAtivAtual, status, bloqueiaStatus, idProxLoteAtiv) {
		if (status == 'CONCLUIDO') {
			if(idProxLoteAtiv) {
				let error = false;

				idProxLoteAtiv.forEach(id => {
					if ($(`[hora-ativ=${id}][vnulo]`).val() == '00:00' ||
						$(`[hora-ativ=${id}][vnulo]`).val() == '' ||
						$(`[hora-ativ=${id}][vnulo]`).val() == '0' ||
						$(`[hora-ativ=${id}][vnulo]`).val() == '00' ||
						$(`[hora-ativ=${id}][vnulo]`).val() == '00:0' ||
						$(`[hora-ativ=${id}][vnulo]`).val() == '000:00') {

						alertAtencao('Favor preencher a duração da atividade.');
						$(`[hora-ativ=${id}][vnulo]`).addClass("alertaCbvalidacao").focus();
						error = true;
						return false;
					}
				});
				...
				if ($(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).val() == '00:00' ||
					$(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).val() == '' ||
					$(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).val() == '0' ||
					$(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).val() == '00' ||
					$(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).val() == '00:0' ||
					$(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).val() == '000:00') {

					alertAtencao('Favor preencher a duração da atividade.');
					$(`[hora-ativ=${idLoteAtivAtual}][vnulo]`).addClass("alertaCbvalidacao").focus();
					return false;
				}
			}
		}
		...
		// Definir status da atividade atual como "Não conforme"
		objetos = {
			_statusla_u_loteativ_idloteativ: idLoteAtivAtual,
			_statusla_u_loteativ_status: `${status}`,
			_statusla_u_loteativ_idfluxostatus: `${idFluxoStatusNaoConforme}`,
			_statusla_u_loteativ_bloquearstatus: `${bloqueiaStatus}`
		}

		if(idProxLoteAtiv) {
			idProxLoteAtiv.forEach((id, indice) => {
				objetos[`_${indice + 1}_u_loteativ_idloteativ`] = id;
				objetos[`_${indice + 1}_u_loteativ_idfluxostatus`] = idFluxoStatusCancelado;
				objetos[`_${indice + 1}_u_loteativ_status`] = 'CANCELADO';
				objetos[`_${indice + 1}_u_loteativ_bloquearstatus`] = 'N';
			});
		}
		...
		CB.post({
			...
			posPost: function() {
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						idloteativ: idProxLoteAtiv.join(),
						idfluxostatus: idFluxoStatusOP,
						tipo: 'cancelarAtividades',
						idformalizacao: idFormalizacao,
						_modulo: CB.modulo,
						motivo: $('#motivo-nao-conforme').val(),
						atividadeNaoConforme: true,
						idprproc,
						idlote: $_2_u_lote_idlote,
						idunidade
					},
	...
	function atualizaPlaceholderSugestao(inTr) {
		...
		$.each($ins.find(".itemestoque[idlote]"), function(ie, iEst) {
			...
			$iQtdd = $(iEst).find("input[nome*=lotecons_qtdd]");
	...
	let idProdservLancamentoPerda = 0;

	function lancarPerda() {
		const corpoModal = `
							<div class="col-12 d-flex flex-wrap align-items-end">
								<div class="col-4">
									<label for="">Tipo</label>
									<select id="opcoes" class="form-control"> 
										<option id="consumo-producao">Consumo - Produção</option>
										<option id="perda-producao">Perda - Produção</option>
									</select>
							</div>
							...
							<div class="col-12 d-flex flex-wrap align-items-end">
								<div class="col-4">
									...
									<input id="produtos-perda" type="text" class="form-control" />
								</div>

								<div class="col-4">
									<label for="">Lote</label>
									<input id="lote-perda" type="text" class="form-control desabilitado" disabled />
								</div>

								<div class="col-4">
									<label for="">Quantidade <span id="label-perda-qtd"></span></label>
									<input id="qtd-lote-perda" type="text" class="form-control desabilitado"  sQtddisp_exp="" sQtddisp="" disabled onkeyup="mostraConsumodes(this)" onchange="verificadiluicaodes(this)"/>
								</div>

								<div class="col-4">
									<button id="btn-gerar-perda" class="btn btn-primary" disabled>Lançar</button>
								</div>
							</div>`;
		CB.modal({
			corpo: corpoModal,
			...
			classe: 'oitenta',
			aoAbrir: () => {
				$("#produtos-perda").autocomplete({
					source: prodsersAtivos,
					delay: 0,
					create: function() {
						$(this).data('ui-autocomplete')._renderItem = function(ul, item) {
					...
					select: function(event, ui) {
						desabilitarCampo($('#lote-perda'))
						desabilitarCampo($('#qtd-lote-perda'))
						desabilitarCampo($('#btn-gerar-perda'), false)
						alterarLabelEstoqueMaximo();

						$.ajax({
							type: "GET",
							url: "ajax/lote.php",
							data: {
								action: 'buscarLotesDisponivesPorIdProdserv',
								params: [ui.item.value, idunidade]
							},
							dataType: 'json',
							success: function(lotes) {
								if (!lotes.length) {
									alert('Nenhum lote encontrado para esta unidade');
									return;
								}
								...
								for (idlote in lotes) {
									lotesArr.push({
										label: lotes[idlote]['lote'],
										value: idlote,
										...lotes[idlote]
									});
								}
								habilitarCampo($("#lote-perda"));

								$("#lote-perda").autocomplete({
									source: lotesArr,
									delay: 0,
									create: function() {
										$(this).data('ui-autocomplete')._renderItem = function(ul, item) {
											return $('<li>').append("<span> " + item.lote + "</span>").appendTo(ul);
										}
									},
									select: function(e, itemPerda) {
										alterarLabelEstoqueMaximo(`${itemPerda.item.estoque}`);
										habilitarCampo($('#qtd-lote-perda'));
										habilitarCampo($('#btn-gerar-perda'));
										obs = $('#opcoes').val();

										$('#btn-gerar-perda').off();
										$('#btn-gerar-perda').on('click', () => {
											gerarPerdaLote(itemPerda.item.idlote, itemPerda.item.idlotefracao, itemPerda.item.estoque, obs);
										})
									}
								})
							},
	...
	function lancarInsumo() {
		const corpoModal = `
							<div class="col-12 d-flex flex-wrap px-0 align-items-start">
								...
								<div class="col-xs-5 d-flex flex-wrap align-items-end">
									<div class="col-xs-11 pt-0">
										<div class="col-xs-12 pt-0 px-0 float-none">
											<label for="">Produto</label>
											<input id="produtos-perda" type="text" class="form-control" />
										</div>
										<div id="lotes-container" class="lotes w-100 rounded mt-4 hidden flex-col">
											...
											<div class="d-flex my-3">
												<strong class="text-uppercase mr-2">Estoque total:</strong>
												<h5 id="estoque-total" class="m-0">0</h5>
											</div>
										</div>
									</div>
									<div class="col-4">
										<button id="btn-gerar-perda" class="btn btn-primary" disabled onClick="consumirLote()">Salvar</button>
									</div>
								</div>
							</div>`;
		...
		CB.modal({
			corpo: corpoModal,
			titulo: "Lançar Insumos",
			classe: 'oitenta',
			aoAbrir: () => {
				$("#produtos-perda").autocomplete({
					source: prodsersInsumosAtivos,
					delay: 0,
					create: function() {
						$(this).data('ui-autocomplete')._renderItem = function(ul, item) {
							return $('<li>').append("<span> " + item.label + "</span>").appendTo(ul);
					...
					select: function(event, ui) {
						...
						idProdservLancamentoPerda = ui.item.value;
						buscarLotesPorIdProdserv(idProdservLancamentoPerda);
					}
				});
			}
		});
	...
	function buscarLotesPorIdProdserv(idProdserv) {
		$('#cbModal').addClass('disable-all');
		...
		$.ajax({
			...
			data: {
				action: 'buscarLotesDisponivesPorIdProdserv',
				params: [idProdserv, idunidade]
			},
			dataType: 'json',
			success: function(lotes) {
				let lote,
					loteHTML = "<h4>Nenhum lote encontrado.</h4>",
					totalEstoque = 0;
				...
				if (!lotes.length) {
					alert('Nenhum lote encontrado para esta unidade');
					$('#linhas-lote').html(loteHTML);
					$('#estoque-total').html(0);
					$('#cbModal').removeClass('disable-all');
					return;
				}
				...
				for (idlote in lotes) {
					...
					if(!unPadrao) unPadrao = lote.unpadrao;
					
					if(lote.estoque && (lote.estoque.toString().includes('d') || lote.estoque.toString().includes('e'))) {
						let letraDivisao = lote.estoque.toString().includes('d') ? 'd' : 'e';
						let [valor, densidade] = lote.estoque.split(letraDivisao);
						totalEstoque += parseFloat(valor ?? 0);
					} else {
						totalEstoque += lote.estoque ? parseFloat(lote.estoque) : 0;
					}

					loteHTML += `<div class="row m-0 w-100 d-flex bg-primary text-white align-items-center mb-2 rounded">
									<div class="col-xs-1">
										<a tabindex="-1" class="text-white" href="/?_modulo=${lote.modulo}&_acao=u&idlote=${lote.idlote}&_idempresa=${lote.idempresa}" target="_blank">
											<strong>${lote.idlote}</strong>
										</a>
									</div>
									<div class="col-xs-2">
										<a tabindex="-1" class="text-white" href="/?_modulo=${lote.modulo}&_acao=u&idlote=${lote.idlote}&_idempresa=${lote.idempresa}" target="_blank">
											<strong>${lote.partida}</strong>
										</a>
									</div>
									<div class="col-xs-2 alert-warning text-center cinza rounded">
										<strong>${lote.vencimento ? formatarData(lote.vencimento) : 'Não definido'}</strong>
									</div>
									<div class="col-xs-2 text-right">
										<strong class="text-uppercase">${lote.status}</strong>
									</div>
									<div class="col-xs-2">
										<strong>${lote.estoque}</strong>
										<strong>${lote.unpadrao ? lote.unpadrao : lote.unlote }</strong>
									</div>
									<div class="col-xs-3 d-flex">
										<input
											class="campo-lote"
											data-idlote="${lote.idlote}"
											data-idlotefracao="${lote.idlotefracao}"
											data-idunidade="${lote.idunidade}"
											data-estoque="${lote.estoque}"
											data-diluicao="${lote.diluicao}"
											type="text" 
											title="Valor a ser consumido" 
											placeholder="Valor a ser consumido" 
											style="height: 20px;" 
										/>
									</div>
								</div>`;
				...
				totalEstoque = `${totalEstoque.toFixed(2)} ${unPadrao}`;

				$('#linhas-lote').html(loteHTML);
				$('#estoque-total').html(totalEstoque);
				habilitarCampo($('#btn-gerar-perda'));
				$('#cbModal').removeClass('disable-all');
			},
			error: function(objxmlreq) {
	...
	function validarCampoConsumo(element) {
		const elementJQ = $(element);
		const regex = /[a-z]/i;

		let valorCampo = elementJQ.val(),
			valorEstoque = parseFloat(elementJQ.data('estoque'));

		elementJQ.removeClass('campo-lote-invalido');

		if(elementJQ.data('diluicao') && elementJQ.data('diluicao') != 'undefined') {
			const letraDivisao = valorCampo.includes('d') ? 'd' : 'e';
			const [valor, diluicao] = valorCampo.split(letraDivisao);
			const [valorReferencia, diluicaoReferencia] = elementJQ.data('diluicao').split(letraDivisao);

			if(!diluicaoReferencia) {
				elementJQ.addClass('campo-lote-invalido');
				alertAtencao(`Informe a diluição`);
				return false;
			}

			if(diluicao != diluicaoReferencia) {
				elementJQ.addClass('campo-lote-invalido');
				alertAtencao(`Valor de diluição deve ser de ${letraDivisao}${diluicaoReferencia}`);
				return false;
			}

			valorCampo = valor;
			valorEstoque = parseFloat(valorReferencia);
		}

		if(regex.test(valorCampo) || (isNaN(parseFloat(valorCampo)))) {
			elementJQ.addClass('campo-lote-invalido');
			alertAtencao('Valor a ser consumido contém caracteres inválidos.');
			return false;
		}

		if(parseFloat(valorCampo) > valorEstoque) {
			elementJQ.addClass('campo-lote-invalido');
			alertAtencao('Valor a ser consumido excede o estoque disponível.');
			return false;
		}
	...
	function verificadiluicaodes(vthis) {
		if (vthis.classList.contains('diluicaoerrada')) {
			// Se possuir, faça algo aqui
			alert("A diluição " + $(vthis).val() + " não é válida!");
			vthis.value = "";
	...
	function mostraConsumodes(inOConsumo) {
		inOConsumo.style.backgroundColor = "";
		inOConsumo.classList.remove('diluicaoerrada');
		inOConsumo.classList.remove('consumoerrado');
		inOConsumo.classList.remove('infdiluicao');
	...
	function habilitarCampo(campo) {
		$(campo).removeAttr('disabled').removeClass('desabilitado')
	}

	function desabilitarCampo(campo, comClasse = true, clear = false) {
		$(campo).prop('disabled', true)

		if (comClasse) $(campo).addClass('desabilitado')

		if (clear) limparCampo($(campo))
	}
	...
	function alterarLabelEstoqueMaximo(label = '') {
		if (label != '') {
			var nlabel = '(Estoque:' + label + ')';
		} else {
			var nlabel = '';
		}

		$('#label-perda-qtd').text(`${nlabel}`);
		sVlr = label;
		if (sVlr.toLowerCase().indexOf("d") > -1) {
			$arrExp = sVlr.toLowerCase().split('d');

			$('#qtd-lote-perda').attr('sQtddisp_exp', sVlr);
			fVlr = (parseFloat($arrExp[0]) * parseFloat($arrExp[1])).toFixed(2);
			fVlr = parseFloat(fVlr);
			$('#qtd-lote-perda').attr('sQtddisp', fVlr);
			...
			$('#qtd-lote-perda').attr('sQtddisp_exp', '');
			fVlr = parseFloat(sVlr).toFixed(2);
			$('#qtd-lote-perda').attr('sQtddisp', fVlr);
		}
	...
	function consumirLote() {
		const camposLoteJQ = $('#cbModal .campo-lote');
		let campoValido = true;
		let objetos = {};
		
		try {
			$('#cbModal').addClass('disable-all');
			camposLoteJQ.each((index, element) => {
				const elementJQ = $(element);

				if(elementJQ.val()) {
					campoValido = validarCampoConsumo(element);

					if(!campoValido) return false;

					objetos[`_${index + 1}_i_lotecons_idobjeto`] = $_2_u_lote_idlote;
					objetos[`_${index + 1}_i_lotecons_tipoobjeto`] = 'lote';
					objetos[`_${index + 1}_i_lotecons_idlotefracao`] = elementJQ.data('idlotefracao');
					objetos[`_${index + 1}_i_lotecons_idlote`] = elementJQ.data('idlote');
					objetos[`_${index + 1}_i_lotecons_obs`] = $('#opcoes').val();
					objetos[`_${index + 1}_i_lotecons_qtdd`] = elementJQ.val();
				}
			});
	...
	function gerarPerdaLote(idLote, idFracao, estoque, obs) {
		//const qtdPerca = parseInt($('#qtd-lote-perda').val());
		const qtdPerca = $('#qtd-lote-perda').val();
		//if(!qtdPerca || isNaN(qtdPerca)) return alertAtencao(`Informe a quantidade a ser consumida`);
		//if(qtdPerca > estoque) return alertAtencao(`Quantidade excede o estoque. estoque atual ${estoque}`);

		desabilitarCampo($('#qtd-lote-perda'));
		desabilitarCampo($('#produtos-perda'));
		desabilitarCampo($('#lote-perda'));
		desabilitarCampo($('#btn-gerar-perda'), false);

		CB.post({
			objetos: {
				_x_i_lotecons_idobjeto: $_2_u_lote_idlote,
				_x_i_lotecons_tipoobjeto: 'lote',
				_x_i_lotecons_idlotefracao: idFracao,
				_x_i_lotecons_idlote: idLote,
				_x_i_lotecons_qtdd: qtdPerca,
				_x_i_lotecons_obs: obs,
	...
	function gerarbioensaio(vthis){

		var idloteativ =$(vthis).closest('.loteativ').attr('idloteativ');

		CB.modal({
			url: "?_modulo=bioensaioformalizacao&_acao=u&idloteativ=" + idloteativ,
			header: "Bioensaios da Formalização"
		});

	}

	CB.on('posPost', () => {
		const qtdEstoqueAnterior = parseInt($('#label-perda-qtd').text().replace('(', '').replace(')', '').split(':')[1]);
		const qtdEstoquePerda = $('#qtd-lote-perda').val() ?? 0;

		if (!isNaN(qtdEstoqueAnterior)) alterarLabelEstoqueMaximo(`${qtdEstoqueAnterior - qtdEstoquePerda}`);

		$('#qtd-lote-perda').val('');
		$('#cbModal .campo-lote').val('');
		habilitarCampo($('#qtd-lote-perda'));
		habilitarCampo($('#lote-perda'));
		habilitarCampo($('#produtos-perda'));
		habilitarCampo($('#btn-gerar-perda'));

		if(idProdservLancamentoPerda) buscarLotesPorIdProdserv(idProdservLancamentoPerda);
	});
