...
<script type="text/javascript">
    const idFormaPagamentoCB = '<?= $_1_u_conciliacaofinanceira_idformapagamento; ?>',
        ...
        visualizarMensagemMobile = <?= $visualizarMensagemMobile ? 'true' : 'false' ?>,
        idFluxoStatusAberto = '<?= $idFluxoStatusAberto ?>',
        statusConciliacao = '<?= $_1_u_conciliacaofinanceira_status; ?>',
        statusAberto = 'INICIO',
        gestor = <?= $gestor ? 'true' : 'false' ?>,
        financeiro = <?= $financeiro ? 'true' : 'false' ?> ;
    ...
    if (
        permissaoEditar &&
        (
            statusConciliacao == 'INICIO' || 
            statusConciliacao == 'EMCONCILIACAO' || 
            statusConciliacao == 'APROVACAOSOLICITADA' || 
            statusConciliacao == 'AGUARDANDO' ||
            statusConciliacao == 'CORRIGIDO' 
        )
    ) {
    ...
    function montarItemLancamento(item, precisaAssociar = false) {
        ...
        if (financeiro) iconeLinkFatura = ` <a href="${linkFatura}" target="_blank" class="flex">
        ...
        if (item.mensagem) {
            ...
            if (visualizarMensagemMobile && !gestor && !financeiro) {
        ...
        if (item.id.indexOf('id') === -1 && financeiro) {
            ...
            .then(function(dados) {
                if (arquivoExcel) {
                    ...
                    const linhaDataInicio = $('#linha-excel-data-inicio').val(),
                        linhaDataFim = $('#linha-excel-data-fim').val(),
                    ...
                    for (const indice in workbook.Sheets) {
                        let sheet = workbook.Sheets[indice], 
                            linhaLimite = linhaDataFim ? parseInt(linhaDataFim) : parseInt(removerLetras(sheet['!ref'].split(':')[1]));

                        for (let i = parseInt(linhaDataInicio); i < linhaLimite; i++) {
    ...
    function abrirModalSelecaoColunasExcel(caminhoArquivo) {
        const camposModal = `<div class="row d-flex align-items-center flex-wrap">
                                        ...
                                        <div class="col-xs-3 form-group">
                                            <label>Linhas das datas de lançamento</label>
                                            <div class="d-flex flex-wrap flex-between">
                                                <input id="linha-excel-data-inicio" class="form-control col-xs-6" placeholder="Inicio" type="number" title="Digite o número da linha da primeira data."/>
                                                <input id="linha-excel-data-fim" class="form-control col-xs-6" placeholder="Fim" type="number" title="Digite o número da linha da última data."/>
                                            </div>
                                        </div>
                                        <div class="col-xs-3 form-group">
