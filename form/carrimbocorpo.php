...
//Inserção da condição de filtro por Plantel.
if(array_key_exists("STRCONTATOCLIENTE", $_SESSION["SESSAO"])){
	$clausula .= " and ta.idpessoa in( ".$_SESSION["SESSAO"]["STRCONTATOCLIENTE"].",".$_SESSION["SESSAO"]["IDPESSOA"].") ";
}
 ...
 */
	...
	if($booexeccount==true){
		$sqlcount = "select ta.idamostra 
			    ...
			    ".$clausula."
			    and ta.status='".$vstatus."'
			    -- and not exists (select 1 from resultado r where r.idamostra=ta.idamostra and r.status in('ABERTO','PROCESSANDO'))
				-- and not exists (select 1 from resultado r where r.idamostra=ta.idamostra and r.status in('PROCESSANDO'))
...
function hide($inCol){
	...
	$sql = 'select * from amostracampos where idunidade = 9 and idsubtipoamostra = '.$idsubtipoamostra.' and campo="'.$inCol.'"';
	...
	if($arrConf["colunas"]["9"][$idsubtipoamostra][$inCol]){
		return "";
	}else{
		return "hidden";
	}
...
if($oAm["status"]=="ABERTO" or $oAm["status"]=="ENVIADO"){
	...
	$sub="TEA";
}elseif($oAm["status"]=="ASSINAR" or $oAm["status"]=="ASSINADO"){
