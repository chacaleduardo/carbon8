...
if($_1_u_amostra_idamostra){
	...
	if((!empty($_1_u_amostra_idamostra) && $_1_u_amostra_idunidade != 9 && $r >= 1 && $_1_u_amostra_status != "ABERTO")
		|| ($_1_u_amostra_status == "PROVISORIO" && !array_key_exists("amostramastercomercial", getModsUsr("MODULOS"))) //Comercial 
		|| ($_1_u_amostra_status == "ABERTO" && !array_key_exists("amostramasterautogena", getModsUsr("MODULOS"))) // Autogenas
		|| ($_1_u_amostra_status != "PROVISORIO" && $_1_u_amostra_status != "ABERTO" && !array_key_exists("amostramaster", getModsUsr("MODULOS")))){ //Laercio
...
function linksolfb(){
	...
	<div class="col-md-7">
		<div class="panel panel-default">
				...
				<table id="amostra" style="width: 100%;">
					<tr>
						...
						<td style="width: 100px;">
							...
							<input <?=$redonAssi ?> name="_statusant_" type="hidden" value="<?=$_1_u_amostra_status ?>">
						...
						<? if($_1_u_amostra_status == 'PROVISORIO' and !empty($_1_u_amostra_idamostra)){?>
							<td>
								<a title="Imprimir TEA" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="janelamodal('report/tra.php?idamostra=<?=$_1_u_amostra_idamostra ?>&unidadepadrao=9&provisorio=Y')"></a>
							</td>
						<? } ?>
						<? if($_1_u_amostra_status != 'PROVISORIO' and !empty($_1_u_amostra_idamostra) and $unidadepadrao != 9){?>
							<td>
								<a title="Imprimir Amostra" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="janelamodal('report/impamostra.php?idamostra=<?=$_1_u_amostra_idamostra ?>')"></a>
							</td>
						<? } ?>
					</tr>
					<?
			...
			<div class="panel-body">
				...
				<div class="row">
					<div class="col-md-3 text-right" style="font-size: 12px;">Cliente:</div>
					<div class="col-md-6">
						<?
				...
				<div class="row rowDin hidden">
					<?	$doubleClique = (array_key_exists("amostramaster", getModsUsr("MODULOS")) == true || (array_key_exists("quantidadeamostramaster", getModsUsr("MODULOS")) == true && $_1_u_amostra_status == 'ABERTO')) ? "removeReadonly('#nroamostra')" : ''; ?>
					<div class="col-md-3 text-right" id="lb_nroamostra" ondblclick="<?=$doubleClique?>">Quantidade:</div>
					<div class="col-md-3" id="col_nroamostra" colspan="10">
						<div class="input-group input-group-sm">
							<input readonly placeholder="Não Informado" type="text" name="_1_<?=$_acao?>_amostra_nroamostra" id="nroamostra" title="Qtd." value="<?=$_1_u_amostra_nroamostra ?>" size="10" autocomplete="off">
							<? if(!empty($_1_u_amostra_nroamostra) and !empty($_1_u_amostra_idamostra)){
								...
								$qtdind = count($resind);
								if(($_acao == 'u')) {
									if($_1_u_amostra_status == 'ABERTO' && array_key_exists("amostramasterautogena", getModsUsr("MODULOS"))){?>
										...
										<span class="input-group-addon indicador" title="Editar Núcleo">
											<i class="fa fa-plus-circle pointer " onclick="NovaidentificacaoSemQtd(<?=$_1_u_amostra_idamostra ?>)" title="Inserir mais uma identificação!"></i>
										</span>
									<?}

									if(($_1_u_amostra_status != "PROVISORIO" && $_1_u_amostra_status != "ABERTO" && array_key_exists("amostramaster", getModsUsr("MODULOS")))){
										?>
										<span class="input-group-addon indicador" title="Limpar informações de Núcleo">
											<i class="fa fa-arrows-v pointer hoverazul" data-toggle="collapse" title="Editar identificadores" href="#identificadores"></i>
										</span>
				...
				<? if(($_1_u_amostra_status == 'ABERTO' && array_key_exists("amostramasterautogena", getModsUsr("MODULOS")))
						|| ($_1_u_amostra_status != "PROVISORIO" && $_1_u_amostra_status != "ABERTO" && array_key_exists("amostramaster", getModsUsr("MODULOS")))) { ?>
					<div id="identificadores" class="collapse" style="margin-left: 50px;">
						<div class="row">
							<div class="col-md-12">
								<div class="panel panel-default">
									<div class="panel-heading">
											...
											<tr>
												...
												foreach($resind as $k => $rowiy){
													...
													<td class="nowrap"><?=$linha ?>-
														...
														<input name="_<?=$b ?>_<?=$_acao?>_identificador_identificacao" style="text-transform: uppercase; width: 75%;" class="identificador_<?=$c?>" type="text" value="<?=strtoupper($rowiy['identificacao'])?>">
														&nbsp;&nbsp;
														<i class="fa fa-trash fa-1x cinzaclaro hoververmelho btn-lg pointer ui-droppable" style="padding-left: 2px !important;" contador="<?=$c?>" onclick="excluiridentificacao(this,<?=$rowiy['ididentificador']?>,<?=$b ?>)" alt="Excluir identificação!"></i>&nbsp;&nbsp;&nbsp;&nbsp;
													</td>
													<? $c++; ?>
												<? } ?>
											</tr>
				...
				<? } ?>
				<div class="row rowDin hidden">
					<?	$doubleClique = (array_key_exists("amostramaster", getModsUsr("MODULOS")) == true || (array_key_exists("quantidadeamostramaster", getModsUsr("MODULOS")) == true && $_1_u_amostra_status == 'ABERTO')) ? "removeReadonly('#descricao')" : ''; ?>
					<div class="col-md-3 text-right" id="lb_nroamostra" ondblclick="<?=$doubleClique?>">Descri&ccedil;&atilde;o:</div>
					<div class="col-md-9 d-flex align-items-center flex-wrap" id="col_descricao">
						<div class="input-group input-group-sm" style="display: block;" onchange="Novaidentificacao(<?=$_1_u_amostra_idamostra?>)">
							<input type="number" class="size5 form-control" id="qtdsum" min="0" pattern="[0-9]+([,\.][0-9]+)?">
							<select id="descrconcat"class="size40 form-control" title="Descrição" value="<?=$_1_u_amostra_descricao?>" autocomplete="off" onchange="Novaidentificacao(<?=$_1_u_amostra_idamostra?>)">
								<option></option>
								<? fillselectNoError(DescricaoAmostraQuery::buscarDescricaoAmostraAtivoParaFillSelect()); ?>
							</select>
							<div></div>
							<div id="ident_temp">
								<? if(!empty($_1_u_amostra_descricao) && $_1_u_amostra_status == 'PROVISORIO') {  
									$campos = explode(";", $_1_u_amostra_descricao);
									$i = 999999;
									$index = 0;
									foreach($campos as $_listarCampos){										
										$dadosCampo = explode(" ", trim($_listarCampos));
										?>
										<div class="input-group input-group-sm" id="<?=$i?>" style="padding: 5px 0px 5px 0px;">
											<input class="size5 form-control" id="qtdsum<?=$i?>" readonly value="<?=$dadosCampo[0]?>">
											<input class="size40 form-control" id="descr<?=$i?>" readonly value="<?=$dadosCampo[1]?>">
											<span>
												<i class="fa fa-trash pointer" style="padding-left: 10px; padding-top: 6px;" onclick="removePorSeparador('<?=$i?>');"></i>
											</span>
										</div>
										<?
										$i++;
							...
							<div id="nova_ident"></div>
						</div>
						<div style="width: 100%;">
							<input type="text" id="descricao" name="_1_<?=$_acao?>_amostra_descricao" readonly title="Descrição" value="<?=$_1_u_amostra_descricao?>" autocomplete="off" style="margin-top: 5px;">
						</div>
						<div id="ident_add" class="hidden"></div>
					</div>
				</div>

				<hr>
				<div class="row rowDin hidden">
					<div class="col-md-3 text-right" id="lb_nucleoamostra">Núcleo:</div>
					<div class="col-md-9" id="col_nucleoamostra">
						<div >
							<input title="Núcleo Amostra" name="_1_<?=$_acao?>_amostra_nucleoamostra" id="nucleoamostra" value="<?=$_1_u_amostra_nucleoamostra ?>">
						</div>
					</div>
				...
				<div class="row rowDin hidden">
					...
					<div class="col-md-3" id="col_idade">
						...
						<span class="hidden cinza" id="regraidademapa"></span>
					</div>
				</div>
				<div class="row rowDin hidden">
				...
				<div class="row rowDin hidden">
					<div class="col-md-3 text-right" id="lb_numeroanimais">Total de animais na propriedade:</div>
				...
				<div class="row rowDin hidden">
					...
					<div class="col-md-3 text-right" id="col_mortalidade">
						...
						<span id="regramortalidademapa" class="hidden cinza"></span>
					</div>
				</div>
				...
				<div class="row rowDin hidden">
					...
					<div class="col-md-9" id="col_condconservacao">
						<select <?=$redonAssi ?> name="_1_<?=$_acao?>_amostra_condconservacao" title="Condições de conservação" cols="61" rows="3">
							<?fillselectNoError([
								'Boas' => 'Boas',
								'Médias' => 'Médias',
								'Ruins' => 'Ruins',
							],$_1_u_amostra_condconservacao)?>
						</select>
					</div>
				</div>

				<div class="row rowDin hidden">
					<div class="col-md-3 text-right" id="lb_valorobjeto">Temperatura de<br> recebimento do Material:</div>
				...
				<div class="row rowDin hidden">
					...
					<? } else {?>
						...
						<div class="col-md-9" colspan="5" id="col_responsavel">
							<input <?=$redonAssi ?> autocomplete="off" type="text" name="_1_<?=$_acao?>_amostra_responsavel" id="responsavel" title="Responsável Coleta" value="<?=$_1_u_amostra_responsavel ?>">
						</div>
					<? } ?>
					...
					<? } ?>
					<div class="col-md-3 text-right" id="lb_responsavelcolcont">Contato:</div>
				...
				<? if((empty($_1_u_amostra_responsavelof) || !empty($_1_u_amostra_idpessoaresponsavelof)) && ($_GET['_modulo'] == 'amostratraprovisorio' || $_GET['_modulo'] == 'amostratra')){?>
					...
					<? } ?>
					<? if(((empty($_1_u_amostra_responsavelofcrmv) && empty($_1_u_amostra_responsavelof)) || !empty($_1_u_amostra_idpessoaresponsavelcrmvof)) && ($_GET['_modulo'] == 'amostratraprovisorio' || $_GET['_modulo'] == 'amostratra')){?>
					...
					<? } ?>
					<div class="col-md-3 text-right" id="lb_responsaveloftel">Contato:</div>
				...
				<div class="row rowDin hidden">
					...
					<div class="col-md-9" id="col_observacao" colspan="5">
						<?if(empty($_1_u_amostra_observacao)){
								$_1_u_amostra_observacao = 'Não se aplica';
						}?>
						<textarea <?=$redonAssi ?> name="_1_<?=$_acao?>_amostra_observacao" id="observacao" title="Observação" cols="61" rows="3"><?=str_replace("<br />", "",nl2br($_1_u_amostra_observacao))?></textarea>
					</div>
				</div>
				<div class="row rowDin hidden">
				...
				if(($unidadepadrao == 9 /*&& strpos($_GET['_modulo'], 'provisorio') == true*/) or !empty($_1_u_amostra_sorologico) or !empty($_1_u_amostra_isolamento) or !empty($_1_u_amostra_pcr)){?>
					...
					<div class="row rowDin hidden">
						<div class="col-md-3 text-right" ondblclick="mostraInputHidden('#isolamento')" id="lb_isolamento">Isolamento:</div>
						<div class="col-md-9" id="col_isolamento" colspan="5">
							<div>
								<textarea class="hidden" <?=$redonAssi ?> name="_1_<?=$_acao?>_amostra_isolamento" id="isolamento" title="Isolamento" cols="61" rows="2"><?=$_1_u_amostra_isolamento?></textarea>
							</div>
							<div class="input-group input-group-sm" style="display: block;">
								<select id="isol" class="size40 form-control"  title="Isolamento" autocomplete="off" onchange="InserirPosicao('#isol','#isolamento')">
									<option></option>
									<? fillselectNoError(IsolamentoAmostraQuery::buscarisolamentoamostraAtivoParaFillSelect()); ?>
								</select>
							</div>
							<div class="col-md-12 px-0 mt-3" style="display: flex;flex-direction: row;flex-wrap: wrap;align-content: center;align-items: center;justify-content: flex-start;" id="divisolamento">
								<?if(!empty($_1_u_amostra_isolamento)){?>
									<?
										$arrIsolamento = explode(";", $_1_u_amostra_isolamento);
										$cont = 0;
										foreach($arrIsolamento as $isolamento){
											$cont++;
											?>
											<div class="col-md-12 panel-heading d-flex space-between px-0" >
												<span style="white-space: normal;"><?=$isolamento?> <i class="fa fa-trash fa-1x cinzaclaro hoververmelho btn-lg pointer ui-droppable" onclick="ExcluiPorPosicao(<?=$cont-1?>,'#isolamento')" alt="Excluir identificação!"></i></span>
											</div>
										<?}?>
									<?}?>
							</div>
				...
				<? } ?>
				<hr>
				<?if($_1_u_amostra_motivopendencia) {?>
					<div class="row rowDin">
						<div class="col-md-3 text-right" id="lb_parasitologico">Motivo da pendência:</div>
						<div class="col-md-9" id="col_parasitologico" colspan="5">
							<textarea title="Parasitológico" cols="61" rows="2" readonly disabled><?= AmostraController::$motivosPendencia[$_1_u_amostra_motivopendencia]  ?></textarea>
						</div>
					</div>
				<?}?>
			</div><!-- panel body -->
				...
				<div class="panel-heading">
					<table style="width: 100%; min-width:100%;">
						<tr>
							...
							<td>
								<a title="Imprimir TEA" class="fa fa-print pull-right fa-lg pointer cinza hoverazul" onclick="janelamodal('report/tra.php?idamostra=<?=$_1_u_amostra_idamostra ?>&unidadepadrao=9&provisorio=Y')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;"></a>
							</td>
						</tr>
						<tr>
							<td>
	...
	if($_GET['_acao'] == "u" && $_1_u_amostra_status == 'CONFERIDO'){
	...
	require_once(__DIR__."/js/amostratra_js.php")
	?>