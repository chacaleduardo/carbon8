...
//Alteração realizada pois estava acessando amostra com status diferente de provisorio no modulo amostratraprovisorio
//sislaudo.laudolab.com.br/?_modulo=evento&_acao=u&idevento=322523 - Lidiane - 26/05/2020
//if($_1_u_amostra_status != 'PROVISORIO' && strpos($_GET['_modulo'], 'provisorio') == true && $_GET['_acao'] != 'i'){
//	echo '<br /><br />';
//	echo "Esta amostra não se encontra como PROVISORIO. Busque em Diagnóstico -> TEA/TRA"; die();
//} 
...
if($_1_u_amostra_status != 'PROVISORIO' && strpos($_GET['_modulo'], 'provisorio') == false and $_1_u_amostra_idunidade != 9){
...
if($_acao=="i" && strpos($_GET['_modulo'], 'provisorio') == true){
	?>
	<script> $("body").addClass("novoprovisorio"); </script>
	<?
}
...
$sql_result = "SELECT count(r.status) AS assinado FROM laudo.amostra as a
				join resultado as r on (r.idamostra=a.idamostra)
			   where a.idamostra=$_1_u_amostra_idamostra and r.status = 'ASSINADO'";
$res_result = d::b()->query($sql_result);
$r = mysqli_fetch_assoc($res_result);
if(!empty($_1_u_amostra_idamostra) and $_1_u_amostra_idunidade != 9 and $r['assinado'] >= 1  and $_1_u_amostra_status != "ABERTO"){
	$redonAssi="readonly='readonly'";
	$disableDta="disabled='disable'";
	$tredonAssi="Amostra possui testes assinados.";
}else{
	$redonAssi="";$tredonAssi="";
}
...
?>
	...
	<td>
        ...
        <td>
			...
			?>
            <input type="checkbox" name="_<?=$i?>_u_resultado_cobrar" atval="<?=$cobrancaobrig?>" value="<?=$r["cobrar"]?>" <?=$checkedob?> idresultado="<?=$r["idresultado"]?>" onclick="flgcobrar(this)" style="border:0px" title="Cobrar"> 
...
<?}else{?>
		<input type="hidden" name="_<?=$i?>_u_resultado_idresultado" value="<?=$r["idresultado"]?>">
                <input type="hidden" name="_<?=$i?>_u_resultado_loteetiqueta" value="<?=$r["loteetiqueta"]?>">
				<a href="?_modulo=<?=$modResultadosPadrao?>&_acao=u&idresultado=<?=$r["idresultado"]?>" target="_blank" style="color:<?switch($r["status"]){case "ABERTO": ?>#c53632;<? break;case "PROCESSANDO": ?>#ffc107;<? break;case "FECHADO": ?>#40708F;<? break;case "ASSINADO": ?>#3c763d;<? break;default: ?>#333;<? break;}?>"><?=$r["codprodserv"]?> <span style="font-size:9px;"><?=$r["versao"]?></span></a>
				...
				<input type="hidden" class="idprodserv" cbvalue="<?=$r["idprodserv"]?>" value="<?=$r["codprodserv"]?>">
...
function logReabertura(){
	...
	if($_1_u_amostra_idamostra){
		$sqlimp="
		select 
			a.valor as edicaomotivo, b.valor as edicaoobs, DATE_FORMAT(a.criadoem, '%d/%m/%Y %H:%m:%s') as criadoem, a.criadopor 
		from 
			_auditoria a 
		join
			_auditoria b on a.objeto = b.objeto and a.idobjeto = b.idobjeto and a.criadoem = b.criadoem
		where 
			a.objeto = 'amostra'  and a.idobjeto =  ".$_1_u_amostra_idamostra."
...
?>
		...
		<div class="row" style="margin: 8px;background: #efefef;border-radius: 4px;">
			<div class="col-md-12" style="font-size:11px;"><b style="text-transform:uppercase;"><?=$rowimp['edicaomotivo']?></b>
				<p style="font-size:11px;"><?=$rowimp['edicaoobs']?></p>
...
function desenhaTRA(){
	...
	$arrTmp=$arrLoteTra;
		...
		$sqla="select  l.partida,l.exercicio,l.idlote,l.status,o.idobjeto,fr.status as statusfr
				...
				<tr>
					<td>			    
			    		<strong>
							<?if($unidadepadrao!=9){?>
								Registro
							<?}elseif($_GET['_modulo'] == 'amostratraprovisorio'){
								echo "TEA";
							} else {
								echo "TRA";
							}?>
				
							<? if(strpos($_GET['_modulo'], 'provisorio') == true and ($_GET['_modulo'] == 'amostratraprovisorio' or $_GET['_modulo'] == 'amostraavesprovisorio')){
								echo(" Provisório"); 
					...
					<td id="cabRegistro">
						...
						if(!empty($_1_u_amostra_idregistroprovisorio)){
					...
					<td style="width: 100px;">
						...
						<input <?=$disableDta?> name="_1_<?=$_acao?>_amostra_dataamostra" type="text" class="calendario" value="<?=$_1_u_amostra_dataamostra?>" autocomplete="off" vnulo style="<?=$stlAlertaDataAnterior?>" title="<?=$titleAlertaDataAnterior?>">
					</td>
					...
					<? if($unidadepadrao==9 and !empty($_1_u_amostra_idamostra) and strpos($_GET['_modulo'], 'provisorio') == false){ ?>
						...
						<td>
                            <i title="Imprimir Amostra" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="showModal()"></i>
                        </td>
					<? } ?>
					...
					<?if($_1_u_amostra_status=='PROVISORIO' and !empty($_1_u_amostra_idamostra)){?>
						...
						<td>
							...
							<select name="_1_<?=$_acao?>_amostra_status">
								<option value=""></option>
								<?fillselect("select 'PROVISORIO','EM ABERTO' union select 'CANCELADO','CANCELADO'",$_1_u_amostra_status);?>		
							</select>
						</td>
						<td>
							<i title="Imprimir TEA" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="janelamodal('report/tra.php?idamostra=<?=$_1_u_amostra_idamostra?>&unidadepadrao=9&provisorio=Y')"></i>
						</td>
					<?}?>   
					<?if($_1_u_amostra_status!='PROVISORIO' and !empty($_1_u_amostra_idamostra) and $unidadepadrao!=9){?>
                        <td>
                            <i title="Imprimir Amostra" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="janelamodal('report/impamostra.php?idamostra=<?=$_1_u_amostra_idamostra?>')"></i>
                        </td>
                    <?}?>
				</tr>
				<?
...
<div class="row rowDin hidden">
	<div class="col-md-3 text-right" id="lb_nroamostra">Qtd.
	</div>
	<div  class="col-md-3" id="col_nroamostra" colspan="10">
            <div class="input-group input-group-sm">
            <input <?=$redon?> placeholder="Não Informado" type="text"
		...
		value="<?=$_1_u_amostra_nroamostra?>" size="10" autocomplete="off" vnulo>
            <?if(!empty($_1_u_amostra_nroamostra) and !empty($_1_u_amostra_idamostra)){
                $sqlind="select * from  identificador where idobjeto=".$_1_u_amostra_idamostra." and tipoobjeto='amostra' order by ididentificador";
                $resind=d::b()->query($sqlind) or die("Erro ao buscar identificação sql=".$sqlind);
                $qtdind= mysqli_num_rows($resind);
                if($qtdind<1){
?>
             <span class="input-group-addon" title="Limpar informações de Núcleo">
            ...
            ?>
                    ...
                    <span class="input-group-addon" title="Editar Núcleo">
        ...
        <div class="col-md-12">
            ...
            <table>
               <tr>   
                ...
                $lin=0;
                $linha=0;
                while($rowiy=mysqli_fetch_assoc($resind)){
                    $b=$b+1;
                    $lin=$lin+1;
                    $linha=$linha+1;
                    if($lin==5){
...
<div class="row rowDin hidden">
	...
	<div class="col-md-9"  id="col_descricao">
		<input <?=$redon?> type="text" name="_1_<?=$_acao?>_amostra_descricao" title="Descrição" value="<?=$_1_u_amostra_descricao?>" autocomplete="off">
	</div>
</div>

<hr>
<div class="row rowDin hidden">
	<div class="col-md-3 text-right" id="lb_idnucleo">
<?
...
?>
		<select <?=$redon?> style="text-align-last:right;" name="_1_<?=$_acao?>_amostra_rotulonucleotipo" id="rotulonucleotipo">
...
?>
	<div class="col-md-9" id="col_idnucleo">
		<div class="input-group input-group-sm">
			...
			<input <?=$redonAssi?> type="hidden" readonly title="Núcleo Amostra" name="_1_<?=$_acao?>_amostra_nucleoamostra" id="nucleoamostra" value="<?=$_1_u_amostra_nucleoamostra?>">
		</div>
	</div>
	<div class="col-md-3 text-right" id="lb_idade">Idade:</div>
	<div class="col-md-3" id="col_idade" >
	    <input <?=$redonAssi?> vnulo placeholder="Não Informado" autocomplete="off" type="text"
			name="_1_<?=$_acao?>_amostra_idade" id="idade" title="Idade"
			value="<?=$_1_u_amostra_idade?>"  style="width:49%;display: inline-block;" >
...
<?}else{?>
	<div class="col-md-12" id="col_nucleoamostra" style="display: inline-block;">
		Núcleo: <input <?=$redonAssi?> autocomplete="off" type="text" readonly title="Núcleo Amostra" name="_1_<?=$_acao?>_amostra_nucleoamostra" id="nucleoamostra" value="<?=$_1_u_amostra_nucleoamostra?>" size="20" style="display: inline-block;" >
	</div>
	<div class="col-md-3 text-right" id="lb_idespeciefinalidade">
		Espécie/finalidade:
	</div>
	<div class="col-md-3" id="col_idespeciefinalidade">
		<input <?=$redonAssi?> type="text" name="_1_<?=$_acao?>_amostra_idespeciefinalidade"  title="Tipo de Espécie/Finalidade" cbvalue  vnulo>
	</div>
<?}?>
</div>
<div class="row rowDin hidden">
	<div class="col-md-3 text-right" id="lb_lote">Lote:</div>
	<div class="col-md-3" id="col_lote"><input <?=$redonAssi?> placeholder="Não Informado" type="text" name="_1_<?=$_acao?>_amostra_lote" title="Lote" id="lote" value="<?=$_1_u_amostra_lote?>" size="25" autocomplete="off" ></div>
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3 text-right" colspan="5" id="col_morbidade">
			<input <?=$redonAssi?> autocomplete="off" type="text" name="_1_<?=$_acao?>_amostra_morbidade" id="morbidade" title="" value="<?=$_1_u_amostra_morbidade?>">
		</div>
		...
		<div class="col-md-3 text-right" id="col_letalidade">
			<input <?=$redonAssi?> autocomplete="off" type="text" name="_1_<?=$_acao?>_amostra_letalidade" id="letalidade" title="" value="<?=$_1_u_amostra_letalidade?>">
		</div>
		...
		<div class="col-md-3 text-right" id="col_mortalidade">
			<input <?=$redonAssi?> autocomplete="off" type="text" name="_1_<?=$_acao?>_amostra_mortalidade" id="mortalidade" title="" value="<?=$_1_u_amostra_mortalidade?>">
		</div>
</div>
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_datachegada">
			<input <?=$redonAssi?> name="_1_<?=$_acao?>_amostra_datachegada" type="text" title="Data chegada Material" class="calendario" value="<?=$_1_u_amostra_datachegada?>" vnulo>
		</div>
</div>
<div class="row rowDin hidden">
		<div class="col-md-3 text-right" id="lb_formaarmazen">Forma de <br>Armazenamento:</div>
		<div class="col-md-3" id="col_formaarmazen">
			<input <?=$redonAssi?> autocomplete="off" type="text" name="_1_<?=$_acao?>_amostra_formaarmazen" id="clienteterceiro" title="Forma de Armazenamento" value="<?=$_1_u_amostra_formaarmazen?>">
		</div>
		<div class="col-md-3 text-right" id="lb_meiotransp">Meio de Transp. Amostras:</div>
		<div class="col-md-3" id="col_meiotransp">
			<input <?=$redonAssi?> autocomplete="off" type="text" name="_1_<?=$_acao?>_amostra_meiotransp" id="clienteterceiro" title="Meio de Transporte" value="<?=$_1_u_amostra_meiotransp?>">
		</div>
</div>
<div class="row rowDin hidden">
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_nucleoorigem" colspan="10"><input <?=$redonAssi?> placeholder="Não Informado" autocomplete="off" type="text"
		...
		<div class="col-md-3" id="col_tipo" colspan="5"><input <?=$redonAssi?> autocomplete="off" type="text" title="Tipo"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_especificacao"><input <?=$redonAssi?> autocomplete="off" type="text"
		...
		<div class="col-md-3" id="col_fornecedor"><input <?=$redonAssi?> autocomplete="off" type="text" title="Fornecedor"
		...
		<div class="col-md-3" id="col_partida"><input <?=$redonAssi?> autocomplete="off" type="text" title="Partida"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_datafabricacao" colspan="3"><input <?=$redonAssi?> autocomplete="off"
		...
		<div class="col-md-3" id="col_identificacaochip"><input <?=$redonAssi?> autocomplete="off" type="text"
		...
		<div class="col-md-3" colspan="3" id="col_diluicoes"><input <?=$redonAssi?> autocomplete="off" type="text"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_nroplacas"><input <?=$redonAssi?> autocomplete="off" type="text"
		...
		<div class="col-md-3" id="col_nrodoses" colspan="5"><input <?=$redonAssi?> autocomplete="off" type="text"
		...
		<div class="col-md-3" id="col_semana" colspan="5"><input <?=$redonAssi?> autocomplete="off" type="text"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_notafiscal" colspan="5"><input <?=$redonAssi?> autocomplete="off"
		...
		<div class="col-md-3" id="col_vencimento" colspan="5"><input <?=$redonAssi?> autocomplete="off"
		...
		<div class="col-md-3" id="col_fabricante" colspan="5"><input <?=$redonAssi?> autocomplete="off"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_sexadores" colspan="5"><input <?=$redonAssi?> autocomplete="off" type="text"
		...
		<div class="col-md-3" colspan="5" id="col_localexp"><input <?=$redonAssi?> autocomplete="off" type="text"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-9" colspan="5" id="col_localcoleta"><input <?=$redonAssi?> autocomplete="off"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-9" colspan="5" id="col_responsavel"><input <?=$redonAssi?> autocomplete="off"
		...
		<div class="col-md-3 text-right" id="col_responsavelcolcrmv"><input <?=$redonAssi?> autocomplete="off"
			type="text" name="_1_<?=$_acao?>_amostra_responsavelcolcrmv" id="responsavelcolcrmv" title="CRMV"
			value="<?=$_1_u_amostra_responsavelcolcrmv?>"></div>
		<div class="col-md-3 text-right" id="lb_responsavelcolcont">Respons&aacute;vel Coleta Contato:</div>
		<div class="col-md-3 text-right" id="col_responsavelcolcont"><input <?=$redonAssi?> autocomplete="off"
			type="text" name="_1_<?=$_acao?>_amostra_responsavelcolcont" id="responsavelcolcont" title="Contato"
			value="<?=$_1_u_amostra_responsavelcolcont?>"></div>
</div>
<div class="row rowDin hidden">
		<div class="col-md-3 text-right" id="lb_responsavelof">Respons&aacute;vel Oficial:</div>
		<div class="col-md-9" colspan="5" id="col_responsavelof"><input <?=$redonAssi?> autocomplete="off"
		...
		<div class="col-md-3 text-right" id="lb_responsavelofcrmv">Crmv:</div>
		<div class="col-md-3 text-right" id="col_responsavelofcrmv"><input <?=$redonAssi?> autocomplete="off"
		...
		<div class="col-md-3 text-right" id="col_responsaveloftel"><input <?=$redonAssi?> autocomplete="off"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-3" id="col_lacre"><input <?=$redonAssi?> vnulo placeholder="Não Informado" autocomplete="off" type="text"
		...
		<div class="col-md-3" id="col_tc"><input <?=$redonAssi?> vnulo placeholder="Não Informado" autocomplete="off" type="text"
...
<div class="row rowDin hidden">
		...
		<div class="col-md-9" id="col_observacao" colspan="5"><textarea <?=$redonAssi?>
...
<div class="row rowDin hidden">
		...
		<div class="col-md-9" id="col_observacaointerna" colspan="5"><textarea <?=$redonAssi?>
		...
		if(($unidadepadrao == 9 && strpos($_GET['_modulo'], 'provisorio') == true) or !empty($_1_u_amostra_sorologico) or !empty($_1_u_amostra_isolamento) or !empty($_1_u_amostra_pcr)){?>
			<div class="row rowDin hidden" style="dispay:none">
			...
			<div class="row rowDin hidden" style="dispay:none">
				<div class="col-md-3 text-right" id="lb_pcr">Histopatológico:</div>
				<div class="col-md-9" id="col_pcr" colspan="5">
					<textarea <?=$redonAssi?> name="_1_<?=$_acao?>_amostra_pcr" id="pcr" title="Histopatológico" cols="61" rows="2"><?=$_1_u_amostra_pcr?></textarea>
				</div>
			</div>
			<div class="row rowDin hidden" style="dispay:none">
...
if(strpos($_GET['_modulo'], 'provisorio') == false or $unidadepadrao!=9 ) {
	?>  
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<div>
				<? //if($_acao != 'i' && $_GET["duplicaramostra"] != 'Y') { ?>
					...
					if(!empty($_1_u_amostra_idamostra)){
						?>
						<i class="fa fa-print fa-2x cinzaclaro btn-lg pointer" onclick="imprimeTestes(<?=$_1_u_amostra_idamostra?>, '<?=$impressora?>');" alt="Imprimir Etiquetas"></i>
						<?
					}
					?>
...
//Valida se está ou não no Provisorio. Caso esteja estes campos não aparecerão (Lidiane - 30-03-2020) 
if($unidadepadrao==9 and !empty($_1_u_amostra_idamostra) and strpos($_GET['_modulo'], 'provisorio') == false){// Diagnóstico Autógenas 
			...
			<table style="width: 100%; min-width:100%;">
				<tr>
					...
					<td>
						...
						<a class="fa fa-bars pointer hoverazul" title="TEA" onclick="janelamodal('?_modulo=amostratraprovisorio&_acao=u&idamostra=<?=$_1_u_amostra_idamostra?>')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;"></a>
					</td>
					<td>
						<?
						$sqlpf="SELECT status 
								  FROM carrimbo c 
								 WHERE c.idobjeto =".$_1_u_amostra_idamostra."
								   AND c.tipoobjeto = 'amostratraprovisorio' 
								   AND c.idpessoa = ".$_SESSION["SESSAO"]["IDPESSOA"]."";
						$respf=d::b()->query($sqlpf) or die("Erro Assinatura Amostra sql=".$sqlpf);
						$qtdpf =mysqli_num_rows($respf);
						$row = mysqli_fetch_assoc($respf);
						if($qtdpf == 0){
							$assinatura = 'SEMASSINATURA';
						} else {
							$assinatura = $row['assinado'];
						}

						?>
						<select readonly>							
							<?fillselect(array("ASSINADO"=>"Assinado","PENDENTE"=>"Pendente", "SEMASSINATURA"=>"Sem Assinatura"),$assinatura)?>
						</select>
					</td>
					<td>
						<i title="Imprimir TEA" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="janelamodal('report/tra.php?idamostra=<?=$_1_u_amostra_idamostra?>&unidadepadrao=9&provisorio=Y')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;"></i>
					</td>
				</tr>
				<tr>
					<td>
...
<div class="col-md-12 inputs">
	<div class="cbupload" id="anexos" title="Clique ou arraste arquivos para cá" style="width:100%;">
		<i class="fa fa-cloud-upload fonte18"></i>
	</div>
</div>
<?
...
//LTM - 16-09-2020 - 373286: Alterta Caso haja idsectretaria em algum teste e ainda não foi marcado
/*
if(!empty($_1_u_amostra_idamostra)){
	$sqlSecretaria = "SELECT SUM(ISNULL(r.idsecretaria)) AS secnull, p.idsecretaria AS secretariapessoa
						FROM resultado r JOIN amostra a ON r.idamostra = a.idamostra 
						JOIN pessoa p ON p.idpessoa = a.idpessoa
						WHERE r.idamostra =".$_1_u_amostra_idamostra;
	$resSecretaria = d::b()->query($sqlSecretaria) or die("A Consulta Validação Secretaria falhou :".mysqli_error(d::b())."<br>Sql:".$sql); 
	$rowsSecretaria = mysqli_fetch_assoc($resSecretaria);
}
*/
?>
...
$(document).keydown(function(event) {
	...
	imprimeTestes(idamostra, '<?=$impressora?>');
...
function imprimeTestes(inIdamostra, impressora){
	...
	if(imprimir){
		$.ajax({
			type: "get",
			url : "ajax/impetiqueta.php?impressora="+impressora+"&idamostra="+inIdamostra,
			success: function(data){
				console.log(data);
	...
	if($bteditar.length==0){
		CB.novoBotaoUsuario({
			...
			,onclick:function(){
				...
				CB.post({
							objetos: "_x_u_amostra_idamostra="+$("[name=_1_u_amostra_idamostra]").attr("value")+
									"&_x_u_amostra_status=ABERTO"+
									"&_x_u_amostra_edicaomotivo="+_100_u_amostra_edicaomotivo+
									"&_x_u_amostra_edicaoobs="+_100_u_amostra_edicaoobs
							,parcial:true
							,refresh: "refresh"
						});
						$("#cbModalCorpo").html('');
...
function alterarNucleo(){
	...
	if(vIdNucleo){
		janelamodal('?_modulo=<?=$modulo?>&_acao=u&idnucleo='+vIdNucleo,1000,1100);
	}else{
...
function novoNucleo(){
	...
	janelamodal('?_modulo=<?=$modulo?>&_acao=i&idpessoa='+vIdpessoa,1000,1100);
}
				...
				<div class="col-md-12">
					<table style="width:100%;" id="imp_tabela">
						<table style="width:100%;" id="imp_tipos">
							<tr onclick="janelamodal('report/tra.php?idamostra=<?=$_1_u_amostra_idamostra?>&unidadepadrao=9')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;">
								<td style="padding: 10px 10px 10px 4px;">
									<i title="Imprimir TRA" class="fa fa-print pull-right fa-lg cinza hoverazul"></i>
								</td>
								<td>Imprimir TRA</td>
							</tr>
							<tr onclick="janelamodal('report/traresultados.php?idamostratra=<?=$_1_u_amostra_idamostra?>&unidadepadrao=9')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;">
								<td style="padding: 10px 10px 10px 4px;">
							...
							<tr onclick="janelamodal('report/emissaoresultado.php?_vids=<?=$_vids?>')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;">
								<td style="padding: 10px 10px 10px 4px;">
									<i title="Imprimir LDA" class="fa fa-print pull-right fa-lg cinza hoverazul"></i>	
								</td>
								<td>Imprimir LDA</td>
							</tr>
							<tr onclick="janelamodal('report/impamostra.php?idamostra=<?=$_1_u_amostra_idamostra?>')" style="cursor: pointer;border: 1px dotted #e4dada;border-radius: 80px;">
								<td style="padding: 10px 10px 10px 4px;">
									<i title="Imprimir Amostra" class="fa fa-print pull-right fa-lg cinza hoverazul" ></i>
								</td>
