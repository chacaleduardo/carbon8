...
$inspecionarConsultas=$_GET["inspecionarConsultas"];
//die();
?>
	       	...
	       	//maf200216: As inconsistencias de dashboard X idpessoa da amostra serao tratadas em outro lugar. Para facilitar o rastreamento de falhas, neste ponto teremos somente a tabela de dashboard
                $sqld = "select 
					...
					from
						resultado r
					join
						amostra a on (a.idamostra = r.idamostra and (not a.idamostratra is null and not a.idamostratra = '') )
					join
...
<?
            ...
            if($rowd["sumnvisualizado"] > 0){
                ...
                $class = "divnucleores";
                //$outra = "javascript:apagacontador(this);janelamodal('?_modulo=dashboardtrafiltro&_autofiltro=" . $_strjson . "',screen.availHeight,screen.availWidth); apagacontador(this);";
                ...
                $class = "divnucleores";
                //$outra = "javascript:apagacontador(this);janelamodal('?_modulo=cliente_visualizarresultados&_autofiltro=" . $_strjson . "',screen.availHeight,screen.availWidth); apagacontador(this);";
                $outra = "javascript:popNucleo(this,'N')";
                $outra = '"' . $outra . '"';
                ...
                $class = "divnucleovis";
                //$outra = "javascript:janelamodal('?_modulo=cliente_visualizarresultados&_autofiltro=" . $_strjson . "',screen.availHeight,screen.availWidth);apagacontador(" . $rown["idnucleo"] . "); apagacontador(this);";
...
function printNucleo(inUrl){
	...
	this.goUrlImpressao = function(){
		var vUrlPrint = "report/traamostraresultados.php?"
...
function popNucleo(inONucleo,inVis){
	...
	vGetAutofiltro = "_modulo=dashboardtrafiltro&_autofiltro=[{\"col\":\"idpessoa\",\"id\":\""+idPessoa+"\",\"valor\":\""+nome+"\"},{\"col\":\"idnucleo\",\"id\":\""+idNucleo+"\",\"valor\":\""+nucleo+"\"}]&_ordcol=idamostratra&_orddir=desc";

	vGet = "_modulo=dashboardtrafiltro&_pagina=0&_ordcol=idamostratra&_orddir=desc&idpessoa="+idPessoa+"&idnucleo="+idNucleo;
			...
			if(!$.isEmptyObject(data)){
				...
				if(parseInt(data.numrows)>parseInt(CB.jsonModulo.limite)||data.numrows>200){
					...
					var tblRes = CB.montaTableResultados(data, function(obj, event){
						...
						janelamodal("report/traamostraresultados.php?" + oTr.attr("goParam"));
					});
