...
function getSetor()
{
    ...
    $sql = "select s.idsgsetor,s.setor
            from  sgsetor s 
            where s.status='ATIVO' 
             ...
             and not exists(
                        ...
                        where  v.idimmsgconf= ".$_1_u_immsgconf_idimmsgconf." 
                            and v.objeto ='sgsetor'
                            and s.idsgsetor = v.idobjeto				
                )
             ".getidempresa('s.idempresa', 'sgsetor')."  order by s.setor";
...
function listaSgsetor()
{
    ...
    $s = "select d.idimmsgconfdest,s.setor,s.idsgsetor,d.criadopor,d.criadoem,s.status
            from immsgconfdest d,sgsetor s
                where s.idsgsetor = d.idobjeto
                and d.objeto ='sgsetor'
                and d.idimmsgconf = ".$_1_u_immsgconf_idimmsgconf." order by s.setor";
    ...
    while ($r = mysqli_fetch_assoc($rts)) {
        ...
        echo "<tr><td title='".$title."'>".$r["setor"]."</td><td><i class=\"fa fa-trash fa-1x cinzaclaro hoververmelho btn-lg pointer ui-droppable\" onclick=\"retirasgsetor(".$r['idimmsgconfdest'].")\" title='Excluir!'></i></td><td><a class='fa fa-bars pointer hoverazul' title='Setor' onclick=\"janelamodal('?_modulo=sgsetor&_acao=u&idsgsetor=".$r["idsgsetor"]."')\"></a></td></tr>";
    }
    echo "</tbody></table>";
}
...
{
    global $_1_u_immsgconf_idimmsgconf;
    $s = "select d.idimmsgconfdest,s.nome,s.idpessoa,d.criadopor,d.criadoem,d.status,s.idtipopessoa
    ...
    while ($r = mysqli_fetch_assoc($rts)) {
        ...
        echo "<tr id=".$r["idimmsgconfdest"]." class='".$opacity."'><td>".$r["nome"]."</td><td><i class='fa fa-check-circle-o $cor' status='".$r["status"]."' idimmsgconfdest='".$r["idimmsgconfdest"]."'  onclick='AlteraStatus(this)'></i></td> <td><a class='fa fa-bars pointer hoverazul' title='Setor' onclick=\"janelamodal('?_modulo=".$mod."&_acao=u&idpessoa=".$r["idpessoa"]."')\"></a></td></tr>";
    }
    echo "</tbody></table>";
}
...
function getJpessoa()
{
    ...
    $s = "select 
            ...
            where 1 ".getidempresa('a.idempresa', 'pessoa')."
                and a.status ='ATIVO'
                 ".$andtppes."
                    and not exists(
                            SELECT 1
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="_1_<?= $_acao ?>_immsgconf_tipo" class="size15" vnulo>
                                    <? fillselect("select 'E','Email'  union select 'ET','Email Token' union select 'EP','Email PDF' ", $_1_u_immsgconf_tipo); ?>
                                </select>
                            </td>
                        </tr>
...
<div class="col-md-12">
    <div class="panel panel-default">
        <div class="panel-heading" data-toggle="collapse" href="#localInfousr">Informações ao Usuário</div>
        <div class="panel-body collapse" id="localInfousr">
            <table>
                <tr>
                    <td>
                        <textarea style="width: 1267px; height: 114px;" name="_1_<?= $_acao ?>_immsgconf_informacao"><?= $_1_u_immsgconf_informacao ?></textarea>
                    </td>
                </tr>
            </table>
        </div>
...
?>
    ...
    if ($_1_u_immsgconf_status != 'ATIVO') {
            ...
            ?>
                    <div class="col-md-6">
                        ...
                        from (
                                ...
                                join immsgconfdest d on(  d.status='ATIVO'
                                                    AND p.idpessoa = d.idobjeto
                                                    AND d.idimmsgconf = ".$_1_u_immsgconf_idimmsgconf.")
        ...
        $sqllog = "select l.idpk,l.modulo,mf.col,l.status as statuslog, l.criadoem,b.msg,m.status ,m.tipo,p.idpessoa,p.nomecurto
            from  immsgconflog l 
            left join immsgbody b on (b.idimmsgbody = l.idimmsgbody)
            left join immsg m on (m.idimmsgbody = b.idimmsgbody)
            left join pessoa p on (p.idpessoa = m.idpessoa)
            left join carbonnovo._modulofiltros mf on( l.modulo = mf.modulo and mf.parget ='Y' )
            where l.idimmsgconf =".$_1_u_immsgconf_idimmsgconf." order by l.criadoem desc limit 400";
        ...
        select: function(event, ui) {
            CB.post({
                objetos: {
                    ...
                    "_x_i_immsgconfdest_objeto": 'sgsetor'
                },
                parcial: true
            });
        }
