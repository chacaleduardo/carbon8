...
function jsonClientes(){
	...
	$sqlc = "SELECT p.idpessoa
			, if(p.centrocusto !='',concat(p.nome,' - ',p.centrocusto),p.nome) as nome
			,pf.observacaore
			,p.cpfcnpj
...
function listaTestes(){
	global $_acao, $_1_u_amostra_idamostra, $_1_u_amostra_idpessoa,$arrCores,$modResultadosPadrao,$iAgentes,$unidadepadrao;
...
?>
<tr class="<?=$classDrag?>" idresultado="<?=$r["idresultado"]?>">
	<td>
...
<?}else{?>
		<input type="hidden" name="_<?=$i?>_u_resultado_idresultado" value="<?=$r["idresultado"]?>">
                <input type="hidden" name="_<?=$i?>_u_resultado_loteetiqueta" value="<?=$r["loteetiqueta"]?>">
		<a href="?_modulo=<?=$modResultadosPadrao?>&_acao=u&idresultado=<?=$r["idresultado"]?>" target="_blank"><?=$r["codprodserv"]?></a>
<?}?>
	</td>
	<td style="white-space: nowrap;">
		<input type="text" name="_<?=$i?>_u_resultado_quantidade" value="<?=$r["quantidade"]?>" style="width:50px" placeholder="Quant." vnulo vnumero>
</td>
<?
...
<div class="row">
	...
	<div class="col-md-8">
            ...
            ?>
		<input <?=$redon?> title="<?=$tredon?>" name="_1_<?=$_acao?>_amostra_idpessoa" type="text" cbvalue="<?=$_1_u_amostra_idpessoa?>" value="<?=traduzid("pessoa","idpessoa","nome",$_1_u_amostra_idpessoa)?>" vnulo>
	</div>
	<div class="col-md-3">
		<table>
		<tr>
...
?>
	...
	<div class="col-md-4" id="col_idade" >
		...
		<select name="_1_<?=$_acao?>_amostra_tipoidade" id="tipoidade" style="width:45%;display: inline-block;" >
			<option></option>
		<?fillselect(array('Dia(s)'=>'Dia(s)','Semana(s)'=>'Sem(s)','Mês(es)'=>'Mês(es)','Ano(s)'=>'Ano(s)'),$_1_u_amostra_tipoidade); ?>
		</select>
	</div>
...
<div class="row rowDin hidden">
	...
	<div class="col-md-2" id="col_regoficial">
...
<div class="row rowDin hidden">
	...
	<div class="col-md-2" id="col_nsvo">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<tr id="modeloNovoTeste">
				...
				<td><input type="text" name="#namequantidade" style="width:50px" placeholder="Qtd." vnulo vnumero></td>
				<td>
			...
			<table style="width: 100%; min-width:100%;">
				<tr>
					...
					<td >
						...
						<?fillselect(array("ABERTO"=>"Aberto","ENVIADO"=>"Enviado","DEVOLVIDO"=>"Devolvido"),$arrTRAAssociado["statustra"])?>
						</select>
						<?}else{?><label class="alert-warning"><?=$arrTRAAssociado["statustra"]?></label><?}?>
					</td>
					<td >
						<i title="Imprimir TEA/TRA" class="fa fa-print pull-right fa-lg cinza hoverazul" onclick="janelamodal('report/tra.php?idamostra=<?=$arrTRAAssociado["idamostra"]?>&unidadepadrao=<?=$arrTRAAssociado["idunidade"]?>')"></i>
					</td>
					<td>
...
function resetNucleo(){
	...
	$("[name=_1_"+CB.acao+"_amostra_rotulonucleotipo],[name=_1_"+CB.acao+"_amostra_idade],[name=_1_"+CB.acao+"_amostra_tipoidade],[name=_1_"+CB.acao+"_amostra_nucleoamostra],[name=_1_"+CB.acao+"_amostra_lote],[name=_1_"+CB.acao+"_amostra_granja],[name=_1_"+CB.acao+"_amostra_regoficial],[name=_1_"+CB.acao+"_amostra_nsvo],[name=_1_"+CB.acao+"_amostra_cidade],[name=_1_"+CB.acao+"_amostra_uf],[name=_1_"+CB.acao+"_amostra_cpfcnpj]").val("");
