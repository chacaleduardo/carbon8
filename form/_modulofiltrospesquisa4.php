...
//die("psqfull:".$_arrModConf["psqfull"]."iget:".$iget);
  ...
  foreach ($ARRRESULTADOSModulo as $key => $value){
    $temp[$value[2]["ord"] . "oldkey" . $key] = $value; //concatenate something unique to make sure two equal weights don't overwrite each other
	   ...
	   //print_r($_arrhlcond[$_modulo]);
    	...
    	while ($_i < $_numcampos) {
    		...
    		if($_arrpagpsq[$_arrModConf["tab"]][$_nomecol]["visres"] == 'Y' or $_mostrartodascolunas==true){
    			...
    			if(!empty($valuetd) and ($iNav==o1 or $iNav==2)){
