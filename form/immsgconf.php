...
function getSetor()
{
    ...
    $sql = "select s.idimgrupo,s.grupo
        ...
        where s.status='ATIVO' 
         ...
         " . getidempresa('s.idempresa', 'immsgconf') . "  order by s.grupo";
...
/* *********************************** Tabelas dos modulos destino configurados ****************************** */
function jsonModulosPais()
{
    $sql = "SELECT modulo as id, modulo as obj from " . _DBCARBON . "._modulo where tipo = 'DROP' and status = 'ATIVO'
        UNION ALL
            SELECT modulo as id, modulo as obj from " . _DBCARBON . "._modulo where tipo = 'SNIPPET' and status = 'ATIVO'
        UNION ALL
            SELECT snippet as id, snippet as obj  from " . _DBCARBON . "._snippet where modulo !='' and status = 'ATIVO'";
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <table>
                ...
                <tr>
                    <td align="right">Título Curto:</td>
                    <td>
                ...
                <div class="col-md-6">
                    <table>
                        <tr>
                            ...
                            <td nowrap="nowrap">
                                <input <?= $readonly ?> type="text" name="_1_<?= $_acao ?>_immsgconf_modulo" cbvalue="<?= $_1_u_immsgconf_modulo ?>" value="<?= $arrmodulo[$_1_u_immsgconf_modulo]["modulo"] ?>" style="width: 20em;">
                            </td>
                            <td align="right">Tabela:</td>
                            <td nowrap="nowrap">
                                <input <?= $readonly ?> type="text" name="_1_<?= $_acao ?>_immsgconf_tabela" cbvalue="<?= $_1_u_immsgconf_tabela ?>" value="<?= $_1_u_immsgconf_tabela ?>" style="width: 20em;">
                            </td>
                            <td>
                            ...
                            <td>
                                <select name="_1_<?= $_acao ?>_immsgconf_tipo" class="size15" vnulo onChange="CB.post();">>
                                    <? fillselect("select 'T','Tarefa' union select 'N','Notificação'", $_1_u_immsgconf_tipo); ?>
                                </select>
                            </td>
                            <td>
                                <? if ($_1_u_immsgconf_modulo) { ?>
                                    <a class="fa fa-bars pointer hoverazul" title="Módulo" onclick="janelamodal('?_modulo=_modulo&_acao=u&idmodulo=<?= $arrmodulo[$_1_u_immsgconf_modulo]['idmodulo'] ?>')"></a>
                                <? } ?>
                            </td>
                            <td nowrap align="right">Sub Tipo:</td>
                            <td>
                                <?
                                if ($_1_u_immsgconf_tipo == 'T') {
                                ?>

                                    <select name="_1_<?= $_acao ?>_immsgconf_ideventotipo" class="size15" vnulo>
                                        <? fillselect("select ideventotipo, eventotipo from eventotipo order by eventotipo ", $_1_u_immsgconf_ideventotipo); ?>
                                    </select>
                                <?
                                } else {
                                ?> <?
                                    $sqst = "select idimmsgconfplataforma, idplataforma from immsgconfplataforma where idimmsgconf = " . $_1_u_immsgconf_idimmsgconf . "";
                                    $rst = d::b()->query($sqst) or die("Erro ao buscar configuracao dos inputs: " . mysqli_error(d::b()));
                                    $ifi = 0;
                                    while ($rowst = mysqli_fetch_assoc($rst)) {
                                        if ($rowst['idplataforma'] == 'browser') {
                                            $checkedbrowser = "checked='checked'";
                                            $idcheckedbrowser = $rowst['idimmsgconfplataforma'];
                                        }
                                        if ($rowst['idplataforma'] == 'mbrowser') {
                                            $checkedmbrowser = "checked='checked'";
                                            $idcheckedmbrowser = $rowst['idimmsgconfplataforma'];
                                        }
                                        if ($rowst['idplataforma'] == 'app') {
                                            $checkedapp = "checked='checked'";
                                            $idcheckedapp = $rowst['idimmsgconfplataforma'];
                                        }
                                    }
                        ?>
                                    <span class="input-group-addon">
                                        <input title="Exibir" id="browser" type="checkbox" aria-label="..." value="browser" onchange="immsgconfplataformachecked(this,<?= $_1_u_immsgconf_idimmsgconf ?>,<?= $idcheckedbrowser; ?>)" <?= $checkedbrowser ?>> Browser/App
                                    </span>
                                    <span class="input-group-addon">
                                        <input title="Exibir" id="mbrowser" type="checkbox" aria-label="..." value="voip" onchange="immsgconfplataformachecked(this,<?= $_1_u_immsgconf_idimmsgconf ?>,<?= $idcheckedmbrowser; ?>)" <?= $checkedmbrowser ?>> Voip
                                    </span>
                                    <input id="idimmsgconf" name="_1_<?= $_acao ?>_immsgconf_ideventotipo" type="hidden" value="<?= $_1_u_immsgconf_ideventotipo ?>" readonly='readonly'>
                                <? }
                                ?>

                            </td>
                            <? if ($_1_u_immsgconf_tipo == 'N') { ?>
                                <td nowrap align="right">
                                    Aparece em:
                                </td>
                                <td>
                                    <input class="size15" name="_1_<?= $_acao ?>_immsgconf_localizacao" cbvalue="<?= $_1_u_immsgconf_localizacao ?>" value="<?= $_1_u_immsgconf_localizacao ?>">
                                </td>
                            <? } ?>
                        </tr>
                        <script>
                            function immsgconfplataformachecked(vthis, idimmsgconf, idimmsgconfplataforma) {
                                var Vval = $(vthis).val();
                                if (vthis.checked) {
                                    CB.post({
                                        objetos: "_x_i_immsgconfplataforma_idimmsgconf=" + idimmsgconf + "&_x_i_immsgconfplataforma_idplataforma=" + Vval,
                                        parcial: false,
                                        refresh: true
                                    });
                                } else {
                                    CB.post({
                                        objetos: "_x_d_immsgconfplataforma_idimmsgconfplataforma=" + idimmsgconfplataforma,
                                        parcial: false,
                                        refresh: true
                                    });
                                }
                        ...
                        <tr>
                            ...
                            <td colspan="4">
                                <div id="diveditor2" class="diveditor2" onkeypress="pageStateChanged=true;" style="width: 100%;height: 200px;"><?= $_1_u_immsgconf_mensagem ?></div>

                                <textarea style="display: none;" name="_1_<?= $_acao ?>_immsgconf_mensagem"><?= $_1_u_immsgconf_mensagem ?></textarea>
                    ...
                    ?>
                        <table>
                            <tr>
                                ...
                                <td class="nowrap">
                                    <input name="_1_<?= $_acao ?>_immsgconf_expiraem" type="text" class="size5" value="<?= $_1_u_immsgconf_expiraem ?>"> Dias após envio.
                                </td>
                            </tr>
                            <tr>
...
if ($_1_u_immsgconf_modulo || $_1_u_immsgconf_tabela) {
?>
    <div class="col-md-5">
        <div class="panel panel-default">
            ...
            <div class="panel-body" id="localInfo1">
                <span class="input-group-addon">
                    <?
                    if ($_1_u_immsgconf_somentecriador == 'Y') {
                        $check = 'checked="checked"';
                    } else {
                        $check = '';
                    }
                    ?>
                    Somente criador <input id="somentecriador" type="checkbox" value="<?= $_1_u_immsgconf_somentecriador ?>" onchange="alteraflag(this,'immsgconf')" <?= $check ?>>
                </span>
                <? if ($_1_u_immsgconf_somentecriador == 'N') { ?>
                    <table>
                        <tr>
    ...
    <?
    if (!empty($_1_u_immsgconf_tabela)) {
        $sqlm = "select tabela as tab from immsgconf where tabela ='" . $_1_u_immsgconf_tabela . "'";
    ...
    ?>
        ...
        <?
            ...
            $sqlin = "INSERT INTO immsgconffiltros
                            ...
                            (select 
                            " . cb::idempresa() . "," . $_1_u_immsgconf_idimmsgconf . ", col,'" . $_SESSION["SESSAO"]["USUARIO"] . "',NOW(),'" . $_SESSION["SESSAO"]["USUARIO"] . "',NOW()
            ...
            if (!empty($_1_u_immsgconf_tabela)) {
                $where = "tc.tab='" . $_1_u_immsgconf_tabela . "'";
            } else {
                $where = "f.modulo='" . $_1_u_immsgconf_modulo . "'";
            }
            // if(m.modulo <> '', m.modulo = ic.modulo, tc.tabela = tc.tab) 

            $sql = "select mf.*,tc.datatype,tc.col as colf,if(length(tc.rotcurto) > 0, tc.rotcurto, tc.col) as rotcurto,tc.dropsql
                from " . _DBCARBON . "._modulo f 
                        right join " . _DBCARBON . "._mtotabcol tc on(tc.tab=f.tab)
                        join immsgconffiltros mf on( mf.idimmsgconf =" . $_1_u_immsgconf_idimmsgconf . " and tc.col = mf.col)                         
                where " . $where . " group by tc.tab, tc.col order by  colf";
                                        ...
                                        <?
                                            ...
                                            <td>
                                                <select name="_<?= $l ?>_u_immsgconffiltros_sinal" vnulo>
                                                    <option value=""></option>
                                                    <? fillselect("select '=','Igual' union select '>','Maior que' union select '>=','Maior igual a'  union select '<','Menor que' union select '<=','Menor igual a' 
                                  union select 'like','Like'", $row["sinal"]); ?>
                                                </select>
                                            </td>
                                            <td class="nowrap">
                                                <select class="size6" name="_<?= $l ?>_u_immsgconffiltros_valor">
                                                    <option value=""></option>
                                                    <? fillselect("select 'now','Hoje' union select 'mais','Hoje +' union  select  'menos','Hoje -'", $row["valor"]); ?>
                                                </select>
                                                <input class="size5" placeholder="Dia" name="_<?= $l ?>_u_immsgconffiltros_nowdias" type="text" value="<?= $row["nowdias"] ?>"> Dias
                                            </td>
                                        <?
                                        } elseif ($row["datatype"] == 'time') { ?>
                                            <td>
                                                <select name="_<?= $l ?>_u_immsgconffiltros_sinal" vnulo>
                                                    <option value="between">Em até</option>
                                                </select>
                                            </td>
                                            <td class="nowrap">
                                                <select class="" name="_<?= $l ?>_u_immsgconffiltros_valor">
                                                    <option value=""></option>
                                                    <? fillselect("SELECT 'date_add(now(), INTERVAL 5 MINUTE)','5 minutos'
                                            union SELECT 'date_add(now(), INTERVAL 10 MINUTE)','10 minutos'
                                            union SELECT 'date_add(now(), INTERVAL 15 MINUTE)','15 minutos'
                                            union  SELECT  'date_add(now(), INTERVAL 45 MINUTE)','45 minutos'
                                            union  SELECT  'date_add(now(), INTERVAL 1 HOUR)','1 hora'
                                            union  SELECT  'date_add(now(), INTERVAL 2 HOUR)','2 horas'
                                            union  SELECT  'date_add(now(), INTERVAL 3 HOUR)','3 horas'
                                            union  SELECT  'date_add(now(), INTERVAL 4 HOUR)','4 horas'
                                            union  SELECT  'date_add(now(), INTERVAL 5 HOUR)','5 horas'
                                            union  SELECT  'date_add(now(), INTERVAL 6 HOUR)','6 horas'", $row["valor"]); ?>
                                                </select>
                                            </td>
                                        <? } else {
                                        ?>
                                            <td>
                                                <select name="_<?= $l ?>_u_immsgconffiltros_sinal" vnulo>
                                                    <? fillselect("select '=','Igual' union select '>','Maior que' union select '>=','Maior igual que'  union select '<','Menor que' union select '<=','Menor igual que' 
                                  union select 'like','Like' union select 'is','Is' union select '!=','Diferente' union select 'sql','SQL'", $row["sinal"]); ?>
                                                </select>
                                            </td>
                                            <td>
    ...
    if ($_1_u_immsgconf_status != 'ATIVO') {
                                ...
                                <table class="planilha grade compacto">
                                    ...
                                    while ($rowx = mysqli_fetch_assoc($resx)) {
                                        ...
                                        if ($row['tipo'] == "A") {
                                            ...
                                            $cl = " AND NOT EXISTS (SELECT 1 FROM carrimbo ca where ca.idpessoa = p.idpessoa  " . getidempresa('ca.idempresa', 'carrimbo') . " and ca.idobjeto = '" . $rowx['idpk'] . "' and status = 'ATIVO' and tipoobjeto = '" . $row['modulo'] . "' ) ";
                                        }
        ...
        $sqllog = "select e.idmodulo, e.modulo,mf.col,l.status as statuslog, l.criadoem,e.evento,if(r.visualizado, 'Sim','-') as visualizado,p.idpessoa,p.nomecurto, e.idevento
			FROM
            immsgconflog l
            JOIN eventoobj eo on eo.idevento = l.idimmsgbody and eo.objeto = 'immsgconf' and eo.idobjeto = l.idimmsgconf
            JOIN evento e on e.idevento = l.idimmsgbody
			join fluxostatuspessoa r on e.idevento = r.idevento AND r.modulo = 'evento'
                    ...
                    <div class="panel-body">
                        <table class="planilha grade compacto">
                            ...
                            while ($ro = mysqli_fetch_assoc($r)) {
                                $link = "?_modulo=" . $ro['modulo'] . "&_acao=u&" . $ro['col'] . "=" . $ro['idmodulo'];
                            ?>
                                <tr>
                                    <td>
                                        <a class="pointer" onclick="janelamodal('<?= $link ?>')"><?= $ro['idmodulo'] ?></a>
                                    </td>
                                    <td><?= $ro['modulo'] ?></td>
                                    <td><?= $ro['nomecurto'] ?></td>
                                    <td><?= dmahms($ro['criadoem']) ?></td>
                                    <td><?= $ro['statuslog'] ?></td>
                                    <td><a onclick="javascript:modalEvento(this,'?_modulo=evento&amp;_acao=u&amp;idevento=<?= $ro['idevento'] ?>')"><?= $ro['idevento'] ?></a></td>
...
<?
$and = '';
if (!empty($_1_u_immsgconf_idimmsgconf)) { // trocar p/ cada tela a tabela e o id da tabela
    $_idModuloParaAssinatura = $_1_u_immsgconf_idimmsgconf; // trocar p/ cada tela o id da tabela
    require 'viewAssinaturas.php';
}
$tabaud = "immsgconf"; //pegar a tabela do criado/alterado em antigo
...
<script>
    function modalEvento(o, url) {
        $(o).parent().removeClass("naoVisualizado");
        CB.modal({
            url: url + "&_modo=form",
            header: "Evento"
        })
    ...
    function alteraflag(vthis, tbl) {
        var strval = $(vthis).val();
        objpost = "_1_u_" + tbl + "_" + $(vthis).attr("id");
    ...
    jsonModulosPais = <?= jsonModulosPais() ?>;
    $(":input[name=_1_" + CB.acao + "_immsgconf_localizacao]").autocomplete({
        source: jsonModulosPais,
        delay: 0,
        create: function() {
            $(this).data('ui-autocomplete')._renderItem = function(ul, item) {
                vitem = item.value;
