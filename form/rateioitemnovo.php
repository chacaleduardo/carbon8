...
JOIN `rateioitemdest` `rid` ON (`rid`.`idrateioitem` = `ri`.`idrateioitem` and rid.status='".$status."')
...
$vw8despesascobradas =  str_replace( "rid.status='PENDENTE'",  "rid.status='COBRADO'",  $vw8despesas);
        ...
        <tr>
            ...
            <td colspan="10">
                <select name="status"  id="status" >
                    <?fillselect("select 'PENDENTE','Pendente' union select 'COBRADO','Cobrado'",$status);?>
                </select>	
            </td>
	    </tr>  
	    </table>	
...
$sqlot="select sum(valor) as valor from(
    SELECT cp.idcontapagar,cp.valor
                    FROM contapagar cp join nf n on(n.idnf=cp.idobjeto and cp.tipoobjeto = 'nf' and n.tiponf='O')
               ...
               SELECT cp.idcontapagar,cp.valor
                    ...
                    and cp.tipo='D'
                    and cp.status != 'INATIVO' 
                    and exists (select 1 from contapagaritem i join nf n on(n.idnf=i.idobjetoorigem and n.tiponf='O') where i.tipoobjetoorigem = 'nf' and i.idcontapagar=cp.idcontapagar)
                    group by cp.idcontapagar order by idcontapagar) as u";

                    $resot =  d::b()->query($sqlot) or die("Falha ao pesquisar dados de Faturamento (P): " .mysqli_error(d::b()). "<p>SQL: $sqlot");

                    $rowOt=mysqli_fetch_assoc($resot);
            ?>
            
            <tr style="font-size:9px">
                <td style="font-size:9px !important">Cobrado:</td>
                <td style="font-size:9px !important" class='totalempresa nowrap' align="right"></td>
                <td style="font-size:9px !important" class='totalempresa nowrap' align="right">R$ <?=number_format(tratanumero((double)$rowFat['valor']), 2, ',', '.')?></td>
            </tr>
            <tr style="font-size:9px">
                <td style="font-size:9px !important">Não Discriminado:</td>
                <td style="font-size:9px !important" class='totalempresa nowrap' align="right"></td>
                <td style="font-size:9px !important" class='totalempresa nowrap' align="right">R$ <?=number_format(tratanumero((double)$rowOt['valor']), 2, ',', '.')?></td>
            </tr>
        </table>
...
function pesquisar(vthis){
   ...
   $(vthis).html('<span class="fa fa-spinner fa-pulse"></span>');
    ...
    var str="idempresa="+idempresa+"&idagencia="+idagencia+"&vencimento_1="+vencimento_1+"&vencimento_2="+vencimento_2+"&idsgdepartamento="+idsgdepartamento+"&_idcontaitem="+idcontaitem+"&pesquisa="+pesquisa+"&status="+status ;
    CB.go(str);
}
