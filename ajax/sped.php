...
class sped{  
    ...
    public function bloco_0($idempresa){
  	...
  	$sqld="select n.idempresa as id,n.xmlret as xml,'P' as tipo
			...
			UNION ALL
				select n.idempresa as id,l.xml,'S' as tipo   
	    	...
	    	$qtdnf = mysql_num_rows($res);
	    		...
	    		$A170=0;
				   ...
				   /* 					
			    	...
			    	$qtda=$qtda+1;
			    	$nitems++;
					$_SESSION['A170'][$A170]="|A170|".$nitems."|777||".$VL_ITEM."||||01|".$VL_BC_I."|0,65|".$VL_PIS_I."|01|".$VL_BC_I."|3,00|".$VL_COFINS_I."|2606||\n";
    ...
    public function bloco_C($idempresa){
    	...
    	$_SESSION['C001']="|C001|".$qtdc001."|\n";
      	...
      	if($qtdnf>0){
	    	...
	    	$sql="select n.idnf,n.idempresa as id,n.xmlret as xml
				from nf n  join natop o on(o.idnatop=n.idnatop and o.natop  like('%VENDA%') and o.natop not like('%DEVOLU%'))
				where n.sped = 'Y'
				and n.status = 'CONCLUIDO'
				and n.envionfe = 'CONCLUIDA'
				and n.tiponf ='V'
                                and n.idempresa = ".$idempresa."
				and n.dtemissao between '".$this->dtInicio." 00:00:00' and '".$this->dtFim." 23:59:00'";
	    	$res=mysql_query($sql) or die($sql." erro ao buscar informações do bloco C001".mysql_error());
	    	$qtdnf = mysql_num_rows($res);
					...
					if($vCSTcofins=='06'){
					  ...
					  $vprodFDesc=number_format($vprodFDesc, 2, ',','');
                                        ...
                                        if($vchNFe!=$vchNFeant){
                                             ...
                                             * C100 NF
                                             */
                                            $_SESSION['C100'][$C100]=("|C100|".$vtpNF."|0|".$vCNPJ."|".$vmod."|00|".$vserie."|".$vnNF."|".$vchNFe."|".$vdEmi."||".$vlrOper."|1|".$vprodFDesc."||".$vc100Prod."|".$vmodFrete."|".$vvFrete."|".$vvSeg."|".$vvOutro."|".$vvBC ."|".$vvtICMS."|||".$vvIPI."|".$nvpis."|".$nvcofins."|||\n");
                                            /*
                                            * O C100 E A NOTA FISCAL 
                                            * */
                                             echo($_SESSION['C100'][$C100]);
     ...
     * */
 	public function bloco_M(){
        ...
        $bcpiscofins=$this->vlrbcpiscofins;
     	...
     	$VL_PIS=$bcpiscofins*0.0065;     	     	
     	$DIF_PIS=$this->vlrRetpis-$VL_PIS;
     	if($DIF_PIS<0){		
			$DIF_PIS= abs($DIF_PIS);
		}
     	$VL_COFINS=$bcpiscofins*0.03;     	     	
        ...
        $this->vlrTotal06=number_format($this->vlrTotal06, 2, ',','');
     	...
     	$_SESSION['M210']="|M210|51|".$TOTAL_NF."|".$TOTAL_VBPC."|0|0|".$TOTAL_VBPC."|0,65|0||".$VL_PIS."|0|0|||".$VL_PIS."|\n";
     	echo($_SESSION['M210']);
        
        $_SESSION['M400']="|M400|06|".$this->vlrTotal06."|||\n";
     	echo($_SESSION['M400']);
        
        $_SESSION['M410']="|M410|107|".$this->vlrTotal06."|||\n";
     	echo($_SESSION['M410']);
     	...
     	$_SESSION['M610']="|M610|51|".$TOTAL_NF."|".$TOTAL_VBPC."|0|0|".$TOTAL_VBPC."|3|0||".$VL_COFINS."|0|0|||".$VL_COFINS."|\n";
  
     	echo($_SESSION['M610']);  
        
        $_SESSION['M800']="|M800|06|".$this->vlrTotal06."|||\n";
     	echo($_SESSION['M800']);
        
        $_SESSION['M810']="|M810|107|".$this->vlrTotal06."|||\n";
     	echo($_SESSION['M810']);        
