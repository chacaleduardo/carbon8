...
class sped{  
    ...
    public function bloco_0($idempresa){
    	$sql="select idempresa as id,concat ('|0000|014|0|".$this->inicio."|".$this->fim."|',ifnull(e.razaosocial,''),'|',
		ifnull(e.cnpj,''),'||',ifnull(e.uf,''),'|',ifnull(e.inscestadual,''),
		'|',ifnull(e.cmun,''),'|',ifnull(e.inscricaoMunicipalPrestador,''),
		'||B|0|') as inf
		from empresa e where idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"];
		...
		*/
    	...
    	$sqld="select idempresa as id,xmlret as xml,tiponf,faticms,idnf,nnfe
				from nf 
				where sped = 'Y'
				and tiponf ='V' 
			 	and status in ('CONCLUIDO','ENVIADO','ENVIAR')
		...
		UNION ALL
		select idempresa as id,xmlret as xml,tiponf,faticms,idnf,nnfe
				from nf 
				where sped = 'Y'
				and tiponf ='C'
...
/*			
    	...
    	*/
    	 ...
    	 $sqlt="select idempresa as id,xmlret as xml,tiponf,faticms,consumo,imobilizado,idnf,nnfe
				from nf
				where sped = 'Y'
				and tiponf = 'T'
			 	and status in ('DIVERGENCIA','RECEBIDO','CONCLUIDO')
    	 ...
    	 */
    	  ...
    	  if($this->inventario=='Y'){
    	  	 $sql1="select distinct(s.un) as un,m.descr  from spedh010 s join medida m on(m.un=s.un) 
						where idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." 
							and ano='".$this->ano."' 
							and status='ATIVO' 
						union 
						select un,descr	from medida where un in ('UN','FR','ML','GR')";
    	 ...
    	 $res1=mysql_query($sql1) or die("Erro ao pesquisar unidades");
    	 $r0190=1;
    	 ...
    	 * */
    	 ...
    	 $_SESSION['0200'][1]="|0200|777|Unidade|||UN|99|||00||||";
    	 echo($_SESSION['0200'][1]."\n");
   	 
    	 //produtos na fabrica de outros produtos
       	 $sqlpprod="select upper(p.un) as un,rtrim(ltrim(p.codprodserv)) as codprodserv,rtrim(ltrim(ifnull(p.descrcurta,p.descr))) as descr,p.venda,p.ncm
			...
			and w.fabricacao between  '".$this->dtInicio."' and '".$this->dtFim."'
			 group by p.codprodserv,pp.codprodserv";
            ...
            if($rowprod['venda']=='Y'){
       	 	...
       	 	if(in_array($rowprod['codprodserv'], $arrcodprod)) {
       	 		...
       	 		$_SESSION['0200'][$r0200]="|0200|".$rowprod['codprodserv']."|".$rowprod['descr']."|||".$rowprod['un']."|".$tipoativ."|".$rowprod['ncm']."||29||||";
    	 ...
    	 $sqllt="select sum(c.qtdd) as qtd,upper(pp.un) as un,rtrim(ltrim(p.codprodserv)) as codprodserv,
                        pp.codprodserv as codprodservprod,rtrim(ltrim(ifnull(pp.descrcurta,pp.descr))) as descr,pp.venda,pp.ncm
			from lotecons c,lote l,prodserv p, lote w,prodserv pp,unidade u
			where  (p.venda = 'Y' or  p.fabricado = 'Y')
                        and p.idprodserv = l.idprodserv
			and l.idlote = c.idlote
			...
			and w.idprodserv = pp.idprodserv
			and w.fabricacao between '".$this->dtInicio."' and '".$this->dtFim."'
			 group by p.codprodserv,pp.codprodserv order by codprodservprod";
    	 ...
    	 while($rowlt=mysql_fetch_assoc($reslt)){
            ...
            if(in_array($rowlt['codprodservprod'], $arrcodprod)) {
                    echo "";
                    ...
                    $r0200=$r0200+1;
                    $l0000=$l0000+1;
                    $arrcodprod[$c]=$rowlt['codprodservprod']; 	 	

                    $_SESSION['0200'][$r0200]="|0200|".$rowlt['codprodservprod']."|".$rowlt['descr']."|||".$rowlt['un']."|".$tipoativ."|".$rowlt['ncm']."||29||||";
                    echo($_SESSION['0200'][$r0200]."\n");
            }
            
            if($passa==1 and $rowlt['venda']=='Y'){
                    $r0210=$r0210+1;
                    $l0000=$l0000+1;
                    $_SESSION['0210'][$r0210]="|0210|".$rowlt['codprodserv']."|".number_format($rowlt['qtd'], 2, ',','')."|0|";
    	 ...
    	 if($this->inventario=='Y'){
			...
			/*
			 $sql1="select p.un,rtrim(ltrim(p.codprodserv)) as codprodserv,rtrim(ltrim(ifnull(p.descrcurta,p.descr))) as descr,'N' as venda,'' as ncm
					from prodserv p,lote l
					...
					UNION
				select upper(p.un) as un,rtrim(ltrim(p.codprodserv)) as codprodserv,rtrim(ltrim(ifnull(p.descrcurta,p.descr))) as descr,p.venda,p.ncm
				       from prodserv p,lote l,prodvalor v
					   ...
					   */
	    	 $sql1="select p.un,rtrim(ltrim(p.codprodserv)) as codprodserv,rtrim(ltrim(ifnull(p.descrcurta,p.descr))) as descr,'N' as venda,'' as ncm
			 from prodserv p join spedh010 s on(s.idprodserv=p.idprodserv)
			 where  s.idempresa = ".$idempresa."
			 and s.ano ='".$this->ano."' and s.status='ATIVO'
			 group by p.idprodserv";
	    	 $res1=mysql_query($sql1) or die("Erro ao pesquisar unidades");
	    	 //$r0200=1;
	    	 while($row1=mysql_fetch_assoc($res1)){
                    ...
                    if(in_array($row1['codprodserv'], $arrcodprod)) {
                           ...
                           $_SESSION['0200'][$r0200]="|0200|".$row1['codprodserv']."|".$row1['descr']."|||".$row1['un']."|".$tipoativ."|".$row1['ncm']."||29||||";
                           echo($_SESSION['0200'][$r0200]."\n");
                    }
    ...
    public function bloco_C($idempresa){
    	...
    	$sql="select idempresa as id,xmlret as xml,tiponf,faticms,consumo,imobilizado,idnf,nnfe,DATE_FORMAT(prazo,'%d%m%Y') as prazo, 
				...
				and dtemissao between '".$this->dtInicio." 00:00:00' and '".$this->dtFim." 23:59:00'
		UNION ALL 
		select idempresa as id,xmlret as xml,tiponf,faticms,consumo,imobilizado,idnf,nnfe,DATE_FORMAT(prazo,'%d%m%Y') as prazo, 
				case icmscpl when 0.00 then null when icmscpl then icmscpl end as icmscpl
				from nf 
				where sped = 'Y'
				and tiponf ='C'
    	...
    	$qtdnf = mysql_num_rows($res);
      		...
      		while ($row=mysql_fetch_assoc($res)){
				...
				$vDesc= $ICMSTot->getElementsByTagName("vDesc")->item(0);//Totais	
	      		...
	      		*/
	      		...
	      		if(($row['consumo']=="Y" or $row['imobilizado']=='Y') and ($row['tiponf'] !='V')){
    			 ...
    			 /*  
                       ...
                       // }
                        ...
                        if($vserie=='890'){//ser for serie 890 lançar como nota fiscal
                            $vmod='01';
                            $vchNFe='';
                        }
					...
					if($ICMSSN201!=null){
						...
						<ICMS>
							<ICMSSN201>
								...
								<vICMSST>1.04</vICMSST>
								<pCredSN>1.60</pCredSN>
								<vCredICMSSN>0.12</vCredICMSSN>
							</ICMSSN201>
						</ICMS>
						*/
				...
				$vProd = $prod->getElementsByTagName("vProd")->item(0);
				    ...
				    // Não é permitido aproveitamento de crédito para material de USO e CONSUMO!
				    if($row['consumo']=="Y" or $row['imobilizado']=='Y'){
    		...
    		if($row['tiponf']=='V'){
                    $sqli="select cst as st,cfop,aliqicms,round(sum((total)+frete),2) as vl_opr,sum(basecalc) as vl_bc_icms,
                            sum(valicms) as vl_icms,(sum((total)+frete)-sum(basecalc)) as vl_red_bc,sum(valipi) as vl_ipi
    			...
    			while($rowi=mysql_fetch_assoc($resi)){
    				...
    				if($row['tiponf']!='V'){
    					...
    					if(($vCFOP=='5101' or $vCFOP=='5102') and $row['faticms']=='Y'){
		    				...
		    				$vCFOP='1653';
		    			}elseif($vCFOP=='5929' and ( $rowi['st']==00 or $rowi['st']==20) and $row['imobilizado']=='Y'){	//5929 = vai ser 1551, quando o produto for tributado, CST: 000 / 020	    				
		    				$vCFOP='1551';
		    			}elseif($vCFOP=='5929'  and $row['imobilizado']=='Y'){//5929 = vai ser 1406, quando o produto for Substituição Tributaria, CST 060 / 070 / 090 
		    				$vCFOP='1406';
	...
	public function bloco_D($idempresa){
		...
		$_SESSION['D001']="|D001|".$qtdd001."|\n";
     	...
     	//$sql="select idempresa as id,xml from xmlnf where mes = 'fevereiro' and tipo = 'P' and idempresa=".$idempresa;
     	$sql="select idempresa as id,xmlret as xml,tiponf,faticms,consumo,imobilizado,idnf
				from nf
				where sped = 'Y'
				and tiponf = 'T'
			 	and status in ('DIVERGENCIA','RECEBIDO','CONCLUIDO')
  ...
  */
     		...
     		$_SESSION['D100'][$D100]="|D100|0|1|".$vCNPJ."|08|00|||".$vnCT."||".$vdEmi."|".$vdRecbto."|0|".$vchCTe."|".$vvTPrest."|0,00|9|".$vvTPrest."|".$vvBCicms."|".$vvICMS."|".$VL_RED_BC."||0511|".$vcMunIni."|".$vcMunFim."|\n";
    ...
    //INVENTaRIO FiSICO
 	public function bloco_H($idempresa){
 		...
 		/*	
 			$sqlv="select sum(vlr) as vlrf from(						
				 ...
				 */
			$sqlv="select sum(vlritemir) as vlrf from spedh010 where idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." and ano='".$this->ano."' and status='ATIVO'";
 			...
 			$H=$H+1;
 			$_SESSION['H005']="|H005|".$this->fim."|".$vlrf."|01|\n"; 			
 			echo($_SESSION['H005']);
 			/*
 			$sqlp="select  p.idprodserv,ROUND((sum(f.qtd/p.uncom)),2) as qtdf,ROUND(((sum(f.qtd)/p.uncom)*p.vlrcompra),2) as vlritem,rtrim(ltrim(p.codprodserv)) as codprodserv,p.vlrcompra,p.descr,p.un
					...
					group by p.idprodserv";
					*/
			$sqlp="select idprodserv,codprodserv,un,sum(qtd) as qtdf, round((sum(vlrcompra)/count(*)),2) as  vlrcompra
			, round((sum(vlritem)/count(*)),2) as vlritem,descr,  round((sum(vlritemir)/count(*)),2) as vlritemir
			 from spedh010 
			 where idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." 
			 and ano='".$this->ano."' 
			 and status='ATIVO' group by idprodserv";
 			...
 			while($rowp=mysql_fetch_assoc($resp)){
 				...
 				$_SESSION['H010'][$H10]="|H010|".$rowp['codprodserv']."|".$rowp['un']."|".number_format($rowp['qtdf'], 2, ',','')."|".number_format($rowp['vlrcompra'], 2, ',','')."|".number_format($rowp['vlritem'], 2, ',','')."|0||".$row['descr']."|0511|".number_format($rowp['vlritemir'], 2, ',','')."|\n";
     ...
     public function bloco_K(){
     	$sqlk="select w.idlote,upper(p.un) as un,rtrim(ltrim(p.codprodserv)) as codprodserv,rtrim(ltrim(p.descr)) as descr,replace(dma(w.fabricacao),'/','') as fabricacao,round(w.qtdprod) as qtdprod,p.venda
			from lote w,prodserv p,unidade u 
			where   p.venda = 'Y' and p.fabricado = 'Y'		
			and p.tipo = 'PRODUTO'
			and w.qtdprod > 0
			and u.idempresa = w.idempresa
			and u.idunidade =w.idunidade
			and u.idtipounidade in (5,3)
			and w.idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]."
     	...
     	echo($_SESSION['K100'][1]."\n");
     		...
     		while($rowk=mysql_fetch_assoc($resk)){
     			...
     			$K990=$K990+1;
     			$K230=$K230+1;
     			$_SESSION['K230'][$K230]="|K230|".$this->inicio."|".$this->fim."|".$rowk['idlote']."|".$rowk['codprodserv']."|".number_format($rowk['qtdprod'], 2, ',','')."|";
     			echo($_SESSION['K230'][$K230]."\n");
                    if($rowk['venda']=='Y'){
			$sqlk1="select round(sum(c.qtdd)) as qtd,upper(p.un) as un,rtrim(ltrim(p.codprodserv)) as codprodserv
			    ...
			    if($rowk1['qtd']>0 and $rowk1['qtd']!=""){
	     			...
	     			$K990=$K990+1;
	     			$K235=$K235+1;
	     			$_SESSION['K235'][$K235]="|K235|".$rowk['fabricacao']."|".$rowk1['codprodserv']."|".number_format($rowk1['qtd'], 2, ',','')."||";
...
$sped->bloco_0($_SESSION["SESSAO"]["IDEMPRESA"]); //Abertura do Arquivo Digital e Identificação da entidade Dados do Contabilista
$sped->bloco_B($_SESSION["SESSAO"]["IDEMPRESA"]);
$sped->bloco_C($_SESSION["SESSAO"]["IDEMPRESA"]); // Abertura do Bloco C Nota Fiscal Eletronica (codigo 55) 
$sped->bloco_D($_SESSION["SESSAO"]["IDEMPRESA"]);//DOCUMENTOS FISCAIS II - SERVIcOS (ICMS)
$sped->bloco_E();//APURAcaO DO ICMS E DO IPI
$sped->bloco_G();//CONTROLE DO CReDITO DE ICMS DO ATIVO PERMANENTE - CIAP - modelos 
$sped->bloco_H($_SESSION["SESSAO"]["IDEMPRESA"]);//INVENTaRIO FiSICO
$sped->bloco_K();//CONTROLE DA PRODUÇÃO E DO ESTOQUE
