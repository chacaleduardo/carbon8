...
function string2Slug($str){
	//  mb_convert_encoding($text, "HTML-ENTITIES", 'IS5976');
 
	
	$str =utf8_decode($str);
	echo $str;
	$nova = explode('Ãƒâ€œ', $str);
	
	echo $nova[1];
	die();
	$str = str_replace("U+00D3","O",$str);
...
/*	MAF: exemplo de profiler funcional para verifica��o de falhas de execu��o: http://queirozf.com/entries/xhprof-php-profiler-full-usage-example
    $XHPROF_ROOT = "/var/www/xhprof";
    include_once $XHPROF_ROOT . "/xhprof_lib/utils/xhprof_lib.php";
    include_once $XHPROF_ROOT . "/xhprof_lib/utils/xhprof_runs.php";
    // start profiling
...
require_once('../libs/NFe/DanfeNFePHP2.class.php');


$res=d::b()->query("SET NAMES 'utf8'") or die("erro ao buscar xml-sql=".$sql);
...
//   $row["xmlret"] = RemoveSpecialChar($row["xmlret"]);
   //$row["xmlret"] = utf8_html_entity_decode($row["xmlret"]);
   $row["xmlret"] = string2Slug((string)$row["xmlret"]);
   //$row["xmlret"] = utf8_html_entity_decode($row["xmlret"]);
	//$row["xmlret"] = string2Slug($row["xmlret"]);
	//echo $row["xmlret"];
   //$row["xmlret"] = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $row["xmlret"]);
   // die($row["xmlret"]);
   $docxml = $row["xmlret"];
//$docxml =$row["xmlret"];
  // echo($docxml);
