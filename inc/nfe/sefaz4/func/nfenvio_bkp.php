 ...
 * par $r: Cont�m informacoes de cada linha da nfpxmltreenova para cada tag
 */
	...
	if(!empty($r["vlrfixo"])){//possui valor fixo
		...
		*/
		...
		$sqladic="select convert(lpad(replace(l.partida,l.spartida,''),'3', '0')using latin1) as partida
				,l.exercicio 
				,dma(vencimento) as vencimento,Concat(f.rotulo,' (',f.volumeformula,' ',f.un,') ') as rotulo
		...
		if(!empty($rowadic['partida']) and !empty($rowadic['vencimento'])){
		    ...
		    $strret.="\n".$t."<infAdProd>".$rowadic['rotulo']."Part:".$rowadic['partida']."/".$rowadic['exercicio']." Venc: ".$vencimento."</infAdProd>";
...
function assina_envia_XML($sXML, $tagID){	
	...
	"razaosocial" => "LAUDO LABORATORIO AVICOLA UBERLANDIA LTDA",
	"siglaUF" => "MG",
	"cnpj" => "23259427000104",
	"schemes" => "PL_009_V4", 
	"versao" => "4.00",
...
*/
    ...
    $certificadoDigital = file_get_contents('../certs/17223991laudo.p12');


    // $tools = new Tools($configJson, Certificate::readPfx($certificadoDigital, '010787'));

    $tools = new NFePHP\NFe\Tools($configJson, NFePHP\Common\Certificate::readPfx($certificadoDigital, '010787'));
    
    //CONTIGENCIA
    //indica que estamos em modo de conting�ncia, se nada for passado estaremos em modo normal
   // $tools->contingency->load($contingency);
    
    try {
	$xmlAssinado = $tools->signNFe($sXML); // O conteúdo do XML assinado fica armazenado na variável $xmlAssinado
    } catch (\Exception $e) {
	//die($e->getMessage());
...
function ASSINATURA_ENVIA(){
   ...
   // $_SESSION["xml"] = str_replace(<vFrete>0.00</vFrete>','', $_SESSION["xml"]); 
    ...
    //Assinar o XML
    $_SESSION["xml"] = assina_envia_XML($xmtlutf,$_SESSION["tagassinatura"]);	
