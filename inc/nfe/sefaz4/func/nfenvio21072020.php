...
function montatag($inidnf,$t,$r){
	...
	if(!empty($r["vlrfixo"])){//possui valor fixo
	    ...
	    */
		...
		if($cst!="00" and $cst!="20" and $cst!="40" and $cst!="50" and $cst!="60" and $cst!="41" and $cst!="500" and $cst!="101" and $cst!="102"){
	...
	if($rowipi['CST']!='50' and $rowipi['CST']!='99'){
		$strret.="\n".$t."<IPINT>";
		...
		if($rowcst['piscst'] =='01' or $rowcst['piscst'] =='02'){
		    ...
		    $strret.="\n".$t."</PISAliq>";
		    $strret.="\n".$t."</PIS>";
		}elseif($rowcst['piscst'] =='99' or $rowcst['piscst'] =='49'){
		...
		//FIM TAG IMPOSTO
                if(!empty($rowv['pDevol']) and !empty($rowv['vIPIDevol'])){
                    $strret.="\n".$t."<impostoDevol>";
                    $strret.="\n".$t."<pDevol>".$rowv['pDevol']."</pDevol>";
                    $strret.="\n".$t."<IPI>";
                    $strret.=("\n".$t.chr(9)."<vIPIDevol>".$rowv['vIPIDevol']."</vIPIDevol>");
                    $strret.="\n".$t."<IPI>";                  
                    $strret.="\n".$t."</impostoDevol>";
                     /*
                <impostoDevol>
...
function assina_envia_XML($sXML, $tagID){	
    ...
    $_sql = "SELECT *,str_to_date(nfatualizacao,'%d/%m/%Y %h:%i:%s') as nfatt FROM empresa WHERE idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"];
...
function ASSINATURA_ENVIA(){
 ...
 $_SESSION["xml"] = str_replace('<CNPJ></CNPJ>','<idEstrangeiro/>', $_SESSION["xml"]);
