...
if($res and mysqli_num_rows($res) > 0){
	...
	while($row=mysql_fetch_assoc($res)){
            ...
            if(!empty($row['obslote'])){
              $_CONTEUDOIMPRESSAO .= "
              ^CF0,17
              ^FO80,180^FH\^FDData Lote: ".retira_acentos(dma($row['criadoem']))."^FS
              ^CF0,17
              ^FO80,200^FB430,2,,^FH\^FDLocal: ".retira_acentos($row['campo'])."^FS
              ^FO80,220^FB430,2,,^FH\^FDObs: ".retira_acentos($row['obslote'])."^FS
              ^XZ";
            }else{
              $_CONTEUDOIMPRESSAO .="
              ^CF0,17
