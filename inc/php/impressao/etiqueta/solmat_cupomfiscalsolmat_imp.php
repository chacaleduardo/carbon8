...
$qr=EtiquetaController::buscarInfosSolmatCupomFiscal( $idsolmat, 1337 ,'solmat' );
//die($sql);
if(count($qr) > 0){
    $aux = "";
    foreach($qr as $k => $row){
    ...
    $_CONTEUDOIMPRESSAO .= GS. 'L'. intLowHigh(0,2);
    $_CONTEUDOIMPRESSAO .= GS .'!'.chr(119);
    ...
    $_CONTEUDOIMPRESSAO .= ESC."d".chr(1); // Blank line (pular linha)
    $_CONTEUDOIMPRESSAO .= "Itens:\n";
    $_CONTEUDOIMPRESSAO .= $aux;
    $_CONTEUDOIMPRESSAO .= GS .'E'.chr(1);
    $_CONTEUDOIMPRESSAO .= GS .'!'.chr(1). "Destino:".strtr( $destino, $unwanted_array )."\n";
    $_CONTEUDOIMPRESSAO .= ESC .'!'.chr(0);
    $_CONTEUDOIMPRESSAO .= GS .'!'.chr(19);
    $_CONTEUDOIMPRESSAO .= GS. 'L'. intLowHigh(0,2);
    $_CONTEUDOIMPRESSAO .= $nomecurto."\n";
    $_CONTEUDOIMPRESSAO .= ESC .'!'.chr(0) ;
    $_CONTEUDOIMPRESSAO .= GS. 'L'. intLowHigh(0,2);
    $_CONTEUDOIMPRESSAO .= "Tipo:\n";
    $_CONTEUDOIMPRESSAO .= GS. 'W'. intLowHigh(1024,2);
    $_CONTEUDOIMPRESSAO .= GS. 'L'. intLowHigh(0,2);
    $_CONTEUDOIMPRESSAO .= GS. 'L'. intLowHigh(64,2);
    $_CONTEUDOIMPRESSAO .= GS .'!'.chr(16).$tipo."\n";
    $_CONTEUDOIMPRESSAO .= ESC .'!'.chr(0) ;
    $_CONTEUDOIMPRESSAO .= "Data de impressao:" .dmahms($now)."\n";
    $_CONTEUDOIMPRESSAO .= ESC."d".chr(1); // Blank line (pular linha)
    $_CONTEUDOIMPRESSAO .= GS . "V" . chr(65) . chr(3); // Cut (cortar papel)
    $_CONTEUDOIMPRESSAO .= ESC."@"; // Reset to defaults (Inicializa nova impressão)
