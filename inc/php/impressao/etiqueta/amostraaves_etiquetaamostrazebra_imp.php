    ...
    foreach($reslotetiqueta as $k => $rl) {
        ...
        foreach($res as $k1 => $row) {
            ...
            if($l==0){
                ...
                $_CONTEUDOIMPRESSAO.='^CF0,30^FO100,'.$altura.'^FD'.$Vstatusp.''.str_pad($row['idregistro'],6).' '.retira_acentos($row['tipoamostra']).'^FS';
                ...
                if(!empty($row['idregistroprovisorio'])){
                    $altura=$altura+30;
                    $_CONTEUDOIMPRESSAO.='^CF0,30^FO100,'.$altura.'^FD'.$row['idregistroprovisorio'].'^FS';
                    $altura=$altura+30;
                }
