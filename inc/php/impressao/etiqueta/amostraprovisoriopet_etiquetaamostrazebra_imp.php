    ...
    foreach($reslotetiqueta as $k => $rl) {
        ...
        foreach($res as $k1 => $row) {
            ...
            if($l==0){
                ...
                $_CONTEUDOIMPRESSAO.='^CF0,30^FO100,'.$altura.'^FD'.$Vstatusp.''.str_pad($row['idregistro'].'PET',9).' '.retira_acentos($row['tipoamostra']).' P-'.$row['idregistroprovisorio'].'^FS';
                ...
                if(!empty($row['idregistroprovisorio'])){
                    $altura=$altura+30;
                    $_CONTEUDOIMPRESSAO.='^CF0,25^FO100,'.$altura.'^FDP-'.$row['idregistroprovisorio'].'^FS';
                }
                */
