...
*/
    ...
    $s1="insert INTO rhevento(dataevento,idpessoa,status,idrhtipoevento,idempresa,criadopor,criadoem,alteradopor,alteradoem)
        (select '".$row['dataevento']."',".$row['idpessoa'].",'PENDENTE',(select idrhtipoevento from rhtipoevento where status='ATIVO' and flhtotais='Y'  ),".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
        where not exists (
    ...
    $s2="insert INTO rhevento(dataevento,idpessoa,status,idrhtipoevento,idempresa,criadopor,criadoem,alteradopor,alteradoem)
        (select '".$row['dataevento']."',".$row['idpessoa'].",'PENDENTE',(select idrhtipoevento from rhtipoevento where status='ATIVO' and flhtotaisajust='Y' ),".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
        where not exists (
    ...
    $s3="insert INTO rhevento(dataevento,situacao,idpessoa,status,idrhtipoevento,idempresa,criadopor,criadoem,alteradopor,alteradoem)
        (select '".$row['dataevento']."','P',".$row['idpessoa'].",'PENDENTE',(select idrhtipoevento from rhtipoevento where status='ATIVO' and flhext='Y' ),".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
        where not exists (
...
*/
    ...
    (select '".$row['dataevento']."',".$row['idpessoa'].",'PENDENTE',(select idrhtipoevento from rhtipoevento where status='ATIVO' and flhextcalc='Y' ),".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
    where not exists (
    ...
    $s3="insert INTO rhevento(dataevento,situacao,idpessoa,status,idrhtipoevento,idempresa,criadopor,criadoem,alteradopor,alteradoem)
    (select '".$row['dataevento']."','P',".$row['idpessoa'].",'PENDENTE',51,".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
    where not exists (
    ...
    // adicional noturno
    $s3="insert INTO rhevento(dataevento,idpessoa,status,idrhtipoevento,idempresa,criadopor,criadoem,alteradopor,alteradoem)
    (select '".$row['dataevento']."',".$row['idpessoa'].",'PENDENTE',428,".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
    where not exists (
...
// ATULIZAR SOMATORIO DE HORAS PARA MOSTRAR UM UNICO EVENTO
    ...
    while($row=mysqli_fetch_assoc($res1)){
        ...
        $s3="insert INTO rhevento(dataevento,idpessoa,valor,status,idrhtipoevento,idempresa,criadopor,criadoem,alteradopor,alteradoem)
                (select '".$row['dataeventos']."',".$row['idpessoa'].",'".$row['valor']."','PENDENTE',".$row['idrhtipoevento'].",".$row['idempresa'].",'ajustaponto',now(),'ajustaponto',now()
                    where not exists (
