...
class Minifier {
    ...
    private function minify_contents( $src_file, $run_minification = true )
    {
        ...
        if( !$this->source )
        {
            ...
            die("Minifier error: ".'Unable to retrieve the contents of '. $src_file . '.  Skipping at '. __LINE__ .' in '. basename( __FILE__ ));
    ...
    {        
        ...
        if( !is_dir( dirname( $new_file ) ) || !is_writeable( dirname( $new_file ) ) )
        {
            ...
            die(dirname( $new_file ) . ' is not writable.  Cannot create minified file: '.$new_file);
            return false;
        }
