...
$resini=d::b()->query($sini) or die(mysqli_error(d::b()) . " erro ao buscar o mdfe " . $sini . " erro db:" . $resini); 
...
function STATUSLOTE($inidmdfelote, $instatus){
    ...
    $sqllote = "update "._DBNFE.".mdfelote set recibo = '1',status = '".$instatus."', xml = '".$xml."', xmlret = '".$xmlret."' where idmdfelote = ".$inidmdfelote;
	...
	//	echo("certificado=".$arquivo);
		...
		try {
			...
			if ($cStat != '100') {
				echo($xMotivo);
				$statusmdfe='ERRO';
				$_SESSION["xmlret"] =$xmlAssinado;

				$sql="update  "._DBAPP.".mdfe 
				set statusmdfe='".$statusmdfe."',mensagem='".$xMotivo."'
				where  idmdfe = ".$_SESSION["idmdfe"];
				$ret = d::b()->query($sql) or die("Erro ao atualizar mdfe sql #1:".$sql);
				...
				$sql2="select concat('<?xml version=\"1.0\" encoding=\"UTF-8\"?><mdfeProc xmlns=\"http://www.portalfiscal.inf.br/mdfe\" versao=\"3.00\">','".$xml."','".$_SESSION["xmlprot"]."','</mdfeProc>') as xmlret";
				$retx = d::b()->query($sql2) or die("Erro ao buscar mdfe sql:".$sql2);

				$rowxml=mysqli_fetch_assoc($retx);

				$sql="update "._DBAPP.".mdfe 
				set statusmdfe='".$statusmdfe."',mensagem='".$xMotivo."',protocolo='".$nProt."'
				,xmlret='".$rowxml['xmlret']."' 			
				where  idmdfe = ".$_SESSION["idmdfe"];
				$ret = d::b()->query($sql) or die("Erro ao atualizar mdfe sql #2:".$sql);

				$_SESSION["xmlret"] =$rowxml['xmlret'];
				echo($xMotivo);
