var Chat = function(){
	...
	self.init=function(){
		...
		self.oSideBarC=					self.oChatContainer.find(".sideBarC");

		self.oChatPainelContatos=				$("#cbChatPainelContatos");
		self.oChatPainelContatosC=				$("#cbChatPainelContatosC");
		...
		self.oGrupoContatosOnlineC=				$("#cbChatContatosOnlineC");
		...
		self.getContatos(function(inContatos){
			...
			$.each(inContatos, function(i,contato){
				...
				if (contato.tipo == 'chat'){
					...
					self.jcContatos[contato.objetocontato+"_"+contato.idcontato]=contato;
					
					if(self.oSideBar.find("#contato_"+contato.objetocontato+"_"+contato.idcontato).length<=0){
						var tmpContatoC = self.montaContato(contato);
						self.oGrupoContatosOnlineC.append(tmpContatoC);
					}
...
//		alert("Em manutenção. Aguarde!");	
	...
	self.pesquisarContatoC=function(inObj){
		$oSearch=$(inObj);
		strSearch=$oSearch.val();
		if(strSearch.trim().length<=0){
			$(".sideBarC .contato").show(self.velAnimacaoShow);
		}else{
			$.each($(".sideBarC .contato"), function(i,contato){
				$contato=$(contato);
				vSelecionado=$contato.hasClass("selecionado")?true:false;//Somente contatos
				if(fullTextCompare(strSearch,contato.title, true)||vSelecionado){
					console.log(contato.title);
  ...
  <div id="cbChatCorpo" class="row">
    <div class="col-sm-5 side">
      <div class="side-one">
		...
		<div id="cbChatPainelContatosC" class="row sideBarC">
			<div id="cbChatContatosOnlineC"></div>
		</div>
      </div>
    </div>
									...
									if(msg.tipo=="E"){
										...
										if(msg.tipo=="T"){
											self.incrementarBadgeTarefa(msg);
												...
												if (msg.datatarefa){
													//alert(self.msg2hid(msg));
													//alert(self.jContatos[self.msg2hid(msg)].rotulo);
...
function cloneContatosChat(inOpt){
	...
	$.each($($oContatos).find(".sideBarC [idcontato]"), function(i,oc){
		$oc=$(oc);
...
function compartilharTarefa(){
	...
	//Recupera todos os contatos do modal
	$oContatos=CB.oModal.find("#cbChatContatosOnlineC .contato.online.selecionado");
	$aContatos=[];
alert();
	//Separa os contatos que foram selecionados e coloca em array
	var iC=0;
