var Chat = function(){
	...
	self.init=function(){
		...
		self.hSkelContato= `
	<div id="contato_%objetocontato%_%idcontato%" class="row sideBar-body contato %status%" tipocontato="%tipocontato%" idcontato="%idcontato%" objetocontato="%objetocontato%" status="%status%" title="%title%" onclick="chat.abrirConversa(%idcontato%,'%objetocontato%');$('#chatNovaMensagem').focus();" order="%order%">
		...
		self.getContatos(function(inContatos){
			...
			$.each(inContatos, function(i,contato){
				...
				if(gIdpessoa==contato.idcontato && contato.objetocontato=="pessoa"){
					...
					if(self.oSideBar.find("#cbChatContatosOnline #contato_"+contato.objetocontato+"_"+contato.idcontato).length<=0){
					...
					if(self.oSideBar.find("#cbChatContatosOnlineNotificacao #contato_"+contato.objetocontato+"_"+contato.idcontato).length<=0){
...
//		alert("Em manutenção. Aguarde!");	
	...
	self.moverContatos=function(inContatos){
		$.each(inContatos, function(objetocontatoIdcontato,grupoStatus){
			$contato=$("#cbChatContatosOnline #contato_"+objetocontatoIdcontato);
			switch (grupoStatus){
				case "topo":
					$contato
							.hide()
