...
function logger($instr){
	...
	if($debug===true){
		...
		$logger->log($instr,$_SERVER["SCRIPT_NAME"]);
	}
}
 ...
 */
	...
	while($row = mysqli_fetch_assoc($sqlres)){
		...
		if(empty($row1['xmlret']) or empty($row1['nnfe'])){
			logger("ERRO: xmlret ou nnfe vazio");
		...
		if($row['emaildanfe']=="Y"){
		    ...
		    logger("INFO: Arquivo pdf gerado");
		...
		if($row['emailxml']=="N" and $row['emaildanfe']=="N"){
		    ...
		    logger("Email Xml ou email Danfe não selecionado");
		...
		// se o campo com os emails estiverem devidamente preenchido	
		    ...
		    $resrep=d::b()->query($sqlrep) or die();

		    if(!$resrep){
		    	logger("Erro ao buscar representante do cliente sql=".$sqlrep);
		    	die("Erro ao buscar representante do cliente sql=".$sqlrep);
		    }
		    ...
		    if($row["tipoenvioemail"]=="VENDA"){
		    	...
		    	logger("tipoenvioemail não previsto: ".$row["tipoenvioemail"]);
		    }
...
Site para consulta: <a href='".$row['url']."'> ".$row['url']."</a> </td></tr>";
			...
			logger("mail1: ".print_r($mail,true));
			...
			logger("mail2: ".print_r($mail,true));
					...
					$_qtd = mysql_num_rows($_resu);
						if($_qtd > 0){
							while($_r = mysql_fetch_assoc($_resu)){
								...
								$_sql = "insert into mailfila (idempresa,remetente,destinatario,queueid,status,idobjeto,tipoobjeto,idpessoa,criadoem,idenvio,enviadode,link,criadopor,alteradopor,alteradoem) 
		    ...
		    $resupd = d::b()->query($sqlu);

		    logger("Erro ao alterar a nf [".mysqli_error(d::b())."] ".$sqlu);
		    die();
