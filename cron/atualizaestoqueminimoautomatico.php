...
$sqlBuscaProdutoRecentes = "SELECT GROUP_CONCAT(idprodserv)  as idprodserv 
							 FROM prodserv WHERE alteradoem > DATE_SUB(NOW(), INTERVAL 10 DAY);";
$resBuscaProdutoRecentes = d::b()->query($sqlBuscaProdutoRecentes) or die("Erro ao consultar idlotefracao:".$sqlu." - ".$row["idprodserv"]);
$_rowBuscaProdutoRecentes = mysqli_fetch_assoc($resBuscaProdutoRecentes);
if($_inspecionar_sql){
	echo "<pre>".$sqlBuscaProdutoRecentes."</pre>";
}

$sqlBuscaProdRecLoteFracao = "SELECT GROUP_CONCAT(DISTINCT p.idprodserv) AS idprodserv
							  FROM lotefracao lt JOIN lote l ON l.idlote = lt.idlote
							  JOIN prodserv p ON p.idprodserv = l.idprodserv AND l.status IN ('APROVADO', 'EMANALISE', 'QUARENTENA')
							   AND lt.status = 'DISPONIVEL'
							   AND lt.idunidade = p.idunidadeest
							 WHERE (p.comprado = 'Y' OR p.fabricado = 'Y') AND p.tipo = 'PRODUTO' 
							   AND lt.alteradoem > DATE_SUB(NOW(), INTERVAL 40 DAY);";
$resBuscaProdRecLoteFracao = d::b()->query($sqlBuscaProdRecLoteFracao) or die("Erro ao consultar idlotefracao:".$sqlu." - ".$row["idprodserv"]);
$_rowBuscaProdRecLoteFracao = mysqli_fetch_assoc($resBuscaProdRecLoteFracao);
if($_inspecionar_sql){
	echo "<pre>".$sqlBuscaProdRecLoteFracao."</pre>";
}

$sqlBuscaProdRecLoteCons = "SELECT GROUP_CONCAT(DISTINCT p.idprodserv) AS idprodserv
							  FROM lotecons lc JOIN lote l ON l.idlote = lc.idlote
							  JOIN prodserv p ON p.idprodserv = l.idprodserv
							   AND l.status IN ('APROVADO', 'EMANALISE', 'QUARENTENA')
							 WHERE (p.comprado = 'Y' OR p.fabricado = 'Y') AND p.tipo = 'PRODUTO'
							   AND lc.alteradoem > DATE_SUB(NOW(), INTERVAL 3 DAY);";
$resBuscaProdRecLoteCons = d::b()->query($sqlBuscaProdRecLoteCons) or die("Erro ao consultar idlotefracao:".$sqlu." - ".$row["idprodserv"]);
$_rowBuscaProdRecLoteCons = mysqli_fetch_assoc($resBuscaProdRecLoteCons);
if($_inspecionar_sql){
	echo "<pre>".$sqlBuscaProdRecLoteCons."</pre>";
}

$sqlBuscaProdServNF = "SELECT GROUP_CONCAT(DISTINCT p.idprodserv) AS idprodserv
						FROM nf n JOIN nfitem ni ON ni.idnf = n.idnf
						JOIN prodserv p ON p.idprodserv = ni.idprodserv
						AND n.status IN ('INICIO', 'ENVIADO', 'AUTORIZADA')
						WHERE (p.comprado = 'Y' OR p.fabricado = 'Y') AND p.tipo = 'PRODUTO'
						AND n.alteradoem > DATE_SUB(NOW(), INTERVAL 10 DAY);";
$resBuscaProdServNF = d::b()->query($sqlBuscaProdServNF) or die("Erro ao consultar idlotefracao:".$sqlu." - ".$row["idprodserv"]);
$_rowBuscaProdServNF = mysqli_fetch_assoc($resBuscaProdServNF);
if($_inspecionar_sql){
	echo "<pre>".$sqlBuscaProdServNF."</pre>";
}

$_agrupaRetornoNaoFormulados = $_rowBuscaProdutoRecentes['idprodserv'].",".$_rowBuscaProdRecLoteFracao['idprodserv'].",".$_rowBuscaProdRecLoteFracao['idprodserv'].",".$_rowBuscaProdRecLoteCons['idprodserv'].",".$_rowBuscaProdServNF['idprodserv'];
$_arrayProdservNaoFormulados = explode(",", $_agrupaRetornoNaoFormulados);

$mergeProdservNaoFormulado = array_merge(array_filter($_arrayProdservNaoFormulados));
$idprodservsNaoFormulado = implode(",", $mergeProdservNaoFormulado);
//re::dis()->hMSet('cron:atualizaestoqueminimoautomatico',['prodservsNaoFormulado' => Date('d/m/Y H:i:s') . " - " .$idprodservsNaoFormulado]);
...
$sqlp = "SELECT idprodserv, nqtdest FROM(SELECT p.idprodserv, p.idunidadeest, ifnull(sum(f.qtd), 0) as nqtdest, ifnull(p.qtdest, 0) as qtdest
										   FROM prodserv p LEFT JOIN lote l ON (p.idprodserv = l.idprodserv AND l.status IN ('APROVADO', 'EMANALISE','QUARENTENA'))
...
if(!empty($idprodservsNaoFormulado))
{
	...
	$sql = "SELECT 
			...
			FROM prodserv p 
			LEFT JOIN lote l ON(l.idprodserv = p.idprodserv AND l.status IN ('APROVADO', 'EMANALISE','QUARENTENA'))
			LEFT JOIN lotefracao f ON(f.idlote = l.idlote AND f.status = 'DISPONIVEL' AND  f.idunidade = p.idunidadeest)
			WHERE (p.comprado = 'Y' OR p.fabricado = 'Y')
			AND p.tipo = 'PRODUTO' 
			$condicaoAtualizarSomenteProdutosAtivos
			AND p.idprodserv IN ($idprodservsNaoFormulado)
...
$sqlBuscaProdRecLoteFracaoFor = "SELECT GROUP_CONCAT(DISTINCT p.idprodserv) AS idprodserv
							  FROM lotefracao lt JOIN lote l ON l.idlote = lt.idlote
							  JOIN prodserv p ON p.idprodserv = l.idprodserv AND l.status IN ('APROVADO', 'EMANALISE', 'LIBERADO')
							   AND lt.status = 'DISPONIVEL'
							   AND lt.idunidade = p.idunidadeest
							 WHERE (p.comprado = 'Y' OR p.fabricado = 'Y') AND p.tipo = 'PRODUTO' 
							   AND l.piloto = 'N'
							   AND lt.alteradoem > DATE_SUB(NOW(), INTERVAL 40 DAY);";
$resBuscaProdRecLoteFracaoFor = d::b()->query($sqlBuscaProdRecLoteFracaoFor) or die("Erro ao consultar idlotefracao:".$sqlBuscaProdRecLoteFracaoFor." - ".$row["idprodserv"]);
$_rowBuscaProdRecLoteFracaoFor = mysqli_fetch_assoc($resBuscaProdRecLoteFracaoFor);
if($_inspecionar_sql){
	echo "<pre>".$sqlBuscaProdRecLoteFracaoFor."</pre>";
}

$sqlBuscaProdRecLoteConsFor = "SELECT GROUP_CONCAT(DISTINCT p.idprodserv) AS idprodserv
							  FROM lotecons lc JOIN lote l ON l.idlote = lc.idlote
							  JOIN prodserv p ON p.idprodserv = l.idprodserv
							 WHERE (p.comprado = 'Y' OR p.fabricado = 'Y') AND p.tipo = 'PRODUTO'
							   AND lc.alteradoem > DATE_SUB(NOW(), INTERVAL 10 DAY);";
$resBuscaProdRecLoteConsFor = d::b()->query($sqlBuscaProdRecLoteConsFor) or die("Erro ao consultar idlotefracao:".$sqlBuscaProdRecLoteConsFor." - ".$row["idprodserv"]);
$_rowBuscaProdRecLoteConsFor = mysqli_fetch_assoc($resBuscaProdRecLoteConsFor);
if($_inspecionar_sql){
	echo "<pre>".$sqlBuscaProdRecLoteConsFor."</pre>";
}

$_agrupaRetornoFormulados = $_rowBuscaProdutoRecentes['idprodserv'].",".$_rowBuscaProdRecLoteFracao['idprodserv'].",".$_rowBuscaProdRecLoteFracaoFor['idprodserv'].",".$_rowBuscaProdRecLoteConsFor['idprodserv'].",".$_rowBuscaProdServNF['idprodserv'];
$_arrayProdservFormulados = explode(",", $_agrupaRetornoFormulados);

$mergeProdservFormulado = array_merge(array_filter($_arrayProdservFormulados));
$idprodservsFormulado = implode(",", $mergeProdservFormulado);
//re::dis()->hMSet('cron:atualizaestoqueminimoautomatico',['prodservsFormulado' => Date('d/m/Y H:i:s') . " - " .$idprodservsFormulado]);
if(!empty($idprodservsFormulado))
{
	...
	$sql1 = "SELECT f.idprodservformula,
			...
			LEFT JOIN lote l ON(l.idprodserv = p.idprodserv AND l.idprodservformula = f.idprodservformula AND l.status IN ('APROVADO', 'EMANALISE', 'LIBERADO')) AND l.piloto = 'N'
			LEFT JOIN lotefracao lf ON(lf.idlote = l.idlote AND lf.idunidade = f.idunidadeest AND lf.status = 'DISPONIVEL')
			WHERE p.fabricado = 'Y'
			AND p.tipo = 'PRODUTO'
			$condicaoAtualizarSomenteProdutosAtivos
			AND p.idprodserv IN ($idprodservsFormulado)
	...
	while ($row1 = mysqli_fetch_assoc($res1)) 
	{
		...
		$sql_pe = " SELECT  ifnull(sum(l.qtdpedida),0) as qtdpa
					FROM lote l 
					WHERE l.status IN('AGUARDANDO', 'FORMALIZACAO', 'PROCESSANDO', 'ABERTO', 'QUARENTENA', 'TRIAGEM', 'EMANALISE')                                    
		...
		if ($np1 > 0) 
		{
			...
			if ($rowp1["controlaestoq"] ==  'Y') 
			{
				$sqll1 = "SELECT IF(pf.estmin >=  ifnull(SUM(f.qtd),0), 'Y', 'N') as controle, COUNT(pc.idprodcomprar) as quantidade
						...
						LEFT JOIN lote l ON (l.idprodserv = p.idprodserv AND l.status IN ('APROVADO', 'EMANALISE', 'QUARENTENA') AND l.idloteorigem is null AND l.idprodservformula = pf.idprodservformula)
	...
	echo 'Nenhuma prodserv para Atualizar';
}
echo "<br><br>--------------------------------------------------------------------------------------------------------------------------------------------<br><br>";
...
//re::dis()->hMSet('cron:atualizaestoqueminimoautomatico',['fim'  => Date('d/m/Y H:i:s')]);
