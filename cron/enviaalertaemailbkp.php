...
function alertageral($inidimmsgconf,$idpk,$modulo,$msg,$status){
    $vinmsg=str_replace("'"," ",$msg);
       $sl="INSERT INTO immsgconflog
        ...
        $res=d::b()->query($sl);
        if(!$res){
            echo("Erro geral id do alerta geral".$msg);
            die();
        }
}
   ...
   //busca  as configurações para envio da mensagem
    $sql="select 
		m.tab,m.modulo,m.rotulomenu,m.urldestino,tc.col,ic.idimmsgconf,ic.assunto,ic.tipo,ic.mensagem,ic.emailteste,ic.apartirde,ic.emailfrom,ic.rotulofrom,ic.idrodapeemail,ic.modulodest,ic.expiraem
                ...
                */
                ...
                if(!$resc){
                    ...
                    while($rowc=mysqli_fetch_assoc($resc)){ 
                        ...
                        if(!$rexist){
                          atualizacomunicacaoextitem("ERRO","A Consulta para verificar se contato possui o alerta falhou : " . mysqli_error(d::b())." sql=".$sqlexist); 

                          alertageral($row["idimmsgconf"],$rowc["idpessoa"],'pessoa'," A Consulta para verificar se contato possui o alerta falhou ".$sqlexist,"ERRO");
                          //continue;
                            ...
                            if($rexiste>0){
                                ...
                                */
                                ...
                                if(!empty($rowc["idlp"])){
                                    ...
                                    $slps="select l.idlp from pessoasgsetor s,"._DBCARBON."._lp l where s.idpessoa=".$rowc["idpessoa"]." and l.idlp=s.idsgsetor";                            
                                    ...
                                    if(empty($stremail)){
                                        ...
                                        for($i=0;$i<count($stremail);$i++){
                                           ...
                                           // echo $emailunico;
                                            ...
                                            /***CCO***/
                                            //$emailDestCCO="hermespedro@yahoo.com.br";
                                            //$emailDestCCONome="hermes yahoo";
                                            /*** ASSUNTO***/
                                            //$assunto=$row["assunto"];
                                            /*** TEXTO***/
                                      ...
                                      //      $mail->AddBCC($emailDestCCO, utf8_decode($emailDestCCONome));
                                          ...
                                          //  $mail->AddAttachment('/var/www/laudo/tmp/resultadopdf/resultadocomext16283.pdf');      
                                            ...
                                            if(!$mail->Send()){
                                                ...
                                                $su="update comunicacaoextdest set status='SUCESSO',alteradoem=sysdate(),alteradopor='sislaudo' where idcomunicacaoextdest=".$_SESSION['IDCOMUNICACAOEXTDEST'];
                                                $ru=d::b()->query($su);
                                                if(!$ru){
