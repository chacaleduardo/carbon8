...
/*
 * Inspecionar usuÃ¡rio. Default: false
 * para inspecionar, deve-se ajustar o usuÃ¡rio desejado
 */
...
class IM{
	...
	private $segundosStatusOffline = 30;//Segundos para considerar usuÃ¡rio como offline
	...
	function IM($call){
		...
		if($call!=='login'){
			if(!logado()){
				//Validar token enviado via Header, GET ou POST. UsuÃ¡rio nÃ£o logado no sistema. Somente utilizando o chat via token
				$arrJwt=validaToken();
				if(!$arrJwt["sucesso"]){
					die('{"erro":"Token invÃ¡lido: '.$arrJwt["erro"].'"}');
				}else{
					//Sucesso!
					$this->token=$arrJwt["token"];
					//Verifica o autenticador
					if($this->token->iss!="sislaudo"){
						die('{"erro":"Token com chave invÃ¡lida #1"}');
					}else{
						$this->setPrivateVars();
					}
				}
		...
		//Desativa o log binÃ¡rio, diminuindo o tamanho deos arquivos de log para replicaÃ§Ã£o
		...
		//Executa a aÃ§Ã£o enviada por POST
		switch($call) {
			case 'login':
			...
			case 'enviar':
				...
				$idimmsgbody=!empty($_POST["idimmsgbody"])?$_POST["idimmsgbody"]:false;//FunÃ§Ã£o de compartilhar: o msgbody nÃ£o serÃ¡ criado
...
*/
	...
	function login($usuario, $password) {
		...
		if(!empty($usuario) and !empty($password)){
			//Recupera os dados do usuÃ¡rio atravÃ©s do token devolvido pela funÃ§Ã£o de login
			...
			if($this->idpessoa){
				...
				if(count($arrcontatos) > 0){
					...
					return '{"erro":"UsuÃ¡rio nÃ£o possui contatos configurados"}';
				}
			}else{
	...
	function logout(){
		...
		//Deletar todos os EVENTOS referentes ao usuÃ¡rio logado
		...
		//Alterar status do usuÃ¡rio de chat
	...
	function contatos(){
		...
		//Deletar todos os EVENTOS referentes ao usuÃ¡rio
		$sdev = "delete from immsg where tipo='E' and descr like '%o%line%".$this->idpessoa."%'";
	...
	//Grupos que o tipo de usuÃ¡rio pode ver
	//@todo: melhoria para recuperar dinamicamente os grupos, para nÃ£o ser necessÃ¡ria intervenÃ§Ã£o de cÃ³digo
	function getRowsetGrupos($inIdcontato=false, $inObjetocontato=false){
		//Recupera informaÃ§Ãµes de um grupo especÃ­fico
		if(!empty($inIdcontato) and !empty($inObjetocontato)){
			$sqlContatoEspecifico=" and c.idcontato= ".$inIdcontato." and c.objetocontato='".$inObjetocontato."'";
		}
	...
	function historico(){
		...
		$sm = "SELECT m.idimmsg
				...
				WHERE m.idpessoa = ".$this->idpessoa."
					and m.tipo in ('M','T','X')
				ORDER BY b.idimmsgbody DESC -- Ãltimas mensagens
		...
		$arrret=array();
		$i=mysqli_num_rows($res);//A consulta Ã© ordenada decrescente, mas Ã© necessÃ¡rio implementar uma lÃ³gica inversa para que a lista de notificaÃ§Ãµes apareÃ§a com "a mais recente primeiro"
    ...
    **/
	function conversa($inIdcontato,$inObjetocontato,$inIdimmsg=false){
		...
		//LÃ³gica para recuperar mensagens anteriores
	...
	function clonargrupo($_idimgrupo, $_idpessoa, $_nomepessoa, $_idimregra, $_grupo) {
		...
		//print_r($_POST);die;
			...
			if($_idpessoa and $_idimgrupo){
				...
				//echo $sm.'<br>';
				d::b()->query($sm) or die("Erro ao cadastrar pessoas apÃ³s clonar grupo: ".mysqli_error(d::b()));
				
				$sm = "Insert into imregra values(null,1,'imgrupo',".$_novogrupo.",'imgrupo',".$_novogrupo.",'GRUPO', 'ATIVO');";
				//echo $sm.'<br>';
				d::b()->query($sm) or die("Erro ao criar regra apÃ³s clonar grupo: ".mysqli_error(d::b()));
				...
				//echo $sm.'<br>';
				d::b()->query($sm) or die($sqm."Erro ao criar atualizar contatos apÃ³s criar regra de clonar grupo: ".mysqli_error(d::b()));
...
function grupogrupo($_idimregra, $_grupo) {
		...
		//print_r($_POST);die;
			...
			if($_idimregra){
				...
				//echo $sm.'<br>';
				//d::b()->query($sm) or die("Erro ao criar regra apÃ³s criar grupogrupo: ".mysqli_error(d::b()));
				 ...
				 $sm.'<br>';
				d::b()->query($sm) or die("Erro ao cadastrar pessoas apÃ³s cadastrar grupogrupo: ".mysqli_error(d::b()));
   ...
   /**
    * Enviar mensagem para outro usuÃ¡rio
    **/
	function enviar($inContatos, $inMsg, $inImmsgbody=false, $inMsgTipo="M",$inDatatarefa,$inmodulopk,$inmodulo){
		...
		foreach ($contatos as $i=>$c){
			...
			//Recupera as informaÃ§Ãµes dos contato(s)/Grupo(s) de destino informados por _POST 
			$contato = $this->contato($c->idcontato, $c->objetocontato);

			if($contato===false){
				//UsuÃ¡rio nÃ£o Ã© contato
				return '{"code":"USUARIO_NAO_ENCONTRADO"}';
			}else{
				
				//Verificar se trata-se de uma mensagem direta ou mensagem para grupo. Em cada caso irÃ¡ realizar loop: 1 vez para mensagem direta e vÃ¡rias vezes para cada membro de grupo
				$arrRecipients=array();
				...
				//Loop em cada usuÃ¡rio de destino para enviar mensagens
				$is=0;
				...
				while (list($idcontatorec, $v) = each($arrRecipients)){
					...
					if($compartilhamento and $idcontatorec==$this->idpessoa){
						continue;//NÃ£o compartilhar mensagem consigo mesmo
					}
					...
					****************************************************************/
					...
					/****************************************************************
					* Atualiza a ordenaÃ§Ã£o dos contatos no painel de contatos, conforme a data da Ãºltima mensagem enviada
					****************************************************************/
				...
				//Atualiza para o usuÃ¡rio que enviou a mensagem:
	...
	function contato($inidcontato,$inobjetocontato){
		...
		if($iContato==0){
			//O idpessoa nÃ£o estÃ¡ configurado como contato (@todo: ou nÃ£o tem permissÃ£o para ver o grupo?)
			return false;
		}else{
	...
	function getMembrosGrupo($inIdcontato,$inObjcontato){
		if($inObjcontato=="imgrupo"){
			...
			$arrret=array();
			while($r = mysqli_fetch_assoc($res)){
				//Cria cor de back e foreground para o avatar do usuÃ¡rio
			...
			//Caso o sender nÃ£o pertenÃ§a ao grupo, mas possua permissÃ£o para vÃª-lo, deve receber uma mensagem tambÃ©m
	...
	function enviaEventoContatos($arrcontatos, $evento) {      
		...
		if(count($arrcontatos)>0){
			...
			$aonline=array();
			//Verifica quais contatos estÃ£o online
	 ...
	 * Aceita '*' como parÃ¢metro d idimmsg para alterar todas de uma vez. Ex: caso de Marcar Todas como Lidas
	 **/
	function alterarStatusMensagem($inIdcontato,$inObjetocontato,$inidimmsg,$instatus){
		...
		if(strlen($inidimmsg)>0 and strlen($instatus)>0 and !is_numeric($instatus)){
			...
			//SeleÃ§Ã£o de mensagens a serem lidas
			if($inidimmsg=="*"){
    ...
    **/
   function gc() {
      //Limpar usuÃ¡rios offline [~20% de chance]
      if(rand(1, 100) <= 20) {
		...
		//Passar para OFFLINE usuÃ¡rios online que pararam de realizar consultas ao servidor, e notificar os contatos da alteraÃ§Ã£o de status
		$suo = "SELECT 
					p.idpessoa
					,c.idcontato
				FROM ".$this->tabimpessoa." p force index(last_ping_isonline) -- UsuÃ¡rios que permanceram offline por motivos de fechamento do browser ou travamento
					JOIN imcontato c ON (c.idpessoa=p.idpessoa AND c.idcontato!=p.idpessoa AND c.objetocontato='pessoa') -- Contatos de cada usuÃ¡rio offline
					JOIN impessoa pc ON pc.idpessoa=c.idcontato AND pc.online=1 -- VerificaÃ§Ã£o de quais contatos estÃ£o online para receber notificaÃ§Ãµes de status
		...
		//Efetua update para o usuÃ¡rio e notifica os contatos online dele
		...
		foreach ($arrUo as $idpessoa => $contatos) {
			...
			//Envia notificaÃ§Ã£o de offline para os contatos online
			$this->enviaEventoContatos($contatos, "{\"offline\":\"".$idpessoa."\"}");
		}
      }
   }
   
   //Gera mensagens no chat confome configuraÃ§Ã£o realizada no carbon
    function eventosLaudo(){
        //Limpar usuÃ¡rios offline [~10% de chance]
        
        if(rand(1, 100) <= 2) {
            session_start();
            $sessionid = session_id();//PEGA A SESSÃO 
            //Altera o status das notas PENDENTES para CONSULTANDO e reserva para a sessÃ£o
       ...
       //busca  as configuraÃ§Ãµes para envio da mensagem
            $sql="select 
            ...
            while($row=mysqli_fetch_assoc($res)){
                //busca os filtros para seleÃ§Ã£o
	...
	function msg2tarefa($inIdimmsgbody,$inTransformar){
		if(!empty($inIdimmsgbody) and is_numeric($inIdimmsgbody) and !empty($inTransformar)){
			...
			//E UMA NOVA TAREFA SERÃ CRIADA, E ASSIM TAREFA E MENSAGEM EXISTIRÃO
...
//					)";
	...
	function apagarMsg($inIdimmsgbody){
		if(!empty($inIdimmsgbody) and is_numeric($inIdimmsgbody)){
			...
			//Apaga a mensagem para os usuÃ¡rios
	...
	function atualizaChat(){
		//Inativa os usuÃ¡rios temporariamente. Os idpessoa que nÃ£o forem atualizados serÃ£o excluÃ­dos no final
		d::b()->query("update ".$this->tabimpessoa." set status='INATIVAR' where inseridomanualmente='N'") or die("ERRO_ATUALIZA_10");
		
		//Listagem de usuÃ¡rios do tipoobjeto (grupo) sgsetor
		$aObjetosExternos["sgsetor"]["sql"] = "SELECT DISTINCT 
												...
												FROM sgsetor a
													JOIN pessoa p on p.idtipopessoa=a.idtipopessoa AND p.idempresa=a.idempresa
														AND NOT p.status='INATIVO'  and  p.idtipopessoa = 9;";
		...
		$aGrupos=array();
		$aUsr=array();
		//A consulta de cada grupo externo deve trazer junto o usuÃ¡rio e os grupos ao qual ele pertence, e serÃ£o separados aqui em arrays de grupos e usuarios
		foreach ($aObjetosExternos as $objext=>$val) {
			...
			while($r = mysqli_fetch_assoc($roe)){
				...
				//Separa os usuÃ¡rios dentro dos grupos
				$aUsr[$objext][$r["idobjetoext"]][$r["idpessoa"]]["idempresa"]=$r["idempresa"];
			}
		}
				
		foreach ($aGrupos as $oExt=>$grupo){
			//Inativa os grupos do Objeto Externo temporariamente. Os que nÃ£o forem atualizados serÃ£o excluÃ­dos no final
			...
			//Para cada "grupo" do obj externo, verifica se jÃ¡ existe. Caso negativo: insere. Update para ATIVO caso exista. Caso nÃ£o exista: permanecerÃ¡ com status INATIVAR.
		...
		//Inclui os usuÃ¡rio snos respectivos grupos
		...
		//Reativa os usuÃ¡rios que vieram nas consultas
		foreach ($aUsr as $oExt=>$obj){//objetoexterno
			foreach ($obj as $idobjetoexterno=>$pessoas){//idpbjetoexterno
				foreach ($pessoas as $idpessoa=>$v){//pessoa
		...
		//Exclui grupos e pessoas que nÃ£o vieram nas consultas
...
select * from imregra
*/
		// ADICIONA OS NOVOS GRUPOS (CADASTRADOS ACIMA ATRAVÃS DA SGSETOR) NA TABELA DE REGRAS DO CHAT. 39 Ã O GRUPO DE FUNCIONÃRIOS
		...
		////	: Cria clone de grupos para que as pessoas possam ter conversas privadas com cada grupo, mesmo nÃ£o pertencendo ao grupo.
		...
		//echo '<pre>'.$sql.'</pre>';
			...
			//DELETA TODAS AS PESSOAS DO "GRUPO" QUE NÃO FAZEM MAIS PARTE DO SETOR.
			...
			//DELETA TODAS AS PESSOAS DO "GRUPO GRUPO" QUE NÃO FAZEM MAIS PARTE DO SETOR.
							$sm = "delete from imgrupopessoa where idimgrupopessoa in (select * from (
                            select gp.idimgrupopessoa from imgrupopessoa gp 
...
SELECT distinct 1, r.idimregra, gp.idpessoa, gc.idpessoa as idcontato, 'pessoa' as objetocontato, now(),'N', 'A' as status, (SELECT ultimamsg from imcontato where idpessoa = gp.idpessoa and idcontato = gc.idpessoa and objetocontato = 'pessoa') as ultimamsg
       ...
       a group by idpessoa, idcontato") or die("ERRO_ATUALIZA_150");
		...
		where 
		-- r.status = 'ATIVO' and
		r.tiporegra='GRUPO' and r.tipoobjetoorigem='imgrupo' and r.tipoobjetodestino = 'imgrupo'") or die("ERRO_ATUALIZA_160");
...
//mysqli_close(d::b());//@todo: EstÃ¡ retornando uma string desconhecida: SysSession[2]:
?>
