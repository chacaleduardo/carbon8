...
$statusCriaTestes = array('ASSINAR', 'ASSINADO', 'CANCELADO', 'INATIVO', 'ENVIADO', 'DEVOLVIDO', 'FECHADO');
$statusAlteraTestesMater = array('CANCELADO', 'INATIVO');
if((in_array($_SESSION["arrpostbuffer"]["1"]["u"]["amostra"]["status"], $statusCriaTestes) && $_GET['_acao'] == 'u' && !array_key_exists("amostramaster", getModsUsr("MODULOS")))
		|| (array_key_exists("amostramaster", getModsUsr("MODULOS")) && in_array($_SESSION["arrpostbuffer"]["1"]["u"]["amostra"]["status"], $statusAlteraTestesMater) && $_GET['_acao'] == 'u')
		|| ($_SESSION["arrpostbuffer"]["1"]["u"]["amostra"]["status"] == 'CONFERIDO')){
	die("Não é possível alterar a amostra, pois a mesma está conferida.");
}
