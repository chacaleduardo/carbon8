...
}elseif($tiponf=="C" or $tiponf=="T" or $tiponf=="S" or $tiponf=="E" or $tiponf=="M" or $tiponf=="F"){//if($tiponf=="V"){
...
// echo($arrParcelas['quant']." - ".$arrlinhasbol['quant']." - ".$qtParcelasitem);die;
	...
	if(empty(trim($diasentrada))){
		...
		for ($index = 1; $index <= $qtdparcelas; $index++) {
			...
			if($contapagaritem=="Y"){
				$tmpsqlins = "INSERT INTO contapagaritem (idempresa,status,idpessoa,idcontaitem,idobjetoorigem,tipoobjetoorigem,idagencia,datapagto,valor,criadopor,criadoem,alteradopor,alteradoem)
				VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",'ABERTO',".$idpessoa.",".$idcontaitem.",".$idnotafiscal.",'nf',".$idagencia.",".$recebcalc.",".$valorparcela.",'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate(),'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
			}else{
			    if($tiponf=="F"){
				...
				if(!empty($idcontaitem)){
					$tmpsqlins = "INSERT INTO contapagar (idempresa".$tipoespecifico.",idcontaitem,idagencia,idpessoa,tipoobjeto,idobjeto,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,visivel,intervalo,criadopor,criadoem)
					VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"]."".$vtipoespecifico.",".$idcontaitem.",".$idagencia.",".$idpessoa.",'nf',".$idnotafiscal.",".$index.",".$qtdparcelas.",".$valorparcela.",".$vencimentocalc.",".$recebcalc.",'".$statuscp."','".$formapgto."','".$tipo."','".$visivel."',".$intervalo.",'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
				}else{
					$tmpsqlins = "INSERT INTO contapagar (idempresa".$tipoespecifico.",idagencia,idpessoa,tipoobjeto,idobjeto,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,visivel,intervalo,criadopor,criadoem)
					VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"]."".$vtipoespecifico.",".$idagencia.",".$idpessoa.",'nf',".$idnotafiscal.",".$index.",".$qtdparcelas.",".$valorparcela.",".$vencimentocalc.",".$recebcalc.",'".$statuscp."','".$formapgto."','".$tipo."','".$visivel."',".$intervalo.",'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
				}  
			}
			...
			if($comissao=='Y' and !empty($arrComisssao['idpessoa']) and !empty($idcontapagar)){
				...
				$tmpsqlins = "INSERT INTO contapagaritem (idempresa,status,idpessoa,idobjetoorigem,tipoobjetoorigem,idagencia,datapagto,valor,criadopor,criadoem,alteradopor,alteradoem)
				VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",'ABERTO',".$arrComisssao['idpessoa'].",".$idcontapagar.",'contapagar',".$idagencia.",".$datapg.",".$valorn.",'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate(),'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
...
if(!empty($gnreval) and !empty($gnre) and !empty($dtemissao) and !empty($idnotafiscal)){
	...
	if ($qtParcelas == 0){
		...
		$tmpsqlins = "INSERT INTO contapagar (idempresa,idpessoa,idcontaitem,idagencia,tipoobjeto,idobjeto,obs,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,intervalo,visivel,criadopor,criadoem)
		VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",6022,10,3,'gnre',".$idnotafiscal.",'NFe (".$nnfe.")-".$gnre."',1,1,".$gnreval.",".$vencimentocalc.",".$vencimentocalc.",'PENDENTE','BOLETO','D',30,'S','".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
...
if(!empty($nf_darf) and ($nf_darf>0) and !empty($dtemissao) and !empty($idnotafiscal)){    
    ...
    if ($qtParcelas == 0){
	    ...
	    $tmpsqlins = "INSERT INTO contapagar (idempresa,idpessoa,idcontaitem,idagencia,tipoobjeto,idobjeto,obs,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,intervalo,visivel,criadopor,criadoem)
	    VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",6021,10,".$idagencia.",'nf_darf',".$idnotafiscal.",'NFe (".$nnfe.") - CSRF ',1,1,".$nf_darf.",".$vencimentocalc.",".$vencimentocalc.",'PENDENTE','BOLETO','D',30,'S','".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
...
if(!empty($nf_ir) and ($nf_ir>0) and !empty($dtemissao) and !empty($idnotafiscal)){
	...
	if ($qtParcelas==0){
		...
		$tmpsqlins = "INSERT INTO contapagar (idempresa,idpessoa,idcontaitem,idagencia,tipoobjeto,idobjeto,obs,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,intervalo,visivel,criadopor,criadoem)
			VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",".$idpessoa.",10,".$idagencia.",'nf_ir',".$idnotafiscal.",'NFe (".$nnfe.") - IRRF ',1,1,".$nf_ir.",".$vencimentocalc.",".$vencimentocalc.",'PENDENTE','BOLETO','D',30,'S','".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
...
if(!empty($nf_inss) and ($nf_inss>0) and !empty($dtemissao) and !empty($idnotafiscal)){
   ...
   if ($qtParcelas == 0){
	...
	$tmpsqlins = "INSERT INTO contapagar (idempresa,idpessoa,idcontaitem,idagencia,tipoobjeto,idobjeto,obs,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,intervalo,criadopor,criadoem)
		    VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",".$idpessoa.",10,".$idagencia.",'nf_inss',".$idnotafiscal.",'NFe (".$nnfe.") - IRRF ',1,1,".$nf_inss.",".$vencimentocalc.",".$vencimentocalc.",'PENDENTE','BOLETO','D',30,'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
...
if(!empty($nf_issret) and ($nf_issret>0) and !empty($dtemissao) and !empty($idnotafiscal)){
   ...
   if ($qtParcelas == 0){
	    ...
	    $tmpsqlins = "INSERT INTO contapagar (idempresa,idpessoa,idcontaitem,idagencia,tipoobjeto,idobjeto,obs,parcela,parcelas,valor,datapagto,datareceb,status,formapagto,tipo,intervalo,criadopor,criadoem)
		    VALUES (".$_SESSION["SESSAO"]["IDEMPRESA"].",".$idpessoa.",10,".$idagencia.",'nf_issret',".$idnotafiscal.",'NFe (".$nnfe.") - ISS Retido ',1,1,".$nf_issret.",".$vencimentocalc.",".$vencimentocalc.",'PENDENTE','BOLETO','D',30,'".$_SESSION["SESSAO"]["USUARIO"]."',sysdate())";
