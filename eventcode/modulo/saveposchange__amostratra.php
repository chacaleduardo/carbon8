...
foreach($_POST as $k=>$v) {
	if (preg_match("/removerIdentificacao(\d*)/", $k, $res)) {
		$arrInsIndicadores[$res[1]][$res[2]] = $v;
	}
}

foreach($arrInsIndicadores as $indice => $indicador){	
	$chaveQtd = 'qtdRemoverIdentificaco' . $indice;
	$chaveDesc = 'removerIdentificacao' . $indice;
	$indicador = $_POST[$chaveDesc];
	$limit = $_POST[$chaveQtd];
	$indicadoresApagar = AmostraController::buscarIndicadoresApagar($_GET['idamostra'], $indicador, $limit);
	foreach($indicadoresApagar as $ind){
		AmostraController::apagarIndicadores($ind['ididentificador']);
	}
}
