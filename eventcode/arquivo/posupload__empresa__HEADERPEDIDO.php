...
if($verifica){
	...
	$_sql = "UPDATE arquivo SET caminho = '".$caminhoaux."' WHERE tipoarquivo = 'HEADERPEDIDO' and tipoobjeto = 'empresa' and idobjeto = ".$_idobjeto;
	$_res = d::b()->query($_sql) or die("Erro ao atualizar caminho da tabela arquivo. SQL: ".$_sql." ".mysqli_error(d::b()));
	$_sql1 = "UPDATE empresa SET figrelatorioprod = '".$caminhoaux."' WHERE idempresa = ".$_idobjeto;
	$_res1 = d::b()->query($_sql1) or die("Erro ao atualizar caminho da tabela empresa. SQL: ".$_sql1." ".mysqli_error(d::b()));
	
	$_sql2 = "INSERT INTO empresaimagem (idempresa,tipoimagem,caminho,criadopor,criadoem,alteradopor,alteradoem) VALUES (".$_idobjeto.",'HEADERPEDIDO','".$caminhoaux."','".$_SESSION["SESSAO"]["USUARIO"]."',now(),'".$_SESSION["SESSAO"]["USUARIO"]."',now())";
