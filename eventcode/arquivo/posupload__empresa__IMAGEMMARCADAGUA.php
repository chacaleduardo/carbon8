...
if($verifica){
	...
	FROM empresaimagem 
	WHERE tipoimagem='IMAGEMMARCADAGUA' 
	...
	$_sql = "UPDATE arquivo SET caminho = '".$caminhoaux."' WHERE tipoarquivo = 'IMAGEMMARCADAGUA' and tipoobjeto = 'empresa' and idobjeto = ".$_idobjeto;
	$_res = d::b()->query($_sql) or die("Erro ao atualizar caminho da tabela arquivo. SQL: ".$_sql." ".mysqli_error(d::b()));

	$_sql2 = "INSERT INTO empresaimagem (idempresa,tipoimagem,caminho,criadopor,criadoem,alteradopor,alteradoem) VALUES (".$_idobjeto.",'IMAGEMMARCADAGUA','".$caminhoaux."','".$_SESSION["SESSAO"]["USUARIO"]."',now(),'".$_SESSION["SESSAO"]["USUARIO"]."',now())";
