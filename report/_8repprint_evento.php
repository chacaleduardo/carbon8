...
if (!empty($_GET)){
        ...
        if ($_REQUEST['_fds']){
            ...
            if ($data1 and $data2)
            {
                ...
                foreach($arrRep["_datas"] as $ko => $vo)
                {
                    ...
                    //echo '<br>';
                    $_sqldata .= $_or . "( e." . $vo . " between $dataFormat1 and $dataFormat2)";	
                    $_or = " or ";
                }
            }
    ...
    if ($_REQUEST['idempresa'] != ''){
        $_andidempresa = "and e.idempresa in (".$_REQUEST['idempresa'].")";
    ...
    $_sqlresultado=" $_tab e $_sqlwhere";
	 ...
	 ****************************************************************************/
	$relatorios = MenuRelatorioController::buscarRelatorioDinamico("select ".implode(",",$arrRep["_colvisiveis"])." ", ($_sqlresultado));
	if ($relatorios === false) {
	    die('<b>Falha na execucao da Consulta para o Report:</b> ' . mysql_error() . "<br>" . $_sqlresultado);
	}

    $ideventotipo = $relatorios[0]['ideventotipo'];
    $colsvselect = "";
    foreach($arrRep["_colvisiveis"] as $k1 => $colsv){
        $colsvselect .= "'".$colsv."',";
    }
    $colsvselect = substr($colsvselect,0,-1);
    $relatorioColunas = MenuRelatorioController::buscarRelatorioDinamicoEvento($ideventotipo,$colsvselect);
...
<?
    ...
    foreach($relatorios as $rrow => $rvalue){
        unset($relatorios[$rrow]['ideventotipo']);
    }
	...
	{	
		if($arrRep["_filtros"][$arrRep["_colvisiveis"][$key+1]]["visres"] == 'Y')
		{
            ...
            if($relatorioColunas[$coluna]['rotulo'] != ''){
                $arrRep["_filtros"][$arrRep["_colvisiveis"][$key+1]]["rotulo"] = $relatorioColunas[$coluna]['rotulo'];
            }
    ...
    foreach($relatorios as $_row)
	{
		...
		foreach(array_keys($_row) as $key => $coluna)
		{
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
				...
				}elseif($relatorioColunas[$_nomecol]["code"] !=''){
                    $_strvlrhtml = MenuRelatorioController::buscarValorColunaRelatorioDinamicoEvento($relatorioColunas[$_nomecol]["code"],$_row[$coluna]);
					// $_strvlrhtml = number_format($_row[$coluna], 2, ',','.');
				}else{
