...
if (!empty($_GET)){
    ...
    $_sqlresultado="   (
                        select s.botao,e.idempresa
                        from vw8rep296 e join
                        fluxostatuspessoa p JOIN fluxostatus f ON f.idfluxostatus = p.idfluxostatus
                        JOIN carbonnovo._status s ON s.idstatus = f.idstatus
                        and p.idmodulo = e.idevento
                        AND p.modulo = 'evento'
                        AND s.idstatus IN (133, 134, 135, 218, 276) 
                        where e.ideventotipo=21 
                        ".$_andidempresa."
                        ".$_sqldata." group by e.idevento) as u  join empresa ep on(ep.idempresa=u.idempresa) group by u.botao,u.idempresa order by ep.empresa,u.botao;";
	 ...
	 ****************************************************************************/
	$relatorios = MenuRelatorioController::buscarRelatorioDinamico("select ep.empresa,u.botao as tipo,count(*) as qtd", ($_sqlresultado));
	if ($relatorios === false) {
