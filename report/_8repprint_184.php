...
if (!empty($_GET)){
	...
	$strs = "Nenhum Registro encontrado entre ".$data[0]." e ".$data[1];

	if($_ipagpsqres==1){
		$strs = "Registro encontrado entre ".$data[0]." e ".$data[1];
	}elseif($_ipagpsqres>1){
		$strs = "$_ipagpsqres registros encontrados entre ".$data[0]." e ".$data[1];
	}
...
<?
    ...
    foreach($relatorios as $_row)
	{
		...
		if($nome!=$_row['contaitem']){
			$mostrar = '<tr style="	text-align: right !important; background-color:#eee; font-size:10px"><td colspan="3" ><b>TOTAL '.$nome.'</td><td align="right"><b>'.number_format($vtotal, 2, ',','.').'</b></td></tr>';
			$vtotal=0; 
			$nome=$_row['contaitem'];
		}else{
			$mostrar = '';
...
<?
	$mostrar = '<tr style="	text-align: right !important; background-color:#eee; font-size:10px"><td colspan="3" ><b>TOTAL '.$nome.'</td><td align="right"><b>'.number_format($vtotal, 2, ',','.').'</b></td></tr>';
	//$mostrar2 = '<tr class="res"  style=" 	text-align: right !important; font-size:10px"><td colspan="4" >TOTAL '.$grupo.'</td><td align="right"><b>'.number_format($valorg	, 2, ',','.').'</b></td></tr>';
	//echo $mostrar2;
	echo $mostrar;
	if(!empty($_arrsoma)){
?>		
