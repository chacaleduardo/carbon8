...
<?
	...
	{	
		if($arrRep["_filtros"][$arrRep["_colvisiveis"][$key+1]]["visres"] == 'Y')
		{
	    	...
	    	if(!empty($conteudoexport) || !empty($conteudoexporttxt)){
	    		$conteudoexport.=";";
	    	}
	...
	// $conteudoexporttxt.="\n";//QUEBRA DE LINHA NO CONTEUDO TXT
		...
		foreach(array_keys($_row) as $key => $coluna)
		{
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
				...
				if($arrRep["_filtros"][$_nomecol]["datatype"]=='longtext'){
					$_strvlrhtml =strtoupper(retira_acentos_esp(nl2br($_row[$coluna])));
					...
					$_strvlrhtml =strtoupper(retira_acentos_esp($_row[$coluna]));
				}
				...
				if($_i>0){
					...
					$conteudoexporttxt.=";";//COLOCA A VIRGULA ENTRE OS VALORES 
				}
