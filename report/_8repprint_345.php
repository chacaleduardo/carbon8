...
if (!empty($_GET)){
		...
		if ($_REQUEST['_fds']){
			...
			if ($data1 and $data2)
			{
				...
				foreach($arrRep["_datas"] as $ko => $vo)
				{
					...
					$_or = " or ";
					$_sqldatagraf ="SELECT ".$dataFormat1." AS start_date, ".$dataFormat2." AS end_date";
				}
			}
	...
	if(!empty($_sqldatagraf)){
		$_sqlinterv=" SELECT 
						DATE_FORMAT(start_date, '%Y-%m-%d') AS start_date,
						DATE_FORMAT(end_date, '%Y-%m-%d') AS end_date,
						DATEDIFF(end_date, start_date) AS date_diff,
						CASE 
							WHEN DATEDIFF(end_date, start_date) <= 7 THEN 'Dia'
							WHEN DATEDIFF(end_date, start_date) > 7 AND DATEDIFF(end_date, start_date) <= 30 THEN 'Semana'
							WHEN DATEDIFF(end_date, start_date) > 30 AND DATEDIFF(end_date, start_date) <= 180 THEN 'Mês'
							WHEN DATEDIFF(end_date, start_date) > 180 AND DATEDIFF(end_date, start_date) <= 360 THEN 'Bimestre'
							WHEN DATEDIFF(end_date, start_date) > 360 THEN 'Trimestre'
						END AS visualizacao
					FROM (
						".$_sqldatagraf."
					) AS date_range;";
			...
			$vizualgraf=MenuRelatorioController::buscarVisualizacaograf($_sqlinterv);

			$visualizacaoGrafico=$vizualgraf['visualizacao'];
	...
	if($visualizacaoGrafico=='Dia'){
		...
		$sql_graf="select count(*) as qtd, valordia as valor
					from (
					".$_SESSION["SEARCH"]["SQL"]."
					) as u group by valor";

	  
	}elseif($visualizacaoGrafico=='Semana'){
		$sql_graf="select count(*) as qtd, valorsemana as valor
					from (
					".$_SESSION["SEARCH"]["SQL"]."
					) as u group by valor";


	}elseif($visualizacaoGrafico=='Mês'){
		$sql_graf="select count(*) as qtd, valormes as valor
					from (
					".$_SESSION["SEARCH"]["SQL"]."
					) as u group by valor";


	}elseif($visualizacaoGrafico=='Bimestre'){
		$sql_graf="select count(*) as qtd, valorbimestre as valor
					from (
					".$_SESSION["SEARCH"]["SQL"]."
					) as u group by valor";


	}elseif($visualizacaoGrafico=='Trimestre'){
		$sql_graf="select count(*) as qtd, valortrimestre as valor
					from (
					".$_SESSION["SEARCH"]["SQL"]."
					) as u group by valor";
	...
	if(!empty($sql_graf)){
		...
		$arrayGrafico = MenuRelatorioController::buscarDadosgraf($sql_graf);
    ...
    foreach($relatorios as $_row)
	{
		...
		foreach(array_keys($_row) as $key => $coluna)
		{
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
    	    	...
    	    	//se for para somar o valor do campox
