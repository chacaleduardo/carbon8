 ...
 */
 if($_GET and !empty($clausulad)){
    ...
    $sql="SELECT  p.razaosocial,c.*,dma(c.datareceb) as dtreceb,dma(n.dtemissao) as dtemissao,CASE c.status    WHEN 'QUITADO' THEN 1
                ...
                and c.tipo='D'
                ".$strtipo."
                and c.status in ('PENDENTE','ABERTO')  order by  p.razaosocial,n.dtemissao asc,ordem asc,c.tipo asc,c.quitadoemseg";
...
<html>
<head>
<title>Sislaudo - Contas à Pagar</title>
...
<table class="tbrepheader">
	...
	<tr>
		...
		<td class="header">Relatório à Pagar - (<?=$agencia?>)</td>
...
<table class="normal" style="font-size: 10px;">
	...
	$conteudoexport='"Emissao";"Fornecedor";"NF";"Parcela";"Vencimento";"Valor"';
...
<?	
	$conteudoexport.='"'.$row["dtemissao"].'";"'.$favorecido.'";"'.$descnf.'";"'.$row["parcela"].'de'.$row["parcelas"].'";"'.$row["dtreceb"].'";"'. $row["valor"].'"';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
...
<?
        $conteudoexport.='"'.$row["dtemissao"].'";"'.$favorecido.'";"'.$rowx['nnfe'].'";"'.$rowx["parcela"].'de'.$rowx["parcelas"].'";"'.$row['dtreceb'].'";"'.$rowx['valor'].'"';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
                    }//while($rowx=mysqli_fetch_assoc($resx)){
