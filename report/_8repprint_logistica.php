...
//	echo  $_i.' - '.$_numcolunas;
	...
	$strtabheader .= "<td class='header' id='percentualtotalfrete' style='text-align:left'>Total Frete(%)</td></tr></thead><tbody>";
	...
	$totalpercentual = 0;
	$_resultados1 = d::b()->query($_sqlresultado);
	while ($_row1 = mysql_fetch_array($_resultados1)){
		$totalpercentual += $_row1["total"];
	}
	...
	if($_numlinha == "Y"){
		...
		$conteudoexport.="\"".number_format((($_row["custoenvio"]*100)/$totalpercentual), 2, ',','.')."\"";
		$percentualtotal += (($_row["custoenvio"]*100)/$totalpercentual);
		echo "<td>".number_format((($_row["custoenvio"]*100)/$totalpercentual), 2, ',','.')."</td>";
		//print_r($_row["custoenvio"]);
...
<?		
		...
		echo "<td class=\"tot\" style=\"text-align:left\">".number_format(($percentualtotal), 2, ',','.')."%</td>";		
?>
