...
<body>
<?
function  buscaitem($idloteconsumido,$percentual,$idNf, $dtEmissao, $nnfe,$_numcolunas,$lvl=0){
    global $arr;	
	$loteInsumoConsumoInsumo = MenuRelatorioController::buscarConsumoInsumoPorIdLoteENnfe($idloteconsumido, $nnfe);	
	foreach($loteInsumoConsumoInsumo as $rowx)
	{
		$_i = 0;
		$rowx['idnf']=$idNf; 
		$rowx['dtemissao']=$dtEmissao; 

		if($rowx['fabricado']=='Y'){
			$percentualcon=$rowx['qtdd']/$rowx['qtdprod'];
			$percent=$percentual*$percentualcon;

			buscaitem($rowx['idlote'],$percent,$idNf, $dtEmissao, $nnfe,$_numcolunas,$lvl+1);
			...
			if($rowx['valor']>0){
				$rand=rand(0,1000);
				foreach(array_keys($rowx) as $coluna)
				{
					$arr[$rowx['descr']][$rowx['idlotecons'].'_'.$lvl.$rand][$coluna]=$rowx[$coluna];
					$_i++;
				} 
			}
		}              
...
if (!empty($_GET)){
	 ...
	 ****************************************************************************/
	$fields = $strselectfields;
	$query = $_sqlresultado.$_sqldata.$_and_idempresa.$str_fts.$strgrp.$strord;

	if($_GET["testehidnf"]>0){
		$fields = "select qtdd, unpadrao, partida, descr, tipoprodserv, contaitem, nnfe, vlrlote, valor, idlote, idlotecons, fabricado, qtdprod";
		$query = "laudo.vwrelprodutosvendidos 
				where idempresa in ('2') and ((dtemissao between '2022-02-24 00:00:00' and '2022-03-02 23:59:59') and idnf=".$_GET["testehidnf"]." and idlote=136642)";
	}

	$relatorios = MenuRelatorioController::buscarRelatorioDinamico($fields, $query);
	if ($relatorios === false) {
...
//adaptação para o REL 201
	...
	foreach($relatorios as $row)
	{
        ...
        if($row['fabricado']=='N'){
			...
			{
                $arr[$row['descr']][$row['idlotecons']."_".$rand][$coluna]=$row[$coluna];          
                $_i++;
            }
        }else{
            $percentualcon=floatval($row['qtdd'])/($row['qtdprod'] ? floatval($row['qtdprod']) : 1);
            buscaitem($row['idlote'],$percentualcon,$row['idnf'], $row['dtemissao'], $row['nnfe'],$_numcolunas,0);
       }
    }

	ksort($arr);
	foreach ($arr as $produto) {
		foreach ($produto as $_row) 
		{
			$_ilinha++;
			$_i = 0;
					...
					*/
					...
					if(!empty($arrRep["_filtros"][$_nomecol]["eixograph"])){
						if($arrRep["_filtros"][$_nomecol]["eixograph"] == 'X'){
							array_push($arrEixoX, $_strvlrhtml);
						}else if($arrRep["_filtros"][$_nomecol]["eixograph"] == 'Y'){
							array_push($arrEixoY, $graficoY);
						}
					}
