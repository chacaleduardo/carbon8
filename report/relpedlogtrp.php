...
<?
    ...
    while ($_row = mysql_fetch_array($_resultados)){
		...
		if(empty($idtransp)){
			$idtransp=$_row[11];
			$transportadora = $_row[10];
		}

		if($idtransp != $_row[11]){
			$transportadora = $_row[10];
			$linhasoma =  '<tr style="background-color:#eee; height: 20px; font-size:10px"><td colspan="5" align="right" ><b> SOMA QTD. VOL: </b> '.$sqtdvol.'    </td><td colspan="5">    <b>SOMA PESO BRUTO:</b>    '.$spbruto.' </td> </tr>';
			$mostrar = '<tr style="background-color:#eee; height: 20px; font-size:10px">  <td colspan="9">    <b>TRANSPORTADORA:</b>    '.$transportadora.'   <td align="right"><b>R$  '.number_format($vtotal, 2, ',','.').'</b></td></tr>';
			$linhadiv = '<tr style="background-color:#eee; height: 35px;"> </tr>'.$strtabheader;
			$vtotal = 0;
			$sqtdvol = 0;
		    $spbruto = 0;
			$idtransp = $_row[11];
...
<?
 ...
 $linhasoma =  '<tr style="background-color:#eee; height: 20px; font-size:10px"><td colspan="5" align="right" ><b> SOMA QTD. VOL: </b> '.$sqtdvol.'    </td><td colspan="5">    <b>SOMA PESO BRUTO:</b>    '.$spbruto.' </td> </tr>';
 $mostrar = '<tr style="background-color:#eee; height: 20px; font-size:10px">  <td colspan="9">    <b>TRANSPORTADORA:</b>    '.$transportadora.' <td align="right"><b>R$  '.number_format($vtotal, 2, ',','.').'</b></td></tr>';
 $linhadiv = '<tr style="background-color:#eee; height: 20px;"> </tr>';
