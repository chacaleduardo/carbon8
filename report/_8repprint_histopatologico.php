...
if (!empty($_GET)){
	...
	while (list($_col, $_val) = each($_GET)) {
			...
			if($_iclausulas > 0){
				$_sqlresultado = getDbTabela($_tab).".". $_tab." a"." ".$_sqlwhere;
				...
				while ($_i < $_numcolunas ) {
						...
						$_arridxcol[$_i] = $_metacmp->name;
							...
							if($arrRep["_filtros"][$_metacmp->name]["visres"] == 'Y'){
								...
								$strtabheader .= "<td class='header".$_i."' style='text-align:center'>" . $arrRep["_filtros"][$_metacmp->name]["rotulo"] ."</td>";
							}
							
						$conteudoexport.= "\"".$arrRep["_filtros"][$_metacmp->name]["rotulo"]."\"";// GRAVA O ROTULO DOS CABEÇALHOS NA VARIAVEL PARA GERAR O CSV
						$_i++;
						
					}   // se quiser adicionar um campo fora das tabelas 
									...
									*/
									...
									if(!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])){
										...
										$_corfontfim="</font>";
										 ...
										 $class = 'class="personalizado'.$_i.'"';
									//Finalmente: desenha o campo na tela
									if(!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])){
										echo "<td>".$_hyperlink."</td>";
									}else{
										echo "<td ".$class.$_stralign." ".$_hyperlink." ".$_colorlink.">".$_corfont.$_strvlrhtml.$_corfontfim."</td>";
									}
								}
								$_i++;
								}	 
