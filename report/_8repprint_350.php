...
if (!empty($_GET)){
    ...
    $_sqldataC= str_replace("periodo", "c.criadoem", $_sqldata);

    if(!empty($_REQUEST['idtipoprodserv'])){
        $tipoprodserv=" and t.idtipoprodserv in (".$_REQUEST['idtipoprodserv'].") ";
    }

    if(!empty($_REQUEST['idcontaitem'])){
        $contaitem=" and c.idcontaitem in (".$_REQUEST['idcontaitem'].") ";
    }

    $strselectfields="SELECT 
    ps.idprodserv,
    ROUND(IFNULL(SUM(qtdd), 0), 2) AS qtd_debito,
    ROUND(SUM(debito), 2) AS vlr_debito,
    ROUND(IFNULL(SUM(u.qtd), 0), 2) AS qtd_credito,
    ROUND(SUM(credito), 2) AS vlr_credito,
     ROUND(IFNULL(SUM(qtdp), 0), 2) AS qtd_perda,
    ROUND(SUM(perda), 2) AS vlr_perda,
    ps.descr,
    'Almoxarifado',
    ...
    SELECT 
        0 AS qtdd,
            0 AS debito,
            SUM((c.qtdc * i.qtd) / l.qtdprod) AS qtd,
            SUM(((c.qtdc * i.qtd) / l.qtdprod) * IFNULL(li.vlrlote, 0)) AS credito,
            0 AS qtdp,
            0 AS perda,
            li.idprodserv
    FROM
    ...
    JOIN lotefracao f ON (f.idlotefracao = c.idlotefracao
        AND f.idunidade = ".$_REQUEST['idunidade'].")
    JOIN lote l ON (l.idlote = f.idlote
        AND f.idunidade != l.idunidade)
    JOIN loteitem i ON (l.idlote = i.idlote)
	join lote li on(li.idlote=i.idloteins)
    WHERE
       c.qtdc > 0
       ".$_sqldataC."
            AND c.status != 'INATIVO'
            AND c.tipoobjeto != 'lote'
            AND c.idempresa IN (".cb::idempresa().")
            AND c.idobjeto IS NOT NULL
    GROUP BY l.idprodserv
	UNION 
    SELECT 
        0 AS qtdd,
            0 AS debito,
            SUM((c.qtdini * i.qtd) / l.qtdprod) AS qtd,
            SUM(((c.qtdini * i.qtd) / l.qtdprod) * IFNULL(li.vlrlote, 0)) AS credito,
             0 AS qtdp,
            0 AS perda,
            li.idprodserv
    FROM
    ...
    JOIN lote l ON (l.idlote = c.idlote
        AND c.idunidade != l.idunidade)
    JOIN loteitem i ON (l.idlote = i.idlote)
	join lote li on(li.idlote=i.idloteins)
    WHERE c.idunidade = ".$_REQUEST['idunidade']."
        ".$_sqldataC."
        AND c.idempresa IN (".cb::idempresa().")
    GROUP BY l.idprodserv 
    UNION 
    SELECT 
        0 AS qtdd,
            0 AS debito,
            SUM(c.qtdini) AS qtd,
            SUM(c.qtdini * IFNULL(l.vlrlote, 0)) AS credito,
             0 AS qtdp,
            0 AS perda,
            l.idprodserv
    FROM
    ...
    JOIN lote l ON (l.idlote = c.idlote
        ...
        AND l.idempresa IN (".cb::idempresa()."))
    WHERE
        c.idunidade = ".$_REQUEST['idunidade']."
        ".$_sqldataC."
    GROUP BY l.idprodserv 
    UNION 
    SELECT 
        0 AS qtdd,
            0 AS debito,
            SUM(c.qtdc) AS qtd,
            SUM(c.qtdc * IFNULL(l.vlrlote, 0)) AS credito,
             0 AS qtdp,
            0 AS perda,
            l.idprodserv
    FROM
    ...
    UNION 
    SELECT 
        SUM((c.qtdd * i.qtd) / l.qtdprod) AS qtdd,
            (((c.qtdd * i.qtd) / l.qtdprod) * IFNULL(li.vlrlote, 0)) AS debito,
            0 AS qtd,
            0 AS credito,
             0 AS qtdp,
            0 AS perda,
    ...
    SELECT 
        SUM(c.qtdd) AS qtdd,
            SUM(c.qtdd * IFNULL(l.vlrlote, 0)) AS debito,
            0 AS qtd,
            0 AS credito,
             0 AS qtdp,
            0 AS perda,
    ...
    SELECT 
            ...
            0 AS qtd,
            0 AS credito,
			SUM((c.qtdd * i.qtd) / l.qtdprod)  AS qtdp,
			(((c.qtdd * i.qtd) / l.qtdprod) * IFNULL(li.vlrlote, 0))  AS perda,
            li.idprodserv
    FROM
        lotecons c
    JOIN lotefracao f ON (f.idlotefracao = c.idlotefracao
    ...
    SELECT 
       0 AS qtdd,
            ...
            0 AS credito,
             SUM(c.qtdd)  AS qtdp,
             SUM(c.qtdd * IFNULL(l.vlrlote, 0)) AS perda,
            l.idprodserv
    FROM
        lotecons c
    JOIN lotefracao f ON (f.idlotefracao = c.idlotefracao
    ...
    GROUP BY l.idprodserv
        ...
        JOIN
    prodserv ps ON (ps.idprodserv = u.idprodserv)
        LEFT JOIN
    tipoprodserv t ON (t.idtipoprodserv = ps.idtipoprodserv ".$tipoprodserv.")
        LEFT JOIN
    prodservcontaitem i ON (i.idprodserv = ps.idprodserv
        AND i.status = 'ATIVO')
        LEFT JOIN
    contaitem c ON (c.idcontaitem = i.idcontaitem ".$contaitem.")
WHERE
    ps.idempresa = ".cb::idempresa()."
GROUP BY ps.idprodserv";
	
	/****************************************************************************
	 * CONCATENACAO PRINCIPAL DO SELECT A SER EXECUTADO PARA O RELATORIO        *
	 ****************************************************************************/
	$relatorios = MenuRelatorioController::buscarRelatorioDinamico($strselectfields, ($_sqlresultado));
	if ($relatorios === false) {
