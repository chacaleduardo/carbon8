...
<?
    ...
    while ($_row = mysql_fetch_array($_resultados)){
		...
		if(empty($representante)){
			$representante=$_row[0];
		}
		
		if($representante!=$_row[0]){
			$mostrar = '<tr style="background-color:#eee; font-size:10px"><td colspan="3" ><b>TOTAL '.$representante.'</td><td align="right"><b>'.number_format($vtotal, 2, ',','.').'</b></td><td align="right"><b>'.number_format($vcomissao, 2, ',','.').'</b></td></tr>';
			$vtotal = 0;
            $vcomissao = 0;
            $representante  = $_row[0];
		}else{
		...
		$vcomissao = $vcomissao + $_row['comissao'];
