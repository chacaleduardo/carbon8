...
if ($_idrep == 94 || $_idrep == 22 ){
	...
	if (base64_decode('MTk0NA==') == $_SESSION["SESSAO"]["IDPESSOA"] || base64_decode('ODE4Nw==') == $_SESSION["SESSAO"]["IDPESSOA"]){
...
if (!empty($_GET["exercicio"])){
			...
			while ($_y < $_numcolunas) {
				...
				if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y' and $arrRep["_filtros"][$_nomecol]["acsum"]=='Y'){ 
					...
					if (containsDecimal($_arrsoma[$_tab][$_nomecol])){
							...
							$auxsoma += $_arrsoma[$_tab][$_nomecol];
						}else{
	...
	?>
			<td class="tot" style="text-align:left"><?=number_format($auxsoma, 2, ',','.');?></td>
			</tr>
	<?	 		
		}	?>
	...
	die("Por Favor informe um exercício para filtrar o relatório");
}
?>
