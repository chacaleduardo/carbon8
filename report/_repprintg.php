...
if (!empty($_GET)){
	 ...
	 ****************************************************************************/
	$_sqlresultado = $strselectfields." from ".$_sqlresultado.$_sqldata.$_and_idempresa.$str_fts.$strord;
...
<?
	...
	while ($_i < $_numcolunas) {
	    ...
	    if($arrRep["_filtros"][$_metacmp->name]["visres"] == 'Y'){
			...
			if ($_i ==0){
				$strtabheader .= "<td style='display:none' class='header'>" . $arrRep["_filtros"][$_metacmp->name]["rotulo"] ."</td>";
			}else{
	...
	if ($_ilinha == 0 ||  $novo != $_row[0]){
		$novo = $_row[0];
		$ncolunas = $_numcolunas-1;
		echo "\n<tr   class=\"res\"  ". $_link ." ". $_strhlcolor . ">
		<td style='height:40px' colspan=".$ncolunas.">$novo</td></tr>";
	}
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
				...
				if(!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])){
					echo "<td>".$_hyperlink."</td>";
						...
						if ($_i ==0){
							echo "<td style='display:none' ".$_stralign." ".$_hyperlink." ".$_colorlink.">".$_corfont.$_strvlrhtml.$_corfontfim."</td>";
						}else{
