	...
	<div style="display:none; text-align: left; font-size: 9px;" class="n_linhas"><span  id="nlinha"><?=$strs?></span></div>
			...
			<?
				...
				*/
				...
				{
					if(count(array_keys($relatorios[0])) - 1 == $key) break;
				...
				*/
				...
				$strtabini = "\n<table class='normal' id='content' style='margin-left: 30px; margin-right:30px;'>";
				...
				*/
				...
				foreach($relatorios as $_row)
				{
					...
					if($uf != $_row['uf']){
						$linhasoma =  '<tr><td colspan="14"><table><tr style="background-color:#eee; height: 20px; font-size:10px"><td width="33%" ><b>QTD. PEDIDOS.: </b> '.$nped.' </td><td width="33%"  align="center" ><b> SOMA QTD. VOL: </b> '.$sqtdvol.'    </td><td width="33%" align="right">    <b>SOMA PESO BRUTO:</b>    '.$spbruto.' </td> </tr>';
						$mostrar = '<tr style="background-color:#eee; height: 20px; font-size:10px">  <td >    <b>UF:</b>    '.$estado. '  - '.$uf.' <td colspan="2" align="right"><b>R$  '.number_format($vtotal, 2, ',','.').'</b></td></tr>';
						$linhadiv = '<tr style="background-color:#eee; height: 20px;"> </tr></table></td></tr>'.$strtabheader;
						$vtotal = 0;
						$sqtdvol = 0;
						$spbruto = 0;
						$idnf = $_row['idnf'];
					...
					*/
					...
					{
						if(count(array_keys($_row)) - 1 == $key) break;
...
<?
 $linhasoma =  '<tr><td colspan="14"><table><tr style="background-color:#eee; height: 20px; font-size:10px"><td  align="center" width="50%"><b> SOMA QTD. VOL: </b> '.$sqtdvol.'    </td><td align="right" width="50%">    <b>SOMA PESO BRUTO:</b>    '.$spbruto.' </td> </tr>';
 $mostrar = '<tr style="background-color:#eee; height: 20px; font-size:10px">  <td >    <b>UF:</b>    '.$estado. ' - '. $uf.' </td><td align="right" ><b>R$  '.number_format($vtotal, 2, ',','.').'</b></td></tr></table></td></tr>';
	echo $linhasoma;
	echo $mostrar;
	echo $linhadiv;
 	if(!empty($_arrsoma)){
