...
<style style="display: none;" class="rTable" type="text/css">
	...
	#cliente, #resultado-semente, .nome, .resultado-semente{
		width: 40%;
	}
...
//	echo  $_i.' - '.$_numcolunas;
	while ($_i < $_numcolunas) {
	   ...
	   // if($arrRep["_filtros"][$_metacmp->name]["visres"] == 'Y'){
			...
			$strtabheader .= "<div class='rTableHead header ".seo_friendly_url(str_replace('`','',$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["rotulo"]))."' id='".seo_friendly_url(str_replace('`','',$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["rotulo"]))."' style=\"text-align:".$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["align"]."\">" . str_replace('`','',$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["rotulo"])."</div>";
	    }	
	 ...
	 */
	$strpagini = "\n<fieldset class='fldsheader' style='margin-bottom: 10px;'><legend>Impressão ".$_nomeimpressao."</legend></fieldset>";
	$strtabini = "\n<div class='rTable'>";
	...
	if($_ilinha>1){
		...
		if($_ilinha==1){
			echo $strpagini; // fieldset
			echo $strtabini; // abre 'table'
			echo $strtabheader; // abre thead
			$var_tra=$_row[2];
	...
	//echo("<br>[$_strhlcolor]<br>");
    	echo "\n<div class='rTableRow res' ". $_link ." ". $_strhlcolor . ">";
	...
	if($_numlinha == "Y"){
		...
		$_strvlrhtml="";
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
				...
				if(!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])){
					echo "<div class='rTableCell ".seo_friendly_url(str_replace('`','',$_arridxcol[$_i]))."' ".$_stralign." >".$_hyperlink."&nbsp;</div>";
				}else{
					echo "<div class='rTableCell ".seo_friendly_url(str_replace('`','',$_arridxcol[$_i]))."' ".$_stralign." ".$_hyperlink." ".$_colorlink.">".$_corfont.$_strvlrhtml.$_corfontfim."&nbsp;</div>";
				}
	    	}
	    	$_i++;
    	}
		...
		if($_ilinhaquebra == 10){
			echo "\n</div><div class='quebrapagina'></div>";
			echo $strtabini;
			echo "<div class='escondeheader'></div>";
			echo $strtabheader;
			$_ilinhaquebra = 0;
		}
...
?>
	...
	<legend>Fim da Impressão <?=$_nomeimpressao . " teste".$varfooter?></legend>
	</fieldset>
	<script>
		$(".escondeheader").siblings('.rTableHeading').addClass('showprint')
	</script>
</body>
</html>
<?
