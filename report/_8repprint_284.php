...
<?
    ...
    $strtabheader .= "<td class='header' id='custo-direto' style=\"white-space: nowrap;\">Custo direto<br>&nbsp;<i class='fa fa-arrow-down pointer' title='Ordenar Crescente' style='font-size: 0.8em; opacity: 0;' attr='desc'></i>&nbsp;<i class='fa fa-arrow-up pointer' title='Ordenar Decrescente' style='font-size: 0.8em; opacity: 0;' attr='asc'></i></td>";
    $strtabheader .= "<td class='header' id='custo-indireto' style=\"white-space: nowrap;\">Custo indireto<br>&nbsp;<i class='fa fa-arrow-down pointer' title='Ordenar Crescente' style='font-size: 0.8em; opacity: 0;' attr='desc'></i>&nbsp;<i class='fa fa-arrow-up pointer' title='Ordenar Decrescente' style='font-size: 0.8em; opacity: 0;' attr='asc'></i></td>";
    $strtabheader .= "<td class='header' id='custo-insumo' style=\"white-space: nowrap;\">Custo insumo<br>&nbsp;<i class='fa fa-arrow-down pointer' title='Ordenar Crescente' style='font-size: 0.8em; opacity: 0;' attr='desc'></i>&nbsp;<i class='fa fa-arrow-up pointer' title='Ordenar Decrescente' style='font-size: 0.8em; opacity: 0;' attr='asc'></i></td>";
    $strtabheader .= "<td class='header' id='custo-testes' style=\"white-space: nowrap;\">Custo testes<br>&nbsp;<i class='fa fa-arrow-down pointer' title='Ordenar Crescente' style='font-size: 0.8em; opacity: 0;' attr='desc'></i>&nbsp;<i class='fa fa-arrow-up pointer' title='Ordenar Decrescente' style='font-size: 0.8em; opacity: 0;' attr='asc'></i></td>";
    $strtabheader .= "<td class='header' id='custo-total' style=\"white-space: nowrap;\">Custo total<br>&nbsp;<i class='fa fa-arrow-down pointer' title='Ordenar Crescente' style='font-size: 0.8em; opacity: 0;' attr='desc'></i>&nbsp;<i class='fa fa-arrow-up pointer' title='Ordenar Decrescente' style='font-size: 0.8em; opacity: 0;' attr='asc'></i></td>";
	...
	// Pegar id do lotes para fazer uma preconsulta de insumos e custos diretos e indiretos
	$todosIdLoteArr = [];

	foreach($relatorios as $item) {
		// Capturar os IDs dos lotes na descrição
		preg_match_all('/idlote=(\d+)/', $item['descr'], $matches);
		$idLoteArr = $matches[1] ?? [];

		foreach($idLoteArr as $idLote) {
			$todosIdLoteArr[$idLote] = $idLote;
		}
	}

	$custosDiretosIndiretos = LoteController::buscarCustoDiretoIndiretoPorIdLote(implode(',', $todosIdLoteArr));
	$custoInsumos = LoteController::buscarInsumosLote(implode(',', $todosIdLoteArr));
	$custoTestes = LoteController::buscarCustoTestes(implode(',', $todosIdLoteArr));

	$custosDiretosIndiretosFormatado = [];
	$custoInsumosFormatado = [];
	$custoTestesFormatado = [];
	
	// Estrutura de custo de testes
	foreach($custoTestes as $teste) $custoTestesFormatado[$teste['idlote']] = $teste;

	// Estruturado custos insumos por idlote
	foreach($custoInsumos as $insumo) $custoInsumosFormatado[$insumo['idlote']] = $insumo;
	
	// Estruturado custos diretos/indiretos por idlote
	foreach($custosDiretosIndiretos as $custo) {
		if(!$custosDiretosIndiretosFormatado[$custo['idlote']])
			$custosDiretosIndiretosFormatado[$custo['idlote']] = [];

		if(!$custosDiretosIndiretosFormatado[$custo['idlote']]['CD'])
			$custosDiretosIndiretosFormatado[$custo['idlote']]['CD'] = ['valor' => [], 'valorformatado' => []];

		if(!$custosDiretosIndiretosFormatado[$custo['idlote']]['CI'])
			$custosDiretosIndiretosFormatado[$custo['idlote']]['CI'] = ['valor' => [], 'valorformatado' => []];

		if($custo['tipocusto'] == 'CD') {
			$custosDiretosIndiretosFormatado[$custo['idlote']]['CD']['valor'][] = floatval($custo['valor']);
			$custosDiretosIndiretosFormatado[$custo['idlote']]['CD']['valorformatado'][] = "R$ ". number_format($custo['valor'], 4, ',', '.');
		} else {
			$custosDiretosIndiretosFormatado[$custo['idlote']]['CI']['valor'][] = floatval($custo['valor']);
			$custosDiretosIndiretosFormatado[$custo['idlote']]['CI']['valorformatado'][] = "R$ ". number_format($custo['valor'], 4, ',', '.');
		}
	}
    ...
    foreach($relatorios as $_row)
	{
		...
		$valoresDiretosIndiretos = false;
		$custoInsumo = [];
		$custoTestes = [];
		$custoTotal = 0;

		// Capturar os IDs dos lotes na descrição
		preg_match_all('/idlote=(\d+)/', $_row['descr'], $matches);
		$idLoteArr = $matches[1] ?? [];

		if (!empty($idLoteArr)) {
			$idLoteStr = implode(',', $idLoteArr);
		}
		...
		foreach ($idLoteArr as $idLote) {
			if (!empty($custosDiretosIndiretosFormatado[$idLote]['CD'])) {
				$custoTotal += array_sum($custosDiretosIndiretosFormatado[$idLote]['CD']['valor']);
				$custos['CD'] = $custos['CD'] + $custosDiretosIndiretosFormatado[$idLote]['CD']['valorformatado'];
			} else
				$custos['CD'][] = '-';

			if(!empty($custosDiretosIndiretosFormatado[$idLote]['CI'])) {
				$custoTotal += array_sum($custosDiretosIndiretosFormatado[$idLote]['CI']['valor']);
				$custos['CI'] = $custos['CI'] + $custosDiretosIndiretosFormatado[$idLote]['CI']['valorformatado'];
			} else 
				$custos['CI'][] = '-';
		}

		// Renderizar custos diretos e indiretos
		foreach (['CD' => 'custo-direto', 'CI' => 'custo-indireto'] as $tipo => $col) {
			echo "<td datatype='varchar' col='{$col}' title='{$tipo}'>";
			if (!empty($custos[$tipo])) {
				echo implode("<br>", $custos[$tipo]);
			} else {
				echo "-";
			}
			echo "</td>";
		...
		// Renderizar custos de insumos
		echo "<td datatype='varchar' col='custo-insumo'>";
		foreach ($idLoteArr as $idLote) {
			$insumo = $custoInsumosFormatado[$idLote];
			$custoTotal += floatval($insumo['insumo']);

			if (!empty($insumo)) {
				echo "R$ ".number_format($insumo['insumo'], 4, ',', '.');
			} else {
				echo "-";
			}
			echo "<br>";
		...
		// Renderizar custos de testes
		echo "<td datatype='varchar'     col='custo-testes'>";
		foreach ($idLoteArr as $idLote) {
			$teste = $custoTestesFormatado[$idLote];
			$custoTotal += floatval($teste['valor']);

			if (!empty($teste)) {
				echo "R$ ".number_format($teste['valor'], 4, ',', '.');
			} else {
				echo "-";
			}
			echo "<br>";
		...
		$custoTotalSoma += $custoTotal; 

		// Renderizar custo total
		echo "<td datatype='varchar' col='custo-total'>R$ ".number_format($custoTotal, 4, ',', '.')."</td>";
...
<?
	...
	if((!empty($_arrsoma) or !empty($_arrsomaavg)) || $custoTotalSoma){
...
<?		
		...
		echo "<td></td>";
		echo "<td></td>";
		echo "<td class=\"tot\">R$ ".number_format($custoTotalSoma, 4, ',', '.')."</td>";	
?>
		</tr>
<?	 		
	}	?>
