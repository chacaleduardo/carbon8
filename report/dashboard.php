...
join loteativ a on a.idlote = l.idlote
 join loteobj o on o.idloteativ = a.idloteativ and o.tipoobjeto = 'tag' 
	...
	and
		idtipoprodserv = 19 and
		especial = '".$especial."' 
		".getidempresa('l.idempresa','formalizacao')."	
...
SELECT
   ...
   FROM
    ...
    WHERE
	l.especial = 'N' and
        l.status = 'APROVADO' and l.statusfr = 'DISPONÍVEL' AND l.estoque >= 0
...
if (count(1) > 0,'danger','success') as card_color,
 ...
 union all
	...
	SELECT
		...
		'col-md-12 col-sm-12 col-xs-6' as card_class_col,
		concat('_modulo=documento&_pagina=0&_ordcol=idsgdoc&_orddir=desc&_filtrosrapidos={%22assinaturadoc%22:%22PENDENTE%22,%status%22:%22APROVADO%22}') as card_url,
		'fundovermelho' as card_notification_bg,
		'0' as card_notification,
		if (count(1) > 0,'danger','success') as card_color,
		if (count(1) > 0,'danger','success') as card_border_color,
...
if (count(1) > 0,'danger','success') as card_color,
 ...
 "	;
	...
	$dashlogistica = "  SELECT 
		...
		'col-md-6 col-sm-6 col-xs-6' as card_class_col,
		concat('_modulo=pedidologistica&_pagina=0&_ordcol=idnf&_orddir=desc&_filtrosrapidos={%22status%22:%22DESPACHADO%22}') as card_url,
		'fundovermelho' as card_notification_bg,
		'0' as card_notification,
		if (count(1) > 0,'danger','success') as card_color,
		if (count(1) > 0,'danger','success') as card_border_color,
	...
	where
		 ...
		 status = 'Despachado'   ".getidempresa('idempresa','pedidologistica')."  
...
union all 
  SELECT
		...
		'col-md-6 col-sm-6 col-xs-6' as card_class_col,
		concat('_modulo=pedidologistica&_pagina=0&_ordcol=idnf&_orddir=desc&_filtrosrapidos={%22status%22:%22DESPACHADO,DESPACHAR%22}&idnf=[',group_concat(idnf separator ','),']') as card_url,
		'fundovermelho' as card_notification_bg,
		'0' as card_notification,
		if (count(1) > 0,'danger','success') as card_color,
		if (count(1) > 0,'danger','success') as card_border_color,
	...
	from
		vwnfsaida
	where 
		status in ('Despachado', 'Despachar') ".getidempresa('idempresa','pedidologistica')."	
...
union all
	...
	'col-md-6' as panel_class_col,
	'LOGÍSTICA - ENVIOS' as panel_title,
	'dashlogisticaenviosconcluidosmespassado' as card_id,
	'col-md-6 col-sm-6 col-xs-6' as card_class_col,
	concat('_modulo=pedidologistica&_pagina=0&_ordcol=idnf&_orddir=desc&_filtrosrapidos={%22status%22:%22CONCLUIDO%22}&idnf=[',group_concat(idnf separator ','),']') as card_url,
	'fundovermelho' as card_notification_bg,
	'0' as card_notification,
	if (count(1) > 0,'success','secundary') as card_color,
	if (count(1) > 0,'success','secundary') as card_border_color,
	'' as card_bg_class,
	'mês passado' as card_title,
	'' as card_title_sub,
	count(1) as card_value,
	'fa-print' as card_icon,
	'LOGÍSTICA - ENVIOS CONCLUÍDOS MÊS PASSADO' as card_title_modal,
	'_modulo=pedidologistica&_acao=u' as card_url_modal
from
	vwnfsaida
where 
	status in ('CONCLUIDO') ".getidempresa('idempresa','pedidologistica')."	
	and envio between last_day(curdate() - interval 2 month) + interval 1 day and last_day(curdate() - interval 1 month)
	...
	'col-md-6' as panel_class_col,
	'LOGÍSTICA - ENVIOS' as panel_title,
	'dashlogisticaenviosconcluidosestemes' as card_id,
	'col-md-6 col-sm-6 col-xs-6' as card_class_col,
	concat('_modulo=pedidologistica&_pagina=0&_ordcol=idnf&_orddir=desc&_filtrosrapidos={%22status%22:%22CONCLUIDO%22}&idnf=[',group_concat(idnf separator ','),']') as card_url,
	'fundovermelho' as card_notification_bg,
	'0' as card_notification,
	...
	count(1) as card_value,
	'fa-print' as card_icon,
	'LOGÍSTICA - ENVIOS CONCLUÍDOS ESTE MÊS' as card_title_modal,
	'_modulo=pedidologistica&_acao=u' as card_url_modal
from
	vwnfsaida
where 
	status in ('CONCLUIDO') ".getidempresa('idempresa','pedidologistica')."	
	and envio between last_day(curdate() - interval 1 month) + interval 1 day and  last_day(curdate())
...
'dashlogisticafaturaaberta' as panel_id,
				'col-md-12' as panel_class_col,
				'FATURAS CTE - ABERTO' as panel_title,
				concat(
					'dashlogisticafaturaaberta',
					replace(trim(lower(c.idcontapagar)), ' ', '')
				) as card_id,
				'col-md-2 col-sm-2 col-xs-6' as card_class_col,
				concat(
					'?_modulo=contapagarlogistica&_acao=u&idcontapagar=',
					c.idcontapagar,
					'&novajanela=Y'
				) as card_url,
				'fundovermelho' as card_notification_bg,
				'0' as card_notification,
				if (sum(c.valor) >= c.valor, 'secundary', 'secundary ') as card_color,
				if (sum(c.valor) >= c.valor, 'secundary', 'secundary ') as card_border_color,
				'' as card_bg_class,
				p.nome as card_title,
				...
				concat('R$ ', format(c.valor, 2, 'de_DE')) as card_value,
				'fa-print' as card_icon,
				'' as card_title_modal,
				'_modulo=contapagarlogistica&_acao=u' as card_url_modal
                
            
        
        from 
`contapagar` `c`
join contapagaritem cp on c.idcontapagar = cp.idcontapagar
JOIN `formapagamento` `f` ON ((`f`.`idformapagamento` = `c`.`idformapagamento`))
join
(select c.idpessoa, min(c.datareceb) as datareceb
    FROM
        ((((`contapagar` `c`
        LEFT JOIN `contapagaritem` `i` ON ((`c`.`idcontapagar` = `i`.`idcontapagar`)))
     )
        LEFT JOIN `nf` `n` ON (((`n`.`idnf` = `i`.`idobjetoorigem`)
            AND (`n`.`tiponf` = 'T')
    ...
    GROUP BY  c.idpessoa ) l on l.idpessoa = c.idpessoa and l.datareceb = c.datareceb
       JOIN `pessoa` `p` ON (((`p`.`idpessoa` = `c`.`idpessoa`)
            AND (`p`.`idtipopessoa` = 11)))
            group by
           c.idpessoa, c.idcontapagar

		   union all

		   select 

'dashlogisticafaturapendente' as panel_id,
				'col-md-12' as panel_class_col,
				'FATURAS CTE - PENDENTE' as panel_title,
				concat(
					'dashlogisticafaturapendente',
					replace(trim(lower(c.idcontapagar)), ' ', '')
				) as card_id,
				'col-md-2 col-sm-2 col-xs-6' as card_class_col,
				concat(
...
'dashlogisticafaturafechado' as panel_id,
				'col-md-12' as panel_class_col,
				'FATURAS CTE - FECHADO' as panel_title,
				concat(
					'dashlogisticafaturafechado',
					replace(trim(lower(c.idcontapagar)), ' ', '')
				) as card_id,
				'col-md-2 col-sm-2 col-xs-6' as card_class_col,
				concat(
...
SELECT
	...
	where
	`e`.`idfluxostatus` in (11,778,10,15,9,162) and e.ideventotipo in ( 21)
union all
SELECT
		'dashti' as panel_id,
		'col-md-2' as panel_class_col,
	...
	from
		evento `e`
	where
	`e`.`idfluxostatus`  in (276,796,271,272,270,273,474,476,471,472,473) and e.ideventotipo in ( 28,53)
	...
	where
		 e.ideventotipo in ( 40) 
		 and  (s.statustipo not in ('CANCELADO','CONCLUIDO') or s.statustipo is null)
...
SELECT
		'dashsuprimentocompras' as panel_id,
		'col-md-2' as panel_class_col,
		'EVENTOS' as panel_title,
		'dashsuprimentocomprastodos' as card_id,
		'col-md-12 col-sm-12 col-xs-6' as card_class_col, 
		concat('_modulo=evento&_pagina=0&_ordcol=prazoamd&_orddir=asc&_filtrosrapidos={%22ideventotipo%22:%2224,44,46,48,56,58,59,60,61,62,63,64,65,66,67,68,70,71,73,74,75,76,77,78,79%22}&idevento=[',group_concat(idevento separator ','),']') as card_url,
		'fundovermelho' as card_notification_bg,
		'0' as card_notification,
		if (count(1) > 0,'danger','success') as card_color,
		if (count(1) > 0,'danger','success') as card_border_color,
		'' as card_bg_class,
		'SOLICITAÇÃO DE COMPRA' as card_title,
		'' as card_title_sub,
		count(1) as card_value,
		'fa-print' as card_icon,
		'EVENTOS - SOLICITAÇÃO DE COMPRA' as card_title_modal,
		'_modulo=evento&_acao=u' as card_url_modal
	from
		evento `e` 
	
		join fluxostatus fs on fs.idfluxostatus = e.idfluxostatus
		join carbonnovo._status s on s.idstatus = fs.idstatus and s.statustipo not in ('CANCELADO','CONCLUIDO')
	where
		 e.ideventotipo in ( 24,44,46,48,56,58,59,60,61,62,63,64,65,66,67,68,70,71,73,74,75,76,77,78,79)
...
1
	...
	SELECT c.idcotacao AS idprodserv,
			...
			FROM nfitem i FORCE INDEX(nf_prod) JOIN nf n ON i.idnf = n.idnf JOIN cotacao c ON n.idobjetosolipor = c.idcotacao
		...
		FROM
			...
			JOIN nfitem i FORCE INDEX (nf_prod) ON n.tipoobjetosolipor = 'cotacao'
				AND i.idnf = n.idnf
...
SELECT
		...
		'col-md-4' as panel_class_col,
		'COMPRAS' as panel_title,
		'dashcomprasaprovadasdivergencia' as card_id,
		'col-md-6 col-sm-6 col-xs-6' as card_class_col,  
		concat('_modulo=nfentrada&_pagina=0&_ordcol=idnf&_orddir=desc&_filtrosrapidos={%22status%22:%22DIVERGENCIA%22}&_fds=', DATE_FORMAT(DATE_SUB(DATE_FORMAT(CURRENT_DATE,'%Y-%m-%d'), interval 1 year),'%d/%m/%Y'),'-',DATE_FORMAT(DATE_SUB(CURRENT_DATE, interval 1 day),'%d/%m/%Y')) as card_url,
		'fundovermelho' as card_notification_bg,
		'0' as card_notification,
		if (count(1) > 0,'danger','success') as card_color,
		if (count(1) > 0,'danger','success') as card_border_color,
		'' as card_bg_class,
		'divergência' as card_title,
		'' as card_title_sub,
		count(1) as card_value,
		'fa-print' as card_icon,
		'COMPRAS - DIVERGÊNCIA' as card_title_modal,
		'_modulo=nfentrada&_acao=u' as card_url_modal
	 FROM
       `nf` `n`
    WHERE
	n.status = 'DIVERGENCIA'
    ".getidempresa('idempresa','nfentrada')." and n.tiponf IN ('C' , 'T', 'E', 'S', 'M', 'F', 'B')
...
select
		...
		'col-md-12' as panel_class_col,
		'dashresultadosematraso' as panel_title,
...
SELECT 
		...
		'col-md-6 col-sm-6 col-xs-6' as card_class_col, 
		concat('_modulo=pessoa&_pagina=0&_ordcol=idpessoa&_orddir=desc&_filtrosrapidos={%22representacao%22:%22N%22,%22status%22:%22ATIVO%22,%22idtipopessoa%22:%222%22}') as card_url,
		'fundovermelho' as card_notification_bg,
		'0' as card_notification,
		if (count(1) > 0,'danger','success') as card_color,
		if (count(1) > 0,'danger','success') as card_border_color,
...
exists(select 1 
	from prodserv p join  unidadeobjeto u on( u.idunidade = 9 and u.idobjeto = p.idprodserv and u.tipoobjeto = 'prodserv')
	 ...
	 and  DATE_FORMAT(CURRENT_DATE,'%Y-%m-%d') >= a.vencimento and a.status = 'APROVADO' and a.statusfr = 'DISPONÍVEL' and a.estoque > 0
	  ".$pessoas."
