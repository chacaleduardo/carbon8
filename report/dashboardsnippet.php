...
$dashboardCardPanelGrupo = [];
$contadorR = count($dashboardCardPanelGrupo);
$lps = _LpController::buscarLpDashPorIdLpEIdEmpresa($idLps);
$i = 0;
...
<div class="row m-0">
    <!-- 
        * MONTA MENU LATERAL
        -->
    <div id="_menu_lateral_" class="col-md-2">
        <div class="panel panel-default">
            ...
            <div class="panel-body" style="padding-top: 30px !important;">
                <div class="list-group">
                    ...
                    foreach ($lps as $v) {
                        $dashConf = json_decode($v['jsondashboardconf'], true);
                        foreach ($dashConf['dashgrupo'] as $dashGrupo) {
                            $classAtivo = (!empty($userPref)
                                and !empty($userPref['grupo'][$dashGrupo['id'] . "_" . $v['idlp']])
                                and $userPref['grupo'][$dashGrupo['id'] . "_" . $v['idlp']] == 'Y') ? 'ativo' : '';
                        ?>
                            <div class="col-md-12 p-0">
                                <button class="botao-menu-lateral <?= $classAtivo ?>" id="grupoBtn-<?= $dashGrupo['id'] ?>-<?= $v['idlp'] ?>" onclick="hideShowGrupo(this, <?= $v['idlp'] ?>, <?= $dashGrupo['id'] ?>)"><?= $dashGrupo['rotulo'] ?></button>
                            </div>
                    <? }
                    } ?>
                </div>
    ...
    <!-- 
        * MONTA PAINEL DASH
        -->
    <div id="_col_md_control_" class="col-md-10" id="dashboards" style="margin-top: 8px;">
...
<? }
foreach ($lps as $v) { ?>
    <div id="dashgrupo-conf-<?= $v['idlp'] ?>"></div>
<? } ?>
</div>
</div>
<?
require_once(__DIR__ . "/../form/js/dashboardsnippet_js.php");
