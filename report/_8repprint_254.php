...
if (!empty($_GET)){
	...
	if(!empty($arrRep["_colvisiveis"])){
		...
		CASE
			...
			ELSE linknf
		END as linknf , replace(round(qtd,2),'.',',')  as qtd , un ,
...
if($_REQUEST['idformapagamento']){
	$cidformapagamento = " and cp.idformapagamento in ( ".$_REQUEST['idformapagamento'].") ";
}


$cstatus='';
$statusFilter = explode(',', strtoupper($_REQUEST['status']));

if($statusFilter) 
	$cstatus = ' and cp.status in ("'.implode('","', $statusFilter).'") ';
...
if($cp_tipo=='C,D' || $cp_tipo=='D'){
	...
	FROM
		...
		JOIN `nfitem` `i` ON (`i`.`idnf` = `n`.`idnf` AND `i`.`nfe` = 'Y'  and i.cobrar ='Y')
		JOIN `tipoprodserv` `p` ON (`p`.`idtipoprodserv` = `i`.`idtipoprodserv`)
		JOIN `contaitem` `c` ON (`c`.`idcontaitem` = `i`.`idcontaitem` ".$strFluxoDre." )
...
if($cp_tipo=='C,D' || $cp_tipo=='C'){
	...
	where cp.tipoespecifico!= 'AGRUPAMENTO' 
		...
		".$cidformapagamento."
		".$cstatus."
		and cp.tipo = 'C' group by  i.idnotafiscal,cp.idcontapagar,i.idprodserv,i.valor,i.desconto
	union all
	SELECT   
		'SV' as tiponf,c.contaitem,c.idcontaitem,c.idcontatipo,c.cor,c.somarelatorio,c.previsao  ,cp.status,cp.tipo,c.faturamento
		,c.ordem,
	...
	where cp.tipoespecifico = 'AGRUPAMENTO'                                    
		...
		".$cidformapagamento."
		".$cstatus."
		and cp.tipo = 'C' group by  i.idnotafiscal,ci.idcontapagaritem,i.idprodserv,i.valor,i.desconto
	...
	where cp.tipoespecifico!= 'AGRUPAMENTO'                                                            
		...
		".$cidformapagamento."
		".$cstatus."
		and cp.tipo = 'C' group by  i.idnotafiscal,cp.idcontapagar,i.idtipoprodserv,i.valor,i.desconto
	union all
	SELECT   
		'SV' as tiponf,c.contaitem,c.idcontaitem,c.idcontatipo,c.cor,c.somarelatorio,c.previsao  ,cp.status,cp.tipo,c.faturamento
		,c.ordem, 
	...
	where cp.tipoespecifico = 'AGRUPAMENTO'                                     
		...
		".$cidformapagamento."
		".$cstatus."
		and cp.tipo = 'C' group by  i.idnotafiscal,ci.idcontapagaritem,i.idtipoprodserv,i.valor,i.desconto
		".$vwcontaitemtp;
}
	...
	from
		...
		left join (select sum(previsao) as previsao, idtipoprodserv from vwtipoprodservprevisao pe1 where pe1.idempresaprev in ( ".$_REQUEST['idempresa'].")  and pe1.exercicio in(".$strano.") and pe1.mes in(".$strmes.")  group by pe1.idtipoprodserv ) pe on(pe.idtipoprodserv=a.idtipoprodserv)
		left join (select sum(previsao) as previsao, idcontaitem from vwcontaitemprevisao pe3 where pe3.idempresaprev in ( ".$_REQUEST['idempresa'].")  and  pe3.exercicio in(".$strano.") and  pe3.mes in(".$strmes.")  group by pe3.idcontaitem) p3 on(p3.idcontaitem=a.idcontaitem)
		left join (select sum(previsao) as previsao, idcontatipo from vwcontatipoprevisao pe2 where pe2.idempresaprev in ( ".$_REQUEST['idempresa'].")  and pe2.exercicio in(".$strano.") and pe2.mes in(".$strmes.")  group by pe2.idcontatipo) p2 on(p2.idcontatipo=t.idcontatipo)
		left join (select sum(previsao) as previsao, idcontatipogrupo from vwcontatipogrupoprevisao pe1 where pe1.idempresaprev in ( ".$_REQUEST['idempresa'].")  and pe1.exercicio in(".$strano.") and pe1.mes in(".$strmes.")  group by pe1.idcontatipogrupo) p1 on(p1.idcontatipogrupo=g.idcontatipogrupo)

	where 1 and g.status='ATIVO' and g.idempresa in ( ".$_REQUEST['idempresa'].") ".$clausulaidcontatipogrupo."
...
*/
	...
	if ($empresarateio <>  $_row[6] || $empresarateio == ''){
		...
		if($_row[16]=='I'){
			...
			$strprevisao="<div title='Previsão' class='somatorio_previsao'>R$ ".number_format(tratanumero($_row[21]), 2, ',', '.')."</div>";
			$strcolor="#ccc";
	...
	if ($empresa <>  $_row[4] || $empresa == ''){
		...
		echo "<tr class='res' style='height:40px; background: #dddddd78;'>
				...
				<td style='width:30%'>
					...
					<div title='Previsão' class='somatorio_previsao'>R$ ".number_format(tratanumero($_row[19]), 2, ',', '.')."</div>               
		...
		if ($tipoprodserv <>  $_row[2] || $tipoprodserv == ''){
			...
			echo "	<tr class='res' style='height:40px; background: #eeeeee70;'>
						...
						<td style='width:30%'>	
							...
							<div title='Previsão' class='somatorio_previsao'>R$ ".number_format(tratanumero($_row[18]), 2, ',', '.')."</div>                  
...
<script class="normal">
	...
	function sortTable(e) {
		...
		function typed(tr) {
			...
			if($(tr).find('td[datatype="int"][col="valor"]').length > 0){
