...
<?
	...
	if($geraarquivo=='Y'){
		...
		$_sqltimbrado="select tipoimagem,caminho from empresaimagem where 1 ".getidempresa('idempresa','empresa')." and tipoimagem in ('".$_timbradoheader."')";
		...
		echo '<div id="_timbradocabecalho" style="margin-bottom: 10px;"><img src="'.$_figtimbrado["caminho"].'" height="100px" width="100%"></div>';
	}
...
<tr>	
    <td class="header" colspan="2" nowrap></td>      
</tr>
</table>
<HR>
...
<tr class="res" >
	...
	<td align="right" nowrap>Part. Interna:</td>
	<td style="min-width:150px;" nowrap><?=$_1_u_lote_partida?>/<?=$_1_u_lote_exercicio?></td>
</tr>
<tr class="res">
    <td align="right" nowrap>Qtd Produzida:</td>
	<td nowrap><?=recuperaExpoente(tratanumero($_1_u_lote_qtdprod),$_1_u_lote_qtdprod_exp)?>-<?=traduzid("prodserv","idprodserv","un",$_1_u_lote_idprodserv)?></td>

          <td align="right" nowrap>Qtd Pedida:</td>
	<td nowrap><?=recuperaExpoente(tratanumero($_1_u_lote_qtdpedida),$_1_u_lote_qtdpedida_exp)?>-<?=traduzid("prodserv","idprodserv","un",$_1_u_lote_idprodserv)?></td>
...
?>
        ...
        <td align="right" nowrap>Qtd Disponivel:</td>
	<td nowrap><?=recuperaExpoente(tratanumero($rowf['qtd']),$rowf['qtd_exp']);?>-<?=$unidade?></td>
...
<p>&nbsp;</p>
<?if(!empty($_1_u_lote_observacao)){?>
<HR>
<table  style="font-size: 11px;">
	<tr >
		<td align="right">Observação:</td>
	
		<td colspan="5" style="width:450px;" ><?=nl2br($_1_u_lote_observacao)?></td>
	</tr>
...
<table  style="font-size: 11px;">
    <tr > 
		<td align="right">Observação Análise</td>
...
if($geraarquivo=='Y'){
	...
	if($gravaarquivo=='Y'){
    	...
    	file_put_contents("/var/www/laudo/tmp/nfe/Lote-".$row['codprodserv']."-part(".$row['npart'].").pdf",$output);
    	echo("OK");
	}else{
