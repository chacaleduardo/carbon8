...
//print_r($arrRep);
	...
	$conteudoexportaux;// guarda o conteudo para exportar para csv
	$strtabheader = "\n<tr class='header'>";
...
//	echo  $_i.' - '.$_numcolunas;
	while ($_i < $_numcolunas) {
	   ...
	   // if($arrRep["_filtros"][$_metacmp->name]["visres"] == 'Y'){
			...
			$strtabheader .= "<td class='header ".seo_friendly_url(str_replace('`','',$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["rotulo"]))."' style=\"text-align:".$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["align"]."\">" . str_replace('`','',$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["rotulo"])."</td>";
	    }	
		if(!empty($arrRep["_filtros"][$_metacmp->name]["rotulo"]) and $_metacmp->name != "tipo"){
			$conteudoexportaux.= "\"".$arrRep["_filtros"][$_metacmp->name]["rotulo"]."\"";// GRAVA O ROTULO DOS CABEÇALHOS NA VARIAVEL PARA GERAR O CSV
		}else if($_metacmp->name != "tipo"){
			$conteudoexportaux.= "\"".str_replace('`','',$arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["rotulo"])."\"";// GRAVA O ROTULO DOS CABEÇALHOS NA VARIAVEL PARA GERAR O CSV
		}
    ...
    $qr = "SELECT count(*) as totalfunc FROM pessoa WHERE status = 'ATIVO' AND idtipopessoa = 1 ".getidempresa('idempresa', 'funcionario');
    $rs = d::b()->query($qr);
    $rw = mysqli_fetch_assoc($rs);
    $totalfunc = $rw["totalfunc"];

    while ($_row = mysql_fetch_array($_resultados)){

        if(!empty($_row["dataadmissaodemissao"]) and $_row["tipo"] == 'ADMISSÃO'){
            $totaladmissao++;
        }else if(!empty($_row["dataadmissaodemissao"]) and $_row["tipo"] == 'DEMISSÃO'){
            $totaldemissao++;
        }

        if($tipo != $_row["tipo"]){
            $tipo = $_row["tipo"];
            $mostrar = '<tr class="cab" tipo="'.$tipo.'" style="background-color:#eee; font-size:15px; text-align: center;"><td colspan="'.($_numcolunas-1).'" ><b>Relatório de '.$tipo.'</b></td></tr>';
            $cab = true;
        }else{
			$mostrar = '';
            $cab = false;
    ...
    echo $mostrar;
    if($cab){
        $conteudoexport .= "Relatório de ".$tipo."\n";
        $conteudoexport .= $conteudoexportaux;
        echo $strtabheader;
    }
	...
	if($_numlinha == "Y"){
		...
		$_strvlrhtml="";
    		...
    		$_corfontfim="";
	    	//Escreve Campo
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y' and $_nomecol != "tipo"){
...
$turnover = round((($totaladmissao + $totaldemissao)/(2*$totalfunc))*100,2);
?>

    <table style="width: 40%;" class="normal">
        <tr class="res">
            <td>Total Admissões:</td>
            <td><?=$totaladmissao?></td>
        </tr>
        <tr class="res">
            <td>Total Demissões:</td>
            <td><?=$totaldemissao?></td>
        </tr class="res">
        <tr class="res">
            <td>Total Funcionários:</td>
            <td><?=$totalfunc?></td>
        </tr>
        <tr class="res">
            <td>Turnover:</td>
    ...
    <?
        $conteudoexport .= "\n\n;;;;;;Total Admissões:;".$totaladmissao;
        $conteudoexport .= "\n;;;;;;Total Demissões:;".$totaldemissao;
        $conteudoexport .= "\n;;;;;;Total Funcionários:;".$totalfunc;
        $conteudoexport .= "\n;;;;;;Turnover:;".$turnover."%\n";
    ?>
    ...
    <script>
        $(".dataadmissaodemissao").each((i,o)=>{
            let tipo = $($(o).parent().siblings("[tipo]")[i]).attr('tipo').toLowerCase();
            o.innerText = "Data de "+tipo;
        });
    </script>
</body>
</html>
