...
if (!empty($_GET)){
	...
	if(!empty($arrRep["_datas"])){
		...
		if ($data1 and $data2){
			while (list($ko, $vo) = each($arrRep["_datas"])) {
                //maf230622: gambiarra para poder remover da consulta vwetl_dash a instrucao "- interval 1 day": estava causando sobrecarga na cpu
                //maf230622: @todo: A arquitetura da solução tem falha: corrigir
                if($vo=="criacao"){
                        $tdata1=evaltipocoldb($_tab, $vo, 'datetime', $data1);
                        $tdata2=evaltipocoldb($_tab, $data2, 'datetime', $data2);
                        $_sqldata = $_or . "(" . $vo . " between " . $tdata1 ." and date_sub(" . $tdata2 . ", INTERVAL 1 DAY))";
                        $_or = " or ";
                }else{
                    //maf230622: original
	...
	if(!empty($arrRep["_colvisiveis"])){
		...
		$strselectfields = "select ".$strselectfields.",idobjeto,itensjson "; 
...
//	echo  $_i.' - '.$_numcolunas;
	while ($_i < $_numcolunas) {
		...
		if($arrRep["_filtros"][$arrRep["_colvisiveis"][$_i+1]]["visres"] == 'Y' && $arrRep["_colvisiveis"][$_i+1] != 'idempresa'){
	...
	if($_numlinha == "Y"){
		...
		$_strvlrhtml="";
			...
			if($_nomecol == 'criacao' and empty($iddash)){
				$iddash = $_row[$_i];
				if(empty($_arrayNovo[$iddash])){
					$_arrayNovo[$iddash] = "";
					$virgula = "";
				}else{
					$_arrayNovo[$iddash] = $_arrayNovo[$iddash];
					$virgula = ",";
				}
			}
			if($_nomecol == 'criacao' and !empty($iddash) and $iddash != $_row[$_i]){
				$iddash = $_row[$_i];
				if(empty($_arrayNovo[$iddash])){
					$_arrayNovo[$iddash] = "";
					$virgula = "";
			...
			if($_nomecol == 'criacao' and empty($iddash)){
				$iddash = $_row[$_i];
				if(empty($_arrayDT[$iddash])){
					$_arrayDT[$iddash] = "";
					$virgula = "";
				}else{
					$_arrayDT[$iddash] = $_arrayDT[$iddash];
					$virgula = ",";
				}
				if(empty($_arrayDT[$iddash])){
					$_arrayDT[$iddash] = "";
			...
			if($_nomecol == 'itensjson' or $_nomecol == 'jadd'){
				
				$json = unserialize(base64_decode($_row[$_i]));
				if($_nomecol == 'itensjson'){
					foreach ($json as $key1 => $value1) {
						foreach ($value1 as $key2 => $value2) {
							foreach ($value2 as $key3 => $value3) {
								if($value3['card_url']){
									$p = strpos($value3['card_url'],"[");
									if($p){
										$_arrayNovo[$iddash] .= $virgula . substr($value3['card_url'], $p+1,-1);
										$virgula = ",";
										unset($p);
									}
								}
					...
					foreach ($json as $key1 => $value1) {
						foreach ($value1 as $key2 => $value2) {
							foreach ($value2 as $key3 => $value3) {
								if($value3['concatidmodulo']){
									$_arrayDT[$iddash] .= $virgula2 . $value3['concatidmodulo'];
								}
							}
						}
					}
...
foreach ($arrayGrafico as $key => $value) {
	...
	foreach($_arrayNovo as $key1 => $value1){
		if($value['criacao'] == $key1){

			$val = array_unique(array_filter(explode(',',$value1)),SORT_NUMERIC);
			$valontem = array_unique(array_filter(explode(',',$valueontem)),SORT_NUMERIC);

			$saida= count(array_diff($valontem,$val));
			$entrada = count(array_diff($val,$valontem));
			...
			$qr = "SELECT 
				...
				FROM
					etl e
				WHERE
					e.objeto = 'vwfluxostatushist_dash'
						AND e.criacao = '".$value['criacao']."'
						AND e.idetlconf = 8
						AND e.idempresa = ".$value['idempresa']."
						AND EXISTS( SELECT 
							1
						FROM
							dashcard dc
								LEFT JOIN
							etapa et ON ((dc.tipoobjeto = 'etapa')
								AND (dc.objeto = et.idetapa))
								LEFT JOIN
							fluxostatus fs ON (((dc.tipoobjeto = 'fluxostatus')
								AND (dc.objeto = fs.idfluxostatus))
								OR (et.idetapa = fs.idetapa))
						WHERE
							dc.iddashcard = ".$_GET["idobjeto"]."
								AND e.idobjeto = fs.idfluxostatus)";
								
			$rs = d::b()->query($qr);
			$rw = mysqli_fetch_assoc($rs);
			if($rs && !empty($rw['itensjson'])){
				$json = unserialize(base64_decode($rw['itensjson']))['dados'];
				foreach ($json as $key2 => $value2) {
					foreach ($value2 as $key3 => $value3) {
						if($value3['concatidmodulo']){
							$virg = (empty($_arrayDT[$key1])) ? "" : ",";
							$_arrayDT[$key1] .= $virg . $value3['concatidmodulo'];
						}
					}
				}
			}

			if(!empty($_arrayDT[$key1])){
				$fshist = array_unique(array_filter(explode(',',$_arrayDT[$key1])));
				$dif   = count(array_diff($fshist,$val));
			}else{
				$dif   = 0;
			}


			$arrayGrafico[$key]['diferenca']['entrada']=$entrada+$dif;
			$arrayGrafico[$key]['diferenca']['saida']=$saida+$dif;
		}
		$keyontem = $key1;
		$valueontem = $value1;
	}
