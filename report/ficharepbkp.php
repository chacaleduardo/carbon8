...
<?
    ...
    if(!empty($row["idespeciefinalidade"])){
        $especie=traduzid("especiefinalidade","idespeciefinalidade","especie",$row["idespeciefinalidade"]);
    }
    if($especie=="Aves"){
        $fase1='Incubadora';
        $fase2='Pinteiro';
...
<pagina class="ordContainer">
   ...
   <div class="row">
        ...
        <div class="col 15"><?=dma($row["inicio"])?></div>
...
<?
if(!empty($row["utilizados"]) and !empty($row["qtd"]) and  $especie=="Aves"){
...
<?
    $sqll="select idbioensaio,qtd,
                    ...
                    dma(fimbio) as fbio,
                    obs
                    from vwreservabioensaio where idlote = ".$row['idlote'];
