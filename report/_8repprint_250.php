...
if (!empty($_GET)){
	...
	if($_REQUEST['idagencia']){
		$str_fts.=" and idagencia in (".$_REQUEST['idagencia'].") ";
	}

	if($_REQUEST['idempresa']){
		$str_fts.=" and idempresa in (".$_REQUEST['idempresa'].") ";
	}
	/****************************************************************************
	...
	$relatorios = MenuRelatorioController::buscarRelatorioFluxoDeCaixaPorClausula($_sqldata.$_and_idempresa.$str_fts.$strgrp.$strord);
...
<?
	...
	{
		if ($_row['agencia'] != $_idagencia)
		{
			if (!empty($_rowanterior['agencia'])){
				$datarow = $ndata = implode("-", array_reverse(explode("/", $data[1])));
				$datacorrida->format('Y-m-d');
				//echo '-'.$datarow.'<br>';
				$fim = true;
				$_idagencia = $_row['agencia'];
			}else{
				$ndata = implode("-", array_reverse(explode("/", $_row['datareceb'])));

				$datacorrida = new DateTime($ndata);
				$_idagencia = $_row['agencia'];
				$datarow = $_row['datareceb'];
				$fim = false;
		...
		while ($datarow > $datacorrida->format('Y-m-d'))
		{
			$_i = 0;
			...
			if ($j == 10)
			{
				die('erro'.$datacorrida->format('Y-m-d').$datarow);
			}
			$_rowanterior['datareceb'] = $datacorrida->format('Y-m-d');
			$_rowanterior['credito'] = 'R$ 0,00';
			$_rowanterior['debito'] = 'R$ 0,00';
			$_rowanterior['dif'] = 'R$ 0,00';
		
			
			###################################### Escreve linhas da <Table>
			
				echo "\n<tr style=\" background: #fcf8e3\"class=\"res\" ". $_link ." ". $_strhlcolor . ">";
			...
			foreach(array_keys($_row) as $key => $coluna)
			{
				...
				if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
					...
					if($arrRep["_filtros"][$_nomecol]["acsum"]=='Y'){
						...
						$_attrHtml = "acsum='$_nomecol' filtervalue='$_rowanterior[$coluna]'";

						$_arrsoma[$_tab][$_nomecol] = $_arrsoma[$_tab][$_nomecol] + $_rowanterior[$coluna];
					}
					
					//se for para somar o valor do campo
					if($arrRep["_filtros"][$_nomecol]["acavg"]=='Y'){
						
						$_arrsomaavg[$_tab][$_nomecol] = $_arrsomaavg[$_tab][$_nomecol] + $_rowanterior[$coluna];
					...
					if(empty($arrRep["_filtros"][$_nomecol]["datatype"])){
						$t = preg_replace("/[^0-9.]/", "",$_rowanterior[$coluna]);
						($t != $_rowanterior[$coluna]) ? $arrRep["_filtros"][$_nomecol]["datatype"]="varchar" : $arrRep["_filtros"][$_nomecol]["datatype"]="double";
					}
					...
					if($arrRep["_filtros"][$_nomecol]["datatype"]=='longtext'){
						$_strvlrhtml = nl2br($_rowanterior[$coluna]);
					}elseif($arrRep["_filtros"][$_nomecol]["datatype"]=='datetime'){
						$_strvlrhtml = validadatadbweb($_rowanterior[$coluna]);
					}elseif($arrRep["_filtros"][$_nomecol]["datatype"]=='date'){
						$_strvlrhtml = dma($_rowanterior[$coluna]);
					}elseif($arrRep["_filtros"][$_nomecol]["datatype"]=='decimal' || $arrRep["_filtros"][$_nomecol]["datatype"]=='double'){
						$graficoY=$_rowanterior[$coluna];
						$_strvlrhtml = number_format($_rowanterior[$coluna], 2, ',','.');
					}else{
						$_strvlrhtml = $_rowanterior[$coluna];
					}
					...
					$arrayGrafico[$_graphLinha][$_nomecol] = $_rowanterior[$coluna];
					
					if (is_numeric($_rowanterior[$coluna])){
						$total[$coluna] = $total[$coluna] + $_rowanterior[$coluna];
					}
					...
					if($arrRep["_filtros"][$_nomecol]["mascara"] == 'MOEDA'){
						$conteudoexport.=strip_tags($_rowanterior[$coluna]);
					} else {
					...
					if(!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])){
						if ($_strvlrhtml != '0.00'){
							...
							if (strpos($arrRep["_filtros"][$_nomecol]["hyperlink"].$_rowanterior[$coluna] , 'pk=')){
								
								$arrRep["_filtros"][$_nomecol]["hyperlink"].$_rowanterior[$coluna];
								$valor = explode('pk=',$arrRep["_filtros"][$_nomecol]["hyperlink"].$_rowanterior[$coluna]);
								$valor = explode('&',$valor[1]);
								$campo = $_rowanterior[$valor[0]];
								
								$_hyperlink="<a target=_blank href='/".$arrRep["_filtros"][$_nomecol]["hyperlink"].$campo."'>".$_strvlrhtml."</a>";
							}else{
								$_hyperlink="<a target=_blank href='/".$arrRep["_filtros"][$_nomecol]["hyperlink"].$_rowanterior[$coluna]."'>".$_strvlrhtml."</a>";
							}
