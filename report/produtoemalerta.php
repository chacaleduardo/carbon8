...
$sql=$prodservclass->getProdservAlerta($idunidadepadrao,'PRODUTO');
$sql.=$ord;
...
<style>
    ...
    * {
	...
	<div class="row margem 0.0">
            ...
            <!-- div class="titulodoc nowrap">PRODUTOS EM ALERTA - <span id="qtd"></span></div-->
...
while ($row=mysqli_fetch_assoc($res)){
    //echo var_dump($row);
    //$estoque=$row['total']+$row['quar'];
    $estoque=$row['total'];//
    //buscando o idunidadeest da prodserv
    $idunidadeest=traduzid('prodserv','idprodserv','idunidadeest',$row['idprodserv']);
    
    //usar o idunidadeest da prodserv ou o idunidadeest padrão do modulo caso não esteja preenchido na prodserv
    //no calculo da media diaria
    if($idunidadeest)
        $mediadiaria=$prodservclass->getMediadiaria($row['idprodserv'],$idunidadeest);
    else
        $mediadiaria=$prodservclass->getMediadiaria($row['idprodserv'],$idunidadepadrao);
    
    //calculando dias em estoque
    if($mediadiaria>0 and $estoque>0 ){
        $diasestoque=$estoque/$mediadiaria;
    }else{
        $diasestoque='0.00';	
    }
    ...
    if($row["ordem"] != '5' and $row["ordem"] != '9' ){  
	...
	if($row["entrada"] == 'MANUAL'){
		...
		$sqlcot="SELECT DISTINCT(c.idcotacao) AS idcotacao, 
                        DATE_FORMAT(c.prazo,'%Y-%m-%d') AS prazo, 
                        DATE_FORMAT(n.previsaoentrega,'%Y-%m-%d') AS previsaoentrega,		 
                        c.status AS statusorc,
                        n.status AS statuscot,
                        pf.pedidoautomatico,
                        pf.pedido_automatico
                   FROM nfitem i JOIN nf n ON i.idnf = n.idnf
                    JOIN cotacao c ON n.idobjetosolipor = c.idcotacao  AND n.tipoobjetosolipor = 'cotacao' 
                    JOIN prodserv p ON i.idprodserv = p.idprodserv
                    JOIN prodservformula pf ON p.idprodserv = pf.idprodserv
    ...
    if( $row['ordem']==7 or $row['ordem']==1 or  $row['ordem']==2 or  $row['ordem']==3 or $row['ordem']==5 or $row['ordem']==6  ){
        ...
        if($i==0){
            ...
            <?
                if ($divordem != $row["ordem"]){
                    ...
                    ?>
                    <div class="panel-heading nowrap">
                    <?if($row["ordem"] == '2'){echo("Produto com Orçamento insuficiente - <span id='var2'></span>"); }
                    elseif($row["ordem"] == '1'){echo("Produto com Orçamento em Andamento - <span id='var1'></span>"); }
...
<?if($i==0){?><div class="titulogrupo">Descrição</div><?}?>
		...
		<div class="col grupo 20 quebralinha" style="text-align:center;">
<?if($i==0){?><div class="titulogrupo"><a href="./?_modulo=<?=$_modulo?>&ordenacao=<? if ($_GET['ordenacao'] == 'minimo'){echo 'minimod';}else{echo 'minimo';}?>">Est. Mín. <? if ($_GET['ordenacao'] == 'minimo'){echo '<i class="fa fa-sort-down"></i>';}elseif ($_GET['ordenacao'] == 'minimod'){echo '<i class="fa fa-sort-up"></i>';}?></a></div><?}?>
        ...
        <!-- div class="col grupo 20 quebralinha" style="text-align:center;">
			<?if($i==0){?><div class="titulogrupo"><a href="./?_modulo=<?=$_modulo?>&ordenacao=<? if ($_GET['ordenacao'] == 'pedidoautomatico'){ echo 'pedidoautomatico';}?>">PA 1<? if ($_GET['ordenacao'] == 'pedidoautomatico'){echo '<i class="fa fa-sort-down"></i>';}elseif ($_GET['ordenacao'] == 'pedidoautomatico'){echo '<i class="fa fa-sort-up"></i>';}?></a></div><?}?>                             
			<?=number_format(tratanumero($row["pedidoautomatico"]), 2, ',', '.');?>
        </div!-->
		<!--div class="col grupo 20 quebralinha" style="text-align:center;">
			<?if($i==0){?><div class="titulogrupo"><a href="./?_modulo=<?=$_modulo?>&ordenacao=<? if ($_GET['ordenacao'] == 'pedido_automatico'){echo 'pedido_automatico';}else{echo 'pedido_automatico';}?>">PA 2<? if ($_GET['ordenacao'] == 'pedido_automatico'){echo '<i class="fa fa-sort-down"></i>';}elseif ($_GET['ordenacao'] == 'pedido_automatico'){echo '<i class="fa fa-sort-up"></i>';}?></a></div><?}?>
			<?=number_format(tratanumero($row["pedido_automatico"]), 2, ',', '.');?>
		</div!-->
		<div class="col grupo 20 quebralinha" style="text-align:center;">
<?if($i==0){?><div class="titulogrupo"><a href="./?_modulo=<?=$_modulo?>&ordenacao=<? if ($_GET['ordenacao'] == 'maximo'){echo 'maximod';}else{echo 'maximo';}?>">Ped. Aut.<? if ($_GET['ordenacao'] == 'maximo'){echo '<i class="fa fa-sort-down"></i>';}elseif ($_GET['ordenacao'] == 'maximod'){echo '<i class="fa fa-sort-up"></i>';}?></a></div><?}?>
			<?=number_format(tratanumero($row["pedido"]), 2, ',', '.');?>
		</div>
        <div class="col grupo 15 quebralinha" title="<?php echo "(".$mediadiaria." * ".$row['tempocompra'].") + (".$row["estmin"]." - ".$estoque.")";?>">
        <?if($i==0){?><div class="titulogrupo">Sug. Compra</div><?}?>
        <?
        $sugcp=tratanumerovisualizacao(($mediadiaria * $row['tempocompra'] ) + ($row["estmin"] - $estoque));
        ?>
        <?=number_format(tratanumero($sugcp), 2, ',', '.');?>
		</div>

<? 

            if($row['entrada'] == 'MANUAL' and !empty($row['idnf'])  and !empty($row['descr'])){
                $sql_pe="select qtd as qtdpa  from nfitem 
                    where idnf = ".$row['idnf']." 
                    and prodservdescr like '%".$row['descr']."%'";

            }else{
                $sql_pe="select 
                ifnull(sum(i.qtd * if(pf.valconv>0, pf.valconv,1)),0) as qtdpa		
                from nfitem i 
                join nf n on(n.idnf=i.idnf and n.tiponf !='V' and status not in('REPROVADO','CANCELADO','ABERTO','CONCLUIDO'))
...
?>   <!--div title="Soma da quantidade solicitada em orçamento." class="col grupo 20 quebralinha" style="text-align:center;">
        <?if($i==0){?><div title="Soma da quantidade solicitada em orçamento." class="titulogrupo">Qtd. Sol.</div><?}?>
			<?//=number_format(tratanumero($_rowpa["qtdpa"]), 2, ',', '.');?>
		</div--> 
        <div title="Dias em Estoque. <?=$estoque."/".$mediadiaria?> " class="col grupo 20 quebralinha" style="text-align:center;">
<?if($i==0){?><div title="Dias em Estoque." class="titulogrupo">Dias Estoque</div><?}?>
			<?=number_format(tratanumero($diasestoque), 2, ',', '.')?>
		</div>         
        <div class="col grupo 40 quebralinha" style="text-align:center;">
<?if($i==0){?><div class="titulogrupo"><a href="./?_modulo=<?=$_modulo?>&ordenacao=<? if ($_GET['ordenacao'] == 'data'){echo 'datad';}else{echo 'data';}?>">Data Entrada<? if ($_GET['ordenacao'] == 'data'){echo '<i class="fa fa-sort-down"></i>';}elseif ($_GET['ordenacao'] == 'datad'){echo '<i class="fa fa-sort-up"></i>';}?></a></div><?}?>
...
<?
        if(!empty($row["idnf"])){
            $rotulo = getStatusFluxo('cotacao','idcotacao', $row["idcotacao"])  
?>
            <a title="<?$titulomodulo?>" target="_blank" href="./?_modulo=nfentrada&_acao=u&idnf=<?=$row["idnf"]?>"><?=$row["idcotacao"]?>-<?=$rotulo['rotulo']?></a>
<?
        }elseif(!empty($row["idcotacao"])){
            $rotulo = getStatusFluxo('cotacao','idcotacao', $row["idcotacao"])                                              
    ?>
                <a title="<?$titulomodulo?>" target="_blank" href="./?_modulo=cotacao&_acao=u&idcotacao=<?=$row["idcotacao"]?>"><?=$row["idcotacao"]?>-<?=$rotulo['rotulo']   ?></a>
    <?
		}else{
?>
...
<?
    ...
    d::b()->query($sql) or die("Falha ao Atualizar Dashboard 36: ".mysqli_error(d::b())."\n".$sql);
...
CB.montaLegenda({"#FF8491": "Produto Pendente.", "#9DDBFF": "Produto em Andamento."});
