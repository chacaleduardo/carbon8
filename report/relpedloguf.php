...
if (!empty($_GET)){
	...
	while (list($_col, $_val) = each($_GET)) {
			...
			if($_iclausulas > 0){
				...
				$_sqlresultado .= " where idempresa =".$_SESSION["SESSAO"]["IDEMPRESA"];
			}
			if (trim($_compl) != ''){
							...
							$strtabheader = $strtabheader;
								...
								*/
								...
								while ($_row = mysql_fetch_array($_resultados)){
									...
									if($uf != $_row[9]){
										$linhasoma =  '<tr style="background-color:#eee; height: 20px; font-size:10px"><td colspan="6" align="right" ><b> SOMA QTD. VOL: </b> '.$sqtdvol.'    </td><td colspan="5">    <b>SOMA PESO BRUTO:</b>    '.$spbruto.' </td> </tr>';
										$mostrar = '<tr style="background-color:#eee; height: 20px; font-size:10px">  <td colspan="10">    <b>UF:</b>    '.$estado. '  <td align="right"><b>R$  '.number_format($vtotal, 2, ',','.').'</b></td></tr>';
										$linhadiv = '<tr style="background-color:#eee; height: 20px;"> </tr>'.$strtabheader;
										$vtotal = 0;
										$sqtdvol = 0;
										$spbruto = 0;
										$idnf = $_row[0];
...
<?
 $linhasoma =  '<tr style="background-color:#eee; height: 20px; font-size:10px"><td colspan="6" align="right" ><b> SOMA QTD. VOL: </b> '.$sqtdvol.'    </td><td colspan="5">    <b>SOMA PESO BRUTO:</b>    '.$spbruto.' </td> </tr>';
 $mostrar = '<tr style="background-color:#eee; height: 20px; font-size:10px">  <td colspan="10">    <b>UF:</b>    '.$estado. ' - '. $uf.' <td align="right"><b>R$  '.number_format($vtotal, 2, ',','.').'</b></td></tr>';
	echo $linhasoma;
	echo $mostrar;
	echo $linhadiv;
 	if(!empty($_arrsoma)){
