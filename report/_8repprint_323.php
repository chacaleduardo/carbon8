...
<?
	...
	$strtabheader = "<thead>
    <tr class='header'>
    <td class='header' style='white-space: nowrap; text-align:left'>Unidade</td>
    <td class='header' style='white-space: nowrap; text-align:left'>Entrada</td>
	<td class='header' style='white-space: nowrap; text-align:left'>Valor Entrada</td>
    <td class='header' style='white-space: nowrap; text-align:left'>Saída</td>
    <td class='header' style='white-space: nowrap; text-align:left'>Valor Saída</td>
    <td class='header' style='white-space: nowrap; text-align:left'>Descarte</td>
	<td class='header' style='white-space: nowrap; text-align:left'>Valor Descartado</td>
	<td class='header' style='white-space: nowrap; text-align:left'>Saldo</td>
    </tr>";

	$strtabheaderdes = "<thead>
	<tr class='header'>
		<td class='header' style='white-space: nowrap; text-align:left' colspan='6'>HISTÓRICO DO DESCARTE</td>
	</tr>
    <tr class='header'>
    	<td class='header' style='white-space: nowrap; text-align:left'>Quantidade do Insumo Descartado</td>
		<td class='header' style='white-space: nowrap; text-align:left'>Valor Descartado</td>
      	<td class='header' style='white-space: nowrap; text-align:left'>Produto Descartado</td>		  
		  <td class='header' style='white-space: nowrap; text-align:left'>Observação</td>
    	<td class='header' style='white-space: nowrap; text-align:left'>Por</td>
		<td class='header' style='white-space: nowrap; text-align:left'>Data</td>
    </tr>";
    ...
    $sqlun="select idunidade,unidade from unidade where idempresa in (".$_REQUEST['idempresa'].") and idtipounidade in (3,8,5,21,12,7,2,5,11,13) and status ='ATIVO' and primtipounidade='Y'";
    ...
    while ($_rowun = mysql_fetch_assoc($_resun)){
		...
		$sd="select round(ifnull(sum(qtd),0),2) as qtd, round(ifnull(sum(valor),0),2) as valor  from (
			...
			$reldesc=$reldesc.$union."
			select round(sum((c.qtdd*i.qtd)/l.qtdprod),2)  as qtd,round(sum((c.qtdd*i.qtd)/l.qtdprod)*(i.valorun),2) as valor,l.idlote,l.partida,l.exercicio,c.qtdd,c.qtdd_exp,c.obs,dmahms(c.criadoem) as criadoem,c.criadopor,p.un,o.idobjeto
				from lotecons c 
					join lotefracao f on(f.idlotefracao = c.idlotefracao and f.idunidade  = ".$_rowun['idunidade'].")
					join lote l on(l.idlote=f.idlote)
				...
				where c.criadoem   ".$_sqldata." 
					and c.status!='INATIVO'
                    and c.qtdd >0
					and c.idempresa  in (".$_REQUEST['idempresa'].")  and c.idobjeto is  null group by c.idlotecons
				union  
				select c.qtdd as qtd,(c.qtdd*l.vlrlote) as valor,l.idlote,l.partida,l.exercicio,c.qtdd,c.qtdd_exp,c.obs,c.criadoem,c.criadopor,p.un,o.idobjeto
                from lotecons c
					join lotefracao f on(f.idlotefracao = c.idlotefracao and f.idunidade  = ".$_rowun['idunidade'].")
				 	join lote l on(l.idlote=f.idlote and l.status !='CANCELADO' and l.idprodserv in (".$rowpd['idprodserv']."))
                     join prodserv p on(p.idprodserv =l.idprodserv)
				...
				where c.criadoem  ".$_sqldata." 
					...
					and c.idempresa in (".$_REQUEST['idempresa'].")  and c.idobjeto is  null
			";
			$saldo = $wc['valor'] - $wd['valor'] - $wdes['valor'];
?>  
    <tr class="res">
        <td><?=$_rowun['unidade']?></td>
        <td><?=number_format(tratanumero($wc['qtd']), 2, ',', '.')?> - <?=$rowpd['un']?></td>
		<td>R$ <?=number_format(tratanumero($wc['valor']), 2, ',', '.')?> </td>
        <td><?=number_format(tratanumero($wd['qtd']), 2, ',', '.')?> - <?=$rowpd['un']?></td>
		<td>R$ <?=number_format(tratanumero($wd['valor']), 2, ',', '.')?> </td>
		<td><?=number_format(tratanumero($wdes['qtd']), 2, ',', '.')?> - <?=$rowpd['un']?></td>
		<td>R$ <?=number_format(tratanumero($wdes['valor']), 2, ',', '.')?> </td>
		<td>R$ <?=number_format(tratanumero($saldo), 2, ',', '.')?> </td>
	</tr>
<?
		$descarte=$descarte+$wdes['qtd'];
		$Vdescarte=$Vdescarte+$wdes['valor'];
		$union=' union ';
    }
?>
 	<tr class="res">
        <td colspan="5">TOTAL</td>     
		<td><?=number_format(tratanumero($descarte), 2, ',', '.')?> - <?=$rowpd['un']?></td>
		<td>R$ <?=number_format(tratanumero($Vdescarte), 2, ',', '.')?> </td>
		<td></td>
	</tr>
 </table>
...
<?
		...
		$sqlrd="select * from (".$reldesc." ) as u order by valor desc;";
		echo("<!-- historico ".$sqlrd." -->");
	
		$resrd = d::b()->query($sqlrd);
		$qtdr=mysqli_num_rows($resrd);
		if($qtdr>0){
			echo $strtabini;
			echo $strtabheaderdes;
			while($rowrd=mysqli_fetch_assoc($resrd)){
				if(!empty($rowrd['qtdd_exp'])){
					$numero=recuperaExpoente(tratanumero($rowrd['qtdd']), $rowrd['qtdd_exp']); 
				}else{
					$numero=number_format(tratanumero($rowrd['qtdd']), 2, ',', '.');
				}
?>  
		<tr class="res">
			<td> <?=number_format(tratanumero($rowrd['qtd']), 2, ',', '.')?> - <?=$rowpd['un']?></td>	
			<td>R$ <?=number_format(tratanumero($rowrd['valor']), 2, ',', '.')?></td>		
			<td>
				<?=$numero?> - <?=$rowrd['un']?> 
				<a target="_blank" href="/?_modulo=<?=$rowrd['idobjeto']?>&_acao=u&idlote=<?=$rowrd['idlote']?>">
				<?=$rowrd['partida']?>/<?=$rowrd['exercicio']?>
				</a>
			</td>
			<td><?=$rowrd['obs']?></td>			
			<td><?=$rowrd['criadopor']?></td>
			<td><?=$rowrd['criadoem']?></td>
		</tr>
<?
			}
?>
