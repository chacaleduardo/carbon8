...
<?
    ...
    while ($_row = mysql_fetch_array($_resultados)){
		...
		if(empty($especie)){
			...
			$vendedor=$_row[2];
                ...
                if($vendedor!=$_row[2]){
			$mostrar2 = '<tr style="background-color:#eee; font-size:10px"><td colspan="'.($_numcolunas-4).'" ><b></td><td align="right"></td><td align="right"><b>'.$vendedor.'</b></td><td align="right"><b>'.number_format($vtotalvend, 2, ',','.').'</b></td></tr>';
			$mostrar2 .= $cliente_faturamento;
			$vtotalvend=0;
			$cliente_faturamento="";
		...
		if($especie!=$_row[1]){
			$mostrar = '<tr style="background-color:#eee; font-size:10px"><td colspan="'.($_numcolunas-4).'" ><b>TOTAL '.$especie.'</td><td align="right"></td><td align="right"><b></b></td><td align="right"><b>'.number_format($vtotal, 2, ',','.').'</b></td></tr>';
			$vtotal=0;
			$vqtd = 0;
			$vicmsdeson=0;
			$fretemtotal=0;
		...
		$vtotal	=	$vtotal + $_row['total'];
                $vtotalvend=$vtotalvend+$_row['total'];
	...
	if($_showtotalcounter == "Y"){
		$cliente_faturamento .= "<td class='tdcounter'>".$_ilinha."</td>";
	}
	if($_numlinha == "Y"){
		...
		$_strvlrhtml="";
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
				...
				if(!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])){
					$cliente_faturamento .= "<td>".$_hyperlink."</td>";
				}else{
					$cliente_faturamento .= "<td ".$_stralign." ".$_hyperlink." ".$_colorlink.">".$_corfont.$_strvlrhtml.$_corfontfim."</td>";
				}
	...
	echo $cliente_faturamento;
?>
		</tr>
<?

        $mostrar2 = '<tr style="background-color:#eee; font-size:10px"><td colspan="'.($_numcolunas-4).'" ><b></td><td align="right"><b></b></td><td align="right"><b>'.$vendedor.'</b></td><td align="right"><b>'.number_format($vtotalvend, 2, ',','.').'</b></td></tr>';

        echo $mostrar2;
        
	$mostrar = '<tr style="background-color:#eee; font-size:10px"><td colspan="'.($_numcolunas-4).'" ><b>TOTAL '.$especie.'</td><td align="right"><b></b></td><td align="right"><b></b></td><td align="right"><b>'.number_format($vtotal, 2, ',','.').'</b></td></tr>';
