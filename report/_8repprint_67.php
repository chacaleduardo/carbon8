...
if (!empty($_GET)){
	...
	while (list($_col, $_val) = each($_GET)) {
		...
		if(!empty($_val) and ($_col != "_modulo") and ($_col != "_rep") and (substr($_col,-2) != "_2")){
		 ...
		 $_col;
			...
			if ($_entre == 'Y' and !empty($_colval1) and !empty($_colval2)){
				 $_sqldata .=  "(" . $_col . " between " . $_colval1 . " and " .$_colval2 . ") and ";
			}
	...
	if(!empty($arrRep["_orderby"])){
		...
		$strord = " order by a.exercicio, a.idregistro, ps.ordenacao".$strord; 
	}
	$strord = " order by a.exercicio, a.idregistro, ps.ordenacao".$strord; 
	$strvirg = "";
	 ...
	 ****************************************************************************/
	 ...
	 $_sqldata = str_replace('idresultado', 'r.idresultado',$_sqldata);
	 ...
	 $_sqldata = str_replace('dataamostra', 'a.dataamostra',$_sqldata);
	 $_sqldata = str_replace('datacoleta', 'a.datacoleta',$_sqldata);
	 $_sqldata = str_replace('criadoem', 'ra.criadoem',$_sqldata);
	                                                                                                                                                                                                                                                                                                                                                  
	 $strselectfields = str_replace('idresultado', 'r.idresultado',$strselectfields);
	 $strselectfields = str_replace('idregistro', 'a.idregistro',$strselectfields);
	 $strselectfields = str_replace('nucleoamostra', 'a.nucleoamostra',$strselectfields);
	 $strselectfields = str_replace('exercicio', 'a.exercicio',$strselectfields);
	 $strselectfields = str_replace('idtipoamostra', 'a.idtipoamostra',$strselectfields);
	 $strselectfields = str_replace('idsubtipoamostra', 'a.idsubtipoamostra',$strselectfields);
	 $strselectfields = str_replace('lote', 'a.lote',$strselectfields);
	 $strselectfields = str_replace('granja', 'a.granja',$strselectfields);
	 $strselectfields = str_replace('galpao', 'a.galpao',$strselectfields);
	 ...
	 $strselectfields = str_replace('resultadoalertagamb', " IF((`r`.`idtipoteste` IN (718 , 719, 720, 2000, 678)),
            IF(EXISTS( SELECT 
                        1
                    FROM
                        (`resultado` `r2`
                        JOIN `amostra` `a2` ON ((`r2`.`idamostra` = `a2`.`idamostra`)))
                    WHERE
                        ((`a`.`idregistro` = `a2`.`idregistro`)
                            AND (`r2`.`idtipoteste` = 640)
                            AND (`a`.`exercicio` = `a2`.`exercicio`)
                            AND (`a`.`idunidade` = `a2`.`idunidade`))),
                'POSITIVO',
                'AUSENTE'),
            IF((`r`.`alerta` = 'Y'),
                'POSITIVO',
                'AUSENTE')) as resultadoalertagamb",$strselectfields);
	 $strselectfields .= " , IF((`r`.`alerta` = 'Y'),
            'POSITIVO',
            'AUSENTE') AS `resultadoalerta`,
			
			 CONCAT('<a target=\"_blank\" href=\"/?_modulo=',
                `uo`.`idobjeto`,
                '&_acao=u&idamostra=',
                `a`.`idamostra`,
                '\">',
	   ...
	   $_sqlresultado = $strselectfields."  
	
	 FROM
        `resultado` `r`
        JOIN `amostra` `a` ON `r`.`idamostra` = `a`.`idamostra`
        LEFT JOIN `amostra` `tra` ON `tra`.`idamostra` = `a`.`idamostratra`
        ...
        LEFT JOIN `unidadeobjeto` `uo` FORCE INDEX (OBJ_TOBJ_IDUN) ON `uo`.`idempresa` = `r`.`idempresa`
            AND `uo`.`tipoobjeto` = 'moduloamostraun'
            AND `a`.`idunidade` = `uo`.`idunidade`
        LEFT JOIN `vwtipoteste` `t` ON `t`.`idtipoteste` = `r`.`idtipoteste` WHERE 1 and `a`.`idunidade` = 1 
		
		
		
		".$_sqldata.$_and_idempresa.$str_fts.$strgrp.$strord;
//die();
// -- and r.idtipoteste in (640, 718 , 719, 720, 2000, 678, 617, 619, 622, 624, 595, 596, 572)
