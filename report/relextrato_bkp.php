...
$sqlant = "select * from contapagar where saldo is not null and status = 'QUITADO' and idagencia=".$idagencia." and idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." order by quitadoemseg desc limit 1;";


$sql = "SELECT *,dma(datareceb) as dtreceb,CASE status    WHEN 'QUITADO' THEN 1
            WHEN 'PENDENTE' THEN 2 
            ELSE 3 END as ordem FROM contapagar where ". $clausulad." and idagencia=".$idagencia." and idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." and status != 'INATIVO' order by datareceb asc,ordem asc,tipo asc,quitadoemseg";
...
<table class="normal" style="font-size: 10px;">
	...
	$conteudoexport='"Data";"Parcela";"NF";"Pessoa";"Categoria";"Valor";"Tipo";"Status"';
	...
	while ($row = mysqli_fetch_array($res)){
		...
		and idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." order by quitadoemseg desc limit 1;";
	    ...
	    if($row["tipoobjeto"] == "nf" and !empty($row["idobjeto"] )){
			...
			$sqlf = "select n.idnf,p.nome as nome,n.tiponf,n.controle,n.nnfe,c.contaitem
                                    ...
                                    where n.idnf=".$row["idobjeto"]." and n.idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." limit 1";
			...
			$sqlf = "select p.nome as nome,n.numerorps,n.nnfe,n.idnotafiscal from notafiscal n,pessoa p where p.idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." and p.idpessoa = n.idpessoa and idnotafiscal =".$row["idobjeto"];
...
<?	
 if($row['tipoespecifico']=='AGRUPAMENTO'){
    $conteudoexport.='"'.$row["dtreceb"].'";"'.$row["parcela"].'de'.$row["parcelas"].'";"'.$descnf.'";"'.$favorecido.'";"'.$obs.'";"'. $row["valor"].'";"'.$row["tipo"].'";"'.$row["status"].'"';	
 }else{
    $conteudoexport.='"'.$row["dtreceb"].'";"'.$row["parcela"].'de'.$row["parcelas"].'";"'.$descnf.'";"'.$favorecido.'";"'.$obs.'";"'. $row["valor"].'";"'.$row["tipo"].'";"'.$row["status"].'"';
 }	
        $conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
		...
		/*if($row["status"]=="QUITADO" AND !empty($row["saldo"])){
			//ja esta quitado 
 ...
 /*               
                if($row['tipoespecifico']=='AGRUPAMENTO'){
                    $sqlx=" select i.parcela,i.parcelas,ci.contaitem,p.nome,n.nnfe,i.valor,i.status
                            from contapagaritem i left join nf n on(n.idnf=i.idobjetoorigem)
                            left join contaitem ci on (ci.idcontaitem = n.idcontaitem)
...
<?
        $conteudoexport.='"'.$row["dtreceb"].'";"'.$rowx["parcela"].'de'.$rowx["parcelas"].'";"'.$rowx['nnfe'].'";"'.$rowx['nome'].'";"'.$descnf.'-'.$favorecido.'";"'. $rowx["valor"].'";" ";"D";"'.$rowx["status"].'"';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
                    }//while($rowx=mysqli_fetch_assoc($resx)){
