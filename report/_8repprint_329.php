	...
	if(!empty($arrRep["_datas"])){
		...
		if ($data1 and $data2){
			while (list($ko, $vo) = each($arrRep["_datas"])) {
				...
				$_sqldatai= evaltipocoldb($_tab, $data1, 'datetime', $data1) . "";
...
$_sqlresultado =$strselectfields." from vw8prodservformulaitempedido where idpessoa in (".$_REQUEST['idpessoa'].") and dtemissao ".$_sqldata;
...
<?
    ...
    <td class='header' style='white-space: nowrap; text-align:left'>ESTOQUE INICIAL</td>
    ...
    <td class='header' style='white-space: nowrap; text-align:left'>CONSUMO CLIENTE</td>
    ...
    $sqlun="select descr, un, compra, estoque, perda, perdap,idprodserv,idpessoa,round(sum(produzir*qtd),2) as consumo 
            from vw8prodservformulaitempedido where dtemissao ".$_sqldata." and idpessoa  in (".$_REQUEST['idpessoa'].") 
                group by idprodserv order by descr";
	echo("<!-- inicial ".$sqlun." -->");
	...
	//comprado apos final do periodo
    ...
    //comprado apos inicio do periodo
    $scpi=" select ifnull(sum(l.qtdprod*valconvori),0) as comprado
				from nf n 
			join nfitem i on(n.idnf=i.idnf and i.idprodserv =".$_rowun['idprodserv']."  and i.nfe='Y' )
			join lote l on(l.idnfitem=i.idnfitem)
			where n.tiponf='C' 
			and n.status not in('CANCELADO','REPROVADO')
			and n.dtemissao between ".$_sqldatai." and now()";
	echo("<!-- comprado ".$scpi." -->");
	$rcpi = d::b()->query($scpi);
	$wcpi=mysqli_fetch_assoc($rcpi);	


    //consumo desde inicio
	
	$scci="select ifnull(sum(c.qtdd),0) as consumo
    ...
    join lotecons c on(c.idlotefracao =f.idlotefracao and c.status ='ABERTO' and c.criadoem between ".$_sqldatai." and now() )
    where p.idprodserv=".$_rowun['idprodserv'];
	

    echo("<!-- consumo ".$scci." -->");
    $rcci = d::b()->query($scci);
    $wcci=mysqli_fetch_assoc($rcci);	
	...
	//consumo total
	$sccT="select ifnull(sum(c.qtdd),0) as consumo
		  from prodserv p join lote l on(l.idprodserv=p.idprodserv)
		  join lotefracao f on(f.idlote=l.idlote and f.idunidade = p.idunidadeest )
		  join lotecons c on(c.idlotefracao =f.idlotefracao and c.status ='ABERTO' and c.criadoem ".$_sqldata.")
		  where p.idprodserv=".$_rowun['idprodserv'];
  
	  echo("<!-- consumo total ".$sccT." -->");
	  $rccT = d::b()->query($sccT);
	  $wccT=mysqli_fetch_assoc($rccT);	
	...
	$valor2=$wcpi['comprado']-$wcci['consumo'];
	$estoquei= ($wce['estoqueatual']-($valor2));


	if(($estoquei+$wc['compra'])==0){
		$pcompra=1;
	}else{
		$pcompra=$estoquei+$wc['compra'];
	}

	$perda =(($estoquei+$wc['compra'])-$wccT['consumo'])-$estoque;
...
?>
    <tr class="res"  style="background: <?=$cor?>;">
        ...
        <td title="ESTOQUE INICIAL"><?=number_format(tratanumero($estoquei), 2, ',', '.')?> </td>
		<td title="COMPRADO"><?=number_format(tratanumero($wc['compra']), 2, ',', '.')?> </td>
		<td title="CONSUMO"><?=number_format(tratanumero($wccT['consumo']-$_rowun['consumo']), 2, ',', '.')?> </td>
		<td title="CONSUMO CLIENTE"><?=number_format(tratanumero($_rowun['consumo']), 2, ',', '.')?></td>  
