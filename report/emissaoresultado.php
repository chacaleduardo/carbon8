...
<head>
	...
	<?
	if (defined('STDIN')) { //se estiver sendo executao em linhade comando
		require_once("/var/www/carbon8/report/reltesteinct2.php");
	} else { //se estiver sendo executado via requisicao http
...
<body>
	...
	while ($resultado = mysql_fetch_array($res)) {
		...
		$row["tipoagente"]		= $rc["resultado"]["res"]["tipoagente"];
		...
		*/
		...
		if (empty($_REQUEST['csv'])) {
			...
			if ($rowend['numero']) {
				$endereco .= "N&ordm;:" . $rowend['numero'];
			}
			if ($rowend['complemento']) {
				$endereco .= ", " . $rowend["complemento"];
			}
			if ($rowend['bairro']) {
				$endereco .= (", BAIRRO: " . ($rowend['bairro']));
			}
			if ($rowend['cep']) {
				$endereco .= (", CEP:" . formatarCEP($rowend['cep'], true));
			}
			if ($rowend['cidade']) {
				$endereco .= (", " . ($rowend['cidade']));
			}
			if ($rowend['uf']) {
				$endereco .= ("-" . $rowend['uf']);
			}
						...
						<tbody>
							<tr>
								<td style="font-size:12px !important">
									...
									<table style="display: inline; margin:4px;">
										<tr>
											<td>
											...
											?>
												<div style="width:220px;float:left;">
													<table style=" border: 1px dashed #ccc;  ">
														<tr>
															<td style="width: 210px;font-size:8px !important">
																<p>
															</td>
														</tr>
														<tr>
															<td nowrap="nowrap" style="width: 210px;font-size:8px !important">
																<font style="font-weight:bold;font-size:8px !important">TC:</font><?= $row["tc"] ?>
															</td>
														</tr>
