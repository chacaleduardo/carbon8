...
?>
	...
	<tr>
		...
		<a onclick="gerarCsvPrint('Movimentacao lote  - Unidade')" class="btbr20 no-print" id="btn_export" href="#">  Download .CSV</a>
		</td>
...
<?
	$sqlpd=" select idunidade,upper(unidade) as unidade from unidade where idunidade in (".$_REQUEST['idunidade'].")  order by unidade";
	$respd = d::b()->query($sqlpd);
	while($_rowun = mysql_fetch_assoc($respd)){
...
$strtabheader2 ="<thead>
<tr class='header'><td class='header' style='white-space: nowrap; text-align:left'>Produto</td><td class='header' style='white-space: nowrap; text-align:left'>Tipo</td>";
    $li=0;
    ...
    $sqlun="  select p.idprodserv,p.descr,p.un from prodserv p where p.idempresa = ".$_REQUEST['idempresa']." and p.comprado='Y' and status ='ATIVO' and p.idtipoprodserv in (".$_REQUEST['idtipoprodserv'].")
    and exists(select 1 from prodservformulains i where i.idprodserv=p.idprodserv and i.status ='ATIVO') order by descr";
    ...
    $linha=0;
    while ($rowpd = mysql_fetch_assoc($_resun)){
        if($linha>0){
?>
