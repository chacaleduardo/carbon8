 ...
 */
 if($_GET and !empty($clausulad)){
    ...
    $sql="SELECT  p.razaosocial,c.*,dma(c.datareceb) as dtreceb,dma(n.dtemissao) as dtemissao,CASE c.status    WHEN 'QUITADO' THEN 1
                ...
                ".$strtipo."
                and c.status = 'QUITADO'  order by  p.razaosocial,n.dtemissao asc,ordem asc,c.tipo asc,c.quitadoemseg";
...
<html>
<head>
<title>Sislaudo - Contas Pagas</title>
...
<table class="tbrepheader">
	...
	<tr>
		...
		<td class="header">Relatório Contas Pagas - (<?=$agencia?>)</td>
...
<table class="normal" style="font-size: 10px;">
	...
	$conteudoexport='"Data";"Fornecedor";"N NF";"Parcela";"Valor"';
...
<?	
	$conteudoexport.='"'.$row["dtreceb"].'";"'.$favorecido.'";"'.$descnf.'";"'.$row["parcela"].'de'.$row["parcelas"].'";"'. $row["valor"].'";';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
...
<?
        $conteudoexport.='"'.$row["dtreceb"].'";"'.$favorecido.'";"'.$rowx['nnfe'].'";"'.$rowx["parcela"].'de'.$rowx["parcelas"].'";"'. $rowx["valor"].'"';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
                    }//while($rowx=mysqli_fetch_assoc($resx)){
