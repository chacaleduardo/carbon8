...
if ($_idrep == 172 || $_idrep == 94 ||  $_idrep == 18 || $_idrep == 127 ||  $_idrep == 22 || $_idrep == 81 ||  $_idrep == 149 ||  $_idrep == 104 ||  $_idrep == 173){
...
if (!empty($_GET)){
	...
	if($_GET['_fds']!=""){
		...
		$filtrodata="AND (( n.dtemissao BETWEEN '".$data1." 00:00:00' AND '".$data2." 23:59:59'))";
	}
	
	if($_GET['idsgdepartamento']!=""){
		$filtrodepartamento= "AND u.idunidade in (".$_GET['idsgdepartamento'].")";
	}
	if($_GET['idcontaitem']!=""){
		$filtrocontaitem= "AND ci.idcontaitem in (".$_GET['idcontaitem'].")";
	}
	if($_GET['idtipoprodserv']!=""){
		$filtrotipoprodserv= "AND t.idtipoprodserv in (".$_GET['idtipoprodserv'].")";
	}

	if($_GET['idpessoa']!=""){
		$filtroidpessoa= "AND i.idpessoa in (".$_GET['idpessoa'].")";
	}

	if($_GET['idempresa']!=""){
		$filtroempresa= "AND e.idempresa in (".$_GET['idempresa'].")";
	}
	...
	$_sqlresultado = "SELECT	
							...
							ifnull(i.prodservdescr,p.descr) as item,
							ci.contaitem, 
							t.tipoprodserv,							
							CAST(ifnull(SUM(i.vlritem*(rid.valor/100)),0)  AS DECIMAL (10 , 2 ))  AS valor	
						-- COUNT(DISTINCT i.idpessoa) AS qtdpessoa
						FROM nfitem i
						JOIN nf n ON (n.idnf = i.idnf AND n.tiponf IN ( 'R') AND n.tipocontapagar = 'D')
						JOIN rateioitem ri ON ri.tipoobjeto = 'nfitem' AND ri.idobjeto = i.idnfitem
						JOIN rateioitemdest rid ON rid.idrateioitem = ri.idrateioitem AND rid.tipoobjeto = 'unidade'
						JOIN unidade u ON u.idunidade = rid.idobjeto  
						join contaitem ci on(ci.idcontaitem = i.idcontaitem)
						join tipoprodserv t on(t.idtipoprodserv = i.idtipoprodserv)
						-- JOIN sgdepartamento d ON d.idsgdepartamento = u.idobjeto AND u.tipoobjeto = 'sgdepartamento'
						...
						WHERE
							...
							".$filtrocontaitem."
							and i.vlritem>0
							GROUP BY  i.idpessoa,contaitem,tipoprodserv,item
							 ORDER BY item,contaitem,tipoprodserv asc";
