	...
	if (!empty($_REQUEST['csv'])){
		...
		$conteudoexport = ("
	Cliente;R. Social;Endereço;Registro;Data Registro;Amostra;Estudo;Descrição;Data Coleta;Lacre;Galpão/Aviário;Linha;Local Coleta;TC;Tipo;Data Fabricação;Responsável;Resp. Oficial;Sexo;N.Doses;Partida;Especificações;Fornecedor;Nota Fiscal;Vencimento;Semana;Identificação/Chip;Diluições;N. Placas;Fabricante;Sexadores;Local Específico;N. Clifor/Pedido;N. SVO;CPF/CNPJ;Cidade;Vol. Aplicado;N Doses;Via de Inoculação;Cor da Anilha;Registro Biotério;Granja;Tipo Aves;Espécie;Núcleo;Núcleo Origem;Cliente Terceiro;N. Reg. Oficial;Orgão Oficial;Lote;Idade;N. de Animais;Local;Observações;Amostra(s) testada(s);ID Teste;Resultado;Interpretação;Considerações"."\n");
	...
	$c++;
		...
		if (empty($row["dataconclusao"])){
			$sqla = "select DATE_FORMAT(max(criadoem), '%d/%m/%Y') as dataconclusao from _auditoria 
						where 
							idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." and 
			...
			if (empty($_REQUEST['csv'])){
			   ...
			   if ($modo == 'IND'){
				...
				$conteudoexport.=(
					...
					$nroamostra." ".$row['tipoamostraformatado'].";".
					$rejeitada." ".$row['rejeitada'].";".
					$row['estexterno'].";".
					$row['descricao'].";".
					$datacoleta.";".
					...
					$row["quantidadeteste"].";".
					$_idresultado.";".
					str_replace($indesejaveis, "", $resultado).";".
					$templateinterpretacao.";".
					$consideracoes.'
				'); 
			}
