		...
		SELECT * FROM 
			(
				SELECT 
					nomeativ,
					acao,
					concat(round(valor,1),' ',b.unidade) as valor,
					h.tipo,
					sensor,
					registradoem,
					iddevice,
				...
				WHERE
					h.iddevice = ".$_REQUEST['iddevice']." and h.grupo = '".$_REQUEST['grupo']."'
				ORDER BY registradoem DESC 
			) AS vw8devicesensorhistagrupado" ;
...
<?
    $sqlcab = "SELECT 
				...
				FROM
					...
					devicesensorbloco b ON b.iddevicesensorbloco = h.iddevicesensorbloco
				WHERE
						h.iddevice = ".$_REQUEST['iddevice']." 
						AND grupo = '".$_REQUEST['grupo']."' 
				GROUP BY grupo";
...
?>
	<table class="normal">
		<tr class='header'>
			...
			<td colspan = "3" class=''  style="height: 20px;border-left: none !important; text-align:right !important; font-weight: bold;">Ciclo: <?=$_rowcab['nomeciclo']?></td>
		</tr>
		<tr class='res'>
...
//print_r($arrRep);
	...
	//$strtabheader .= "\n<div>Detalhes Ciclo</div>";

	//$strtabheader .= "\n<thead><tr class='header'><td>Sala: ".$_rowcab['descricao']."</td><td colspan = '5' style='text-align:center'>Ciclo: ".$_rowcab['nomeciclo']."</td></tr>";

	//$strtabheader .= "<tr class='res'><td>Início</td><td>".$_rowcab['mindate']."</td><td>Fim</td><td>".$_rowcab['maxdate']."</td><td>Duração</td><td>".$_rowcab['dif']."</td></tr>";

	//$strtabheader .= "<tr class='res'><td>Minimo</td><td>".$_rowcab['minvalor']."</td><td>Máximo</td><td>".$_rowcab['maxvalor']."</td><td>Média</td><td>".$_rowcab['media']."</td></tr>";
