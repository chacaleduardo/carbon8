...
if (!empty($_GET)){
    ...
    */
    if($_SESSION["SESSAO"]["IDTIPOPESSOA"]==3){//usuariocliente
        ...
        $_sqlwhere .= $_and . "  (idunidade=6 or idunidade=9) ";
        $_iclausulas++;
    }elseif($_SESSION["SESSAO"]["IDTIPOPESSOA"]==4){

        $_sqlwhere .= $_and . " idsecretaria in(".$_SESSION["SESSAO"]["STRCONTATOSECRETARIA"].")";

        $_sqlwhere .= $_and . " flgoficial ='Y'";
    ...
    if($_SESSION["SESSAO"]["OBRIGATORIOCONTATO"]=="Y"  and array_key_exists("idpessoa", $_arrpagpsq[$_arrModConf["tab"]])){
		if(empty($_SESSION["SESSAO"]["STRCONTATOCLIENTE"])){
            ...
            $_sqlwhere .= $_and . " idpessoa in( ".$_SESSION["SESSAO"]["STRCONTATOCLIENTE"].",".$_SESSION["SESSAO"]["IDPESSOA"].") ";
            $_iclausulas++;
		}
	}else{
		if(array_key_exists("STRCONTATOCLIENTE", $_SESSION["SESSAO"]) and array_key_exists("idpessoa", $_arrpagpsq[$_arrModConf["tab"]])){
			$_sqlwhere .= $_and . " idpessoa in( ".$_SESSION["SESSAO"]["STRCONTATOCLIENTE"].",".$_SESSION["SESSAO"]["IDPESSOA"].") ";
            $_iclausulas++;
		}elseif($_SESSION["SESSAO"]["IDTIPOPESSOA"]!=1 and $_SESSION["SESSAO"]["IDTIPOPESSOA"]!=113 and array_key_exists("idpessoa", $_arrpagpsq[$_arrModConf["tab"]] )){
			cbSetPostHeader("0","alert");	
			die("Sua sessão expirou, gentileza deslogar e logar novamente.<br><a href='?_acao=logout'>Deslogue aqui.</a>");
		}
	...
	if(in_array($_arrModConf["tab"],$arrbypassempresa)){
		$_idempresa = "";//Nao adiciona o campo IDEMPRESA nas clausulas where
	}else{
		...
		//	$_SESSION["SEARCH"]["WHERE"]["idempresa"] = " 1 ".getidempresa('idempresa',$_arrModConf["tab"]);
			$_idempresa = getidempresa('idempresa',$_modulo);
		}
	}
	
	if($_iclausulas > 0){
		$_sqlresultado = getDbTabela($_tab).".". $_tab ." ".$_sqlwhere;
		$_sqlresultado .= $_idempresa;
	}else{
		$_sqlresultado = getDbTabela($_tab).".". $_tab." a";
		$_sqlresultado .= " where 1 ".$_idempresa;
	}
	...
	if(array_key_exists("STRCONTATOCLIENTE", $_SESSION["SESSAO"]) and array_key_exists("idpessoa", $_arrpagpsq['vwpessoa'])){
