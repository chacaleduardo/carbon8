 ...
 */
 if($_GET and !empty($clausulad)){
    ...
    $sql="SELECT  p.razaosocial,c.*,dma(c.datareceb) as dtreceb,dma(n.dtemissao) as dtemissao,CASE c.status    WHEN 'QUITADO' THEN 1
                ...
                ".$strtipo."
                and c.status = 'PENDENTE'  order by  p.razaosocial,n.dtemissao asc,ordem asc,c.tipo asc,c.quitadoemseg";
...
<table class="normal" style="font-size: 10px;">
	...
	$conteudoexport='"Data";"Parcela";"Pessoa";"Item";"Obs.";"Valor";"Tipo";"Status"';
	...
	while ($row = mysqli_fetch_array($res)){
	    ...
	    if($row["tipoobjeto"] == "nf" and !empty($row["idobjeto"] )){
			...
			$sqlf = "select n.idnf,ifnull(p.razaosocial, p.nome) as nome,n.tiponf,n.controle,n.nnfe from nf n,pessoa p where p.idpessoa = n.idpessoa and p.idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." and idnf =".$row["idobjeto"];
			...
			$sqlf = "select ifnull( p.razaosocial,  p.nome) as nome,n.numerorps,n.nnfe,n.idnotafiscal from notafiscal n,pessoa p where p.idempresa = ".$_SESSION["SESSAO"]["IDEMPRESA"]." and p.idpessoa = n.idpessoa and idnotafiscal =".$row["idobjeto"];
...
<?	
	$conteudoexport.='"'.$row["dtreceb"].'";"'.$row["parcela"].'de'.$row["parcelas"].'";"'.$descnf.'";"'.$favorecido.'";"'.$intemext.'";"'.$row["obs"].'";"'. $row["valor"].'";"'.$row["tipo"].'";"'.$row["status"].'"';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
                ...
                if($row['tipoespecifico']=='AGRUPAMENTO' AND $row['tipoobjeto']=='nf' and !empty($row['idobjeto'])){
                    $sqlx=" select i.parcela,i.parcelas,ci.contaitem,p.nome,n.nnfe,i.valor,i.status,dma(n.dtemissao) as dtemissao
                            from contapagaritem i join nf n on(n.idnf=i.idobjetoorigem)
...
<?
        $conteudoexport.='"'.$row["dtreceb"].'";"'.$rowx["parcela"].'de'.$rowx["parcelas"].'";"'.$rowx['nnfe'].'";"'.$rowx['nome'].'";"'.$rowx['contaitem'].'";"'.$descnf.'-'.$favorecido.'";"'. $rowx["valor"].'";"D";"'.$rowx["status"].'"';
	$conteudoexport.="\r\n";//QUEBRA DE LINHA NO CONTEUDO CSV
                    }//while($rowx=mysqli_fetch_assoc($resx)){
