...
function aplicaMascaraSemRS($mascara, $val){
...
if (!empty($_GET)){
	 ...
	 ****************************************************************************/
	$sqcustoun=$_sqlresultado.$_sqldata.$_and_idempresa.$str_fts.$strgrp.$strord;


	$sqcustoun = preg_replace("/and idprodserv in \('\\d+'\\)/", "", $sqcustoun);

	$strselectfieldssomacustoun="select  round(sum(lotecustototal),2) as somcustototal ";
	$relatorioscustoun = MenuRelatorioController::buscarRelatorioDinamico($strselectfieldssomacustoun, $sqcustoun);

/*
	$strselectfieldssomacustoun="select  round(sum(lotecustototal),2) as somcustototal ";
	$relatorioscustoun = MenuRelatorioController::buscarRelatorioDinamico($strselectfieldssomacustoun, ($_sqlresultado.$_sqldata.$_and_idempresa.$str_fts.$strgrp.$strord));
*/
	
	foreach($relatorioscustoun as $_rcun){
		$custoUnS=$_rcun['somcustototal'];
	}
	//echo("Custo un".$custoUnS.'<br>');

	$_sqldatamo = str_replace("dtemissao", "cp.datareceb", $_sqldata);
	$strselectfieldsmo="select sum(total) as totalmo";	
	$querymo =" (
		select  i.total
		from contapagaritem c 
		join contapagar cp on (cp.idcontapagar=c.idcontapagar) 
		join nf n on(c.idobjetoorigem = n.idnf)
		join nfitem i on(i.idnf=n.idnf and i.nfe='Y' and i.idcontaitem=508)
		where 
		  c.tipoobjetoorigem = 'nf'
			AND n.status = 'CONCLUIDO'
		  AND cp.status!='INATIVO'
		  and cp.idempresa =".cb::idempresa()."
		  ".$_sqldatamo."
		  UNION  
		  select i.total as total
		from  contapagar cp  
		join nf n on(cp.idobjeto = n.idnf)
		join nfitem i on(i.idnf=n.idnf and i.nfe='Y' and i.idcontaitem=508)
		where  cp.tipoobjeto = 'nf'
	...
	$relatoriosmo = MenuRelatorioController::buscarRelatorioDinamico($strselectfieldsmo,$querymo);

	foreach($relatoriosmo as $_rmo){
		$custoMO=$_rmo['totalmo'];
	}
	//echo("Custo MO".$custoMO.'<br>');
...
<?
    ...
    foreach($relatorios as $_row)
	{
		...
		foreach(array_keys($_row) as $key => $coluna)
		{
			...
			$_row['custounfab']=(((($_row['lotecustoun']*100)/($custoUnS))*($custoMO))/(100));
			

			$_row['totalun']=$_row['lotecustoun']+$_row['custounfab']+$_row['freteun']+$_row['valipiun'];

			$_row['custoitemfab']=$_row['custounfab']*$_row['qtd'];

			$_row['totalcusto']=$_row['lotecustototal']+$_row['custoitemfab']+$_row['frete']+$_row['valipi'];

			if(empty($_row['totalcusto'])){
				$_row['porcentagemcustoun']= $_row['valortotalnfitem']/1;
			}else{
				$_row['porcentagemcustoun']= $_row['valortotalnfitem']/$_row['totalcusto'];

			}

			

			$_row['dif']= $_row['valortotalnfitem'] - $_row['totalcusto'];
    	    ...
    	    if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y'){
				...
				$_strvlrhtml=aplicaMascaraSemRS($arrRep["_filtros"][$_nomecol]["mascara"], $_strvlrhtml);					
...
<?		
		...
		foreach(array_keys($relatorios[0]) as $key => $coluna)
		{
    		...
    		if($arrRep["_filtros"][$_nomecol]["visres"] == 'Y' and $arrRep["_filtros"][$_nomecol]["acsum"]=='Y'){ 			
				...
				echo(aplicaMascaraSemRS($arrRep["_filtros"][$_nomecol]["mascara"],$_arrsoma[$_tab][$_nomecol]));							
				echo("</td>");
    			
    		}elseif($arrRep["_filtros"][$_nomecol]["visres"] == 'Y' and $arrRep["_filtros"][$_nomecol]["acavg"]=='Y'){

				echo("<td class=\"tot\" style=\"text-align:".$arrRep["_filtros"][$_nomecol]["align"]."\">");
				echo(aplicaMascaraSemRS($arrRep["_filtros"][$_nomecol]["mascara"],($_arrsomaavg[$_tab][$_nomecol]/$_ilinha)));					
				echo("</td>");
