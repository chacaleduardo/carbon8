...
<body>
	 ...
	 */
		...
		$strtabheader .= '</tr></thead><tbody style="font-size:11px">';
	 ...
	 */
		...
		foreach ($relatorios as $_row) {
			...
			echo "\n<tr class=\"res".$_row['ultimo_lote']."\" " . $_link . " " . $_strhlcolor . ">";
		 ...
		 */
				 ...
				 */
																		if ($arrRep["_filtros"][$_nomecol]["datatype"] == 'longtext') {
																			...
																			$_strvlrhtml = number_format($_row[$coluna], 4, ',', '.');
																		} else {
																		...
																		if (!empty($arrRep["_filtros"][$_nomecol]["hyperlink"])) {
																			...
																			if ($_nomecol == 'confirmar') {
																				echo "<td " . $_attrHtml . " " . $_stralign . " " . $_hyperlink . " " . $_colorlink . '><button type="button" class="btn btn-primary" onclick="confirmavalor(' . $_row['ultimo_lote'] . ')">' . $_corfont . $_strvlrhtml . $_corfontfim . "</button></td>";
																			} else {
...
<script class="normal">
	...
	function confirmavalor(value) {
		var userConfirmed = 
		confirm("Você deseja confirmar o lote " + value + "? Essa ação não poderá ser desfeita.");
		if (userConfirmed) {
			//vamos fazer o carbon funcionar aqui.
			CB.post({
            objetos:  '_x_u_lote_idlote='+value+'&_x_u_lote_aprova=1&_x_u_lote_useraprova=<?=$_SESSION["SESSAO"]["USUARIO"]?>',
            parcial: false,
			refresh: false
        });
			$(".res"+value).closest('tr').remove();
			alert("Lote "+value+" aprovado!");
