...
$produtosEmAlerta = EstoqueFormuladosController::buscarProdutosFormuladosEmAlertaDeProducao($instrvenda, $idUnidadePadrao, true, $_modulo);
...
<table class="table table-striped ">
	<thead>
		...
		<th style="width: 130px;"><a class="btn btn-success no-print" href="<?= $_SERVER['REQUEST_URI'] ?>&reportexport=csv" target="_blank">Exportar .csv</a></th>
	</thead>
</table>
<table id="restbl" class="table table-striped fixarthtable">
	...
	<tbody class="restblbody">
		<?
		$conteudoexport;
		$conteudoexport = '"Descrição";"Estoque";"Estoque em Falta";"Est. Futuro";"Est. Futuro em Falta";"Estoque Mínimo"';
		$conteudoexport .= "\n";
		...
		foreach ($arrayProdutosEmAlerta as $key => $row) 
		{
			...
			if($row['idunidade'] != $idunidadeAnt && count(explode(",", $idUnidadePadrao)) > 1)
			{
				?>
			...
			<tr style="background-color: <?=$cortr?>;">
				...
				<td class="center valign col-md-1" filterValue="<?=$estoqueFaltaTd ?>">
					<?=$estoqueFaltaTd ?>
				</td>	
				...
				<td class="center valign col-md-1" filterValue="<?=$estoqueFuturoTD?>">
					<?=$estoqueFuturoTD ?>
				</td>		
				...
				<td class="center valign col-md-1" filterValue="<?=$estoqueFuturofaltaTd?>">
					...
					<? if (!empty($row["idloteprod"])) {
						foreach ($lotesFormalizacaoEmAndamento as $key => $rowqua1) {
							if ($_GET['_modulo'] == "lotealertapesqdes") { ?>
								<span>
									<a class="twrap" title="<? $titulomodulo ?>" target="_blank" href="./?_modulo=<?=$linklote ?>&_acao=u&idlote=<?=$rowqua1["idlote"] ?>">
							...
							<? } else { ?>
								<span>
									<a class="twrap" target="_blank" href="./?_modulo=<?=$row['modulo']?>&_acao=u&idformalizacao=<?=$rowqua1["idformalizacao"] ?>&_idempresa=<?=$row['idempresa'] ?>">
										<?=$rowqua1["status"] ?> / <?=$rowqua1["rotulo"] ?> - <?=$rowqua1['partida']  ?> 
									</a><br>
								</span>
							<? }
			...
			<? 
			$conteudoexport .= '"'.$row['descr'].' - '.$row['rotulo'].'";"'.$estoque.'";"'.$estoqueFaltaTd.'";"'.$estoqueFuturoTD.'";"'.$estoqueFuturofaltaTd.'";"'.$estoqueMin.'"';
			$conteudoexport .= "\r\n";
		} ?>
	</tbody>
</table>
...
require_once("../form/js/estoqueformulados_js.php"); 
?>