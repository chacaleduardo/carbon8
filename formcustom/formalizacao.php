...
if (!empty($_2_u_lote_idlote) && !empty($_2_u_lote_idprodservformula)) 
{
	...
	if (in_array($_1_u_formalizacao_status, $statusFormalizacao)) {
		...
		$arrLoteAtiv = FormalizacaoController::buscarLoteAtividade($_2_u_lote_idlote, null, $_GET["_modulo"]);
	} else {
		//Seta o modudlo da formalizacao para pegar o hist ($pagvalmodulo)
		$arrLoteAtiv = FormalizacaoController::buscarLoteAtividade($_2_u_lote_idlote, 'Y', $_GET["_modulo"]);
	}
...
<div class="row screen">
	<div class="col-md-12 loteativ" idloteativ="000000">
		<div class="panel panel-default" style="font-size: 11px;">
			<div class="panel-heading">
				...
				<div class="row d-flex d-md-block flex-wrap align-items-end px-2 px-md-0 py-2">
					...
					if (!empty($_2_u_lote_idlote)) 
					{
						...
						if ($_1_u_formalizacao_status != "TRIAGEM" || ($qtdLoteativ == 0 && $_1_u_formalizacao_status == "TRIAGEM")) {
						...
						<div class="d-flex flex-column col-xs-2">
							...
							<div class="d-flex align-items-center form-valor">
								...
								<input <?=$desablepr ?> class="calendario" id="dataproducao" size="6" type="text" name="_2_<?=$_acao ?>_lote_producao" value="<?=$_2_u_lote_producao ?>" vnulo autocomplete="off">
							</div>
						</div>
					<?
					}
					...
					<div class="d-flex flex-column col-xs-1">
						...
						<div class="d-flex align-items-center form-valor">
							...
							if ($_2_u_lote_idlote and (($prodForm["especial"] == "Y" and !empty($_2_u_lote_idsolfab)) or ($prodForm["especial"] == "N"))) 
							{
								...
								<input type="hidden" name="_1_<?=$_acao ?>_formalizacao_status" id="idformalizacao" value="<?=$_1_u_formalizacao_status ?>" vnulo>
				...
				<div class="imprimirFormalizacao">
					<i class="fa fa-print fa-2x  fade pointer hoverazul  btn-lg pointer" onclick="imprimir()" title="Imprimir Formalização"></i>
				</div>
				<?
				if ((!empty($_2_u_lote_idlote) && $prodForm['tipo'] != 'SERVICO' && $_GET['_acao'] == 'u') || $_GET['_acao'] == 'i') 
				{
					...
					<div class="row d-flex d-md-block flex-wrap align-items-end px-2 px-md-0 py-2">
						...
						if ($prodForm['tipo'] != 'SERVICO') 
						{
							...
							<div class="d-flex flex-column col-xs-1">
								...
								<div class="d-flex align-items-center form-valor">
									...
									$valorQtdSol = empty($valorQtdSol) ? "" : $valorQtdSol;
									?>
						...
						?>
						<!-- Formula -->
						<div class="d-flex flex-column col-xs-5">
							<label>Fórmula:</label>
				...
				if (($prodForm["especial"] == "Y" && $_GET['_acao'] == 'u') || $_GET['_acao'] == 'i') 
				{
					...
					<div class="row d-flex d-md-block flex-wrap align-items-end px-2 px-md-0 py-2">
						...
						<div class="d-flex flex-column col-xs-3 div_sol_fab" style="<?=$displayNone?>">
							<label>Solicitação Fabricação:</label>
							<div class="d-flex align-items-center form-valor">
								<?
								...
								?>
								<span class="input-group-addon pointer hoverazul flex-center" onclick="janelamodal('?_modulo=solfab&_acao=u&idsolfab=<?=$_2_u_lote_idsolfab ?>')" title="Editar Solicitação de Fabricação #<?=$_2_u_lote_idsolfab ?>">
									<i class="fa fa-bars pointer"></i>
								</span>
								<span class="input-group-addon pointer hoververde flex-center" onclick="geraSolfab()" title="Nova Solicitação de Fabricação">
			...
			<?
			//if ($_1_u_formalizacao_status != "ABERTO" && $_1_u_formalizacao_status != "AGUARDANDO" && !empty($_2_u_lote_idlote) && $prodForm['tipo'] != 'SERVICO') 
			//{
				...
				<div class="panel-body">
					<div class="row d-flex d-md-block flex-wrap align-items-end px-2 px-md-0">
						...
						<div class="d-flex flex-column col-xs-2">
							...
							<div class="d-flex align-items-center form-valor">
								<input class="size10 mgRightOito" <?=$peddisabled ?> name="_2_<?=$_acao ?>_lote_qtdajust" size="4" type="text" value="<?=recuperaExpoente(tratanumero($_2_u_lote_qtdajust), $_2_u_lote_qtdajust_exp) ?>" vnulo> 
								<?=$unidadeVolume?>
							</div>
						</div>
						...
						<div class="d-flex flex-column col-xs-2">
							...
							<div class="d-flex align-items-center form-valor">
								...
								?>
								<input class="size10 mgRightOito" name="_2_<?=$_acao ?>_lote_qtdprod" size="4" type="text" value="<?=$valprod ?>" <?=$strnul ?>>
								<?=$unidadeVolume?>
							</div>
						</div>
...
<?
if (!empty($_2_u_lote_idprodservformula) && !empty($_2_u_lote_idpessoa) && !empty($_2_u_lote_idsolfab) && ($_1_u_formalizacao_status == "ABERTO" or $_1_u_formalizacao_status == 'AGUARDANDO')) 
{
	...
	if ($qtdsf > 0) 
	{
		...
		<div class="row screen">
			<div class="col-md-12">
				<div class="panel panel-default ">
					<div class="panel-heading">Sementes que não estão na Solicitação de fabricação</div>
	...
	<thead>
		<tr>
			<th class="print tituloPagina">
				...
				<? } ?>
				<div class="tituloDoc" style="font-size: 0.4cm;"><? if ($_2_u_lote_tipo == 'SERVICO') { ?>PROTOCOLO<? } else { ?>ORDEM DE PRODUÇÃO<? } ?><div class="cinza quebralinha" style="font-size: 0.4cm;"><?=$prodForm["descr"] ?></div>
				</div>
				...
				<th class="print cabecalhoPagina">
					<table>
						<tr>
							...
							<td><?//=$_2_u_lote_fabricacao?></td>
							<td>Data Vencimento:</td>
							<td><?//=$_2_u_lote_vencimento?></td>
						</tr>
						<tr>
							<td>Qtd. Produzida:</td>
							<td></td>
...
<? require_once('../form/js/formalizacaonova_js.php'); ?>